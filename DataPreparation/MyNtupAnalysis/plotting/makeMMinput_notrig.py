...
## Zmet
trigger_setup = {}
trigger_setup["2e12_lhloose_L12EM10VH"] = {"color":kOrange+10, "year":[2015],"marker":22} 
trigger_setup["2e17_lhvloose_nod0"] = {"color":kOrange+10, "year":[2016],"marker":32}  
trigger_setup["2e24_lhvloose_nod0"] = {"color":kOrange+10, "year":[2017,2018],"marker":22}  
trigger_setup["2e17_lhvloose_nod0_L12EM15VHI"] = {"color":kOrange+10, "year":[2017,2018],"marker":25}   
trigger_setup["mu26_imedium"] = {"color":kOrange+10, "year":[2015],"marker":26}  
trigger_setup["mu26_ivarmedium"] = {"color":kOrange+10, "year":[2016,2017,2018],"marker":27}   
trigger_setup["mu50"] = {"color":kOrange+10, "year":[2015,2016,2017,2018],"marker":28}
...
if "light" in typ:
    ...
    doPlot = [mckey, 'Zeejets', 'Wjets', 'data15', 'singletop', 'Diboson', 'ttbar', 'Zmmjets', 'Zttjets']
    ...
    for key in triglist.keys():
        ...
        notrig.append("h_lep_pT_nT_%slepnotrig_ALL_E_trueLightFlavorDecay_FAKE2L05%s_2LTrig_%s"%("%s_"%key if key else "","_%s"%uncert if uncert else "",analy))
    for key in trigger_setup.keys():
        if key.startswith("m_"): continue
        trig[key] = "h_lep_pT_nT_%sALL_E_trueLightFlavorDecay_FAKE2L05%s_2LTrig_%s"%("%s_"%key if key else "","_%s"%uncert if uncert else "",analy)
elif "conv" in typ:
    doPlot = ["Zeejets",datakey]
    ...
    for key in triglist.keys():
        ...
        for yr in tryear:
            if yr in triglist[key]["year"]:
                ...
                break
        if not isOK: continue
        notrig.append("h_lep_pT_nT_%slepnotrig_ALL_E_FAKE2L23_2LTrig_%s"%("%s_"%key if key else "",analy))
    for key in trigger_setup.keys():
        trig[key] = "h_lep_pT_nT_%sALL_E_FAKE2L23_2LTrig_%s"%("%s_"%key if key else "",analy)
    ...
    for key in triglist.keys():
        ...
        if lep == "mu":
            notrig.append("h_lep_eta_nT_%slepnotrig_MMOS_M_REAL2L04%s_2LTrig_%s"%("%s_"%key if key else "","_%s"%uncert if uncert else "",analy))
        elif lep == "el":
            notrig.append("h_lep_eta_nT_%slepnotrig_EEOS_E_REAL2L04%s_2LTrig_%s"%("%s_"%key if key else "","_%s"%uncert if uncert else "",analy))
    for key in trigger_setup.keys():
        if lep == "mu":
            ...
            if not usetriggerbytrigger:
              trig[key] = "h_lep_eta_nT_%sMMOS_M_REAL2L04%s_2LTrig_%s"%("%s_"%key if key else "","_%s"%uncert if uncert else "",analy)
            else:
              trig[key] = "h_lep_eta_nT_%sMMOS_M_REAL2L04%s_2LTrig_%s"%("%s_"%key if key else "","_%s"%uncert if uncert else "",analy)
        elif lep == "el":
            ...
            if not usetriggerbytrigger:
              trig[key] = "h_lep_eta_nT_%sEEOS_E_REAL2L04%s_2LTrig_%s"%("%s_"%key if key else "","_%s"%uncert if uncert else "",analy)
            else:
              trig[key] = "h_lep_eta_nT_%sEEOS_E_REAL2L04%s_2LTrig_%s"%("%s_"%key if key else "","_%s"%uncert if uncert else "",analy)
elif "real1d" in typ:
    vari = "pT"
    ...
    for key in triglist.keys():
        ...
        #print("Trigger %s is ok for year %s"%("%s_"%key if key else "",fnkey))
        if lep == "mu":
            notrig.append("h_lep_%s_nT%s_%slepnotmatched_MMOS_M_REAL2L01%s_2LTrig_%s"%(vari,"_"+BDTscore if BDTscore else "","%s_"%key if key else "","_%s"%uncert if uncert else "",analy))
        elif lep == "el":
            notrig.append("h_lep_%s_nT%s_%slepnotmatched_EEOS_E_REAL2L01%s_2LTrig_%s"%(vari,"_"+BDTscore if BDTscore else "","%s_"%key if key else "","_%s"%uncert if uncert else "",analy))
    for key in trigger_setup.keys():
        if lep == "mu":
            if vari == "pT": rebx = array.array('f',[0,15,20,30,40,60,100,1000])
            ytit = "Real Efficiency (muons)"
            xtit = "p_{T}^{#mu} [GeV]"
            #notrig.append("h_lep_%s_eta_nT_%slepnotrig_MMOS_M_REAL2L01%s_%s"%("%s_"%key if key else "","_%s"%uncert if uncert else ""))
            if (not usetriggerbytrigger and key.startswith("e_")) or (usetriggerbytrigger and key.startswith("e")): continue
            if not usetriggerbytrigger:
              trig[key] = "h_lep_%s_nT%s_%sMMOS_M_REAL2L01%s_2LTrig_%s"%(vari,"_"+BDTscore if BDTscore else "","%s_"%key if key else "","_%s"%uncert if uncert else "",analy)
            else:
              trig[key] = "h_lep_%s_nT%s_%sMMOS_M_REAL2L01%s_2LTrig_%s"%(vari,"_"+BDTscore if BDTscore else "","%s_"%key if key else "","_%s"%uncert if uncert else "",analy)
        elif lep == "el":
            ytit = "Real Efficiency (electrons)"
            xtit = "p_{T}^{el} [GeV]"
            #notrig.append("h_lep_%s_eta_nT_%slepnotrig_EEOS_E_REAL2L01%s_%s"%("%s_"%key if key else "","_%s"%uncert if uncert else ""))
            if (not usetriggerbytrigger and key.startswith("m_")) or (usetriggerbytrigger and key.startswith("m")): continue
            if not usetriggerbytrigger:
              trig[key] = "h_lep_%s_nT%s_%sEEOS_E_REAL2L01%s_2LTrig_%s"%(vari,"_"+BDTscore if BDTscore else "","%s_"%key if key else "","_%s"%uncert if uncert else "",analy)
            else:
              trig[key] = "h_lep_%s_nT%s_%sEEOS_E_REAL2L01%s_2LTrig_%s"%(vari,"_"+BDTscore if BDTscore else "","%s_"%key if key else "","_%s"%uncert if uncert else "",analy)
elif "real" in typ:
    doPlot = [datakey]
    rebx = array.array('f',[0,20,25,30,35,40,45,60,70,80,100,1000])
    ...
    for key in triglist.keys():
        ...
        #print("Trigger %s is ok for year %s"%("%s_"%key if key else "",fnkey))
        if lep == "mu":
            notrig.append("h_lep_pT_eta_nT_%slepnotrig_MMOS_M_REAL2L04%s_2LTrig_%s"%("%s_"%key if key else "","_%s"%uncert if uncert else "",analy))
        elif lep == "el":
            notrig.append("h_lep_pT_eta_nT_%slepnotrig_EEOS_E_REAL2L04%s_2LTrig_%s"%("%s_"%key if key else "","_%s"%uncert if uncert else "",analy))
    for key in trigger_setup.keys():
        if lep == "mu":
            rebx = array.array('f',[0,20,25,30,35,40,45,60,70,80,100,1000])
            ...
            ytit = "|#eta^{#mu}|"
            xtit = "p_{T}^{#mu} [GeV]"
            #notrig.append("h_lep_pT_eta_nT_%slepnotrig_MMOS_M_REAL2L04%s_%s"%("%s_"%key if key else "","_%s"%uncert if uncert else ""))
            if key.startswith("e_"): continue
            trig[key] = "h_lep_pT_eta_nT_%sMMOS_M_REAL2L04%s_2LTrig_%s"%("%s_"%key if key else "","_%s"%uncert if uncert else "",analy)
        elif lep == "el":
            rebx = array.array('f',[0,20,25,30,35,40,45,60,70,80,100,1000])
            reby = array.array('f',[0.0,0.4,1.0,1.37,1.52,2.0,2.7])
            ytit = "|#eta^{el}|"
            xtit = "p_{T}^{el} [GeV]"
            #notrig.append("h_lep_pT_eta_nT_%slepnotrig_EEOS_E_REAL2L04%s_%s"%("%s_"%key if key else "","_%s"%uncert if uncert else ""))
            if key.startswith("m_"): continue
            trig[key] = "h_lep_pT_eta_nT_%sEEOS_E_REAL2L04%s_2LTrig_%s"%("%s_"%key if key else "","_%s"%uncert if uncert else "",analy)
elif "heavy" in typ:
    doPlot = [datakey]
    for key in triglist.keys():
        isOK = False
        ...
        if not isOK: continue
        if lep == "mu":
            notrig.append("h_lep_pT_nT_%slepnotmatched_ALL_M_FAKE2L21_2LTrig_%s"%("%s_"%key if key else "",analy))
            subt_notrig.append("h_lep_pT_nT_%slepnotmatched_ALL_M_trueREAL_FAKE2L21_2LTrig_%s"%("%s_"%key if key else "",analy))
        elif lep == "el":
            notrig.append("h_lep_pT_nT_%slepnotmatched_ALL_E_FAKE2L21_2LTrig_%s"%("%s_"%key if key else "",analy))
            subt_notrig.append("h_lep_pT_nT_%slepnotmatched_ALL_E_trueREAL_FAKE2L21_2LTrig_%s"%("%s_"%key if key else "",analy))
    for key in trigger_setup.keys():
        if lep == "mu":
            ...
            if not usetriggerbytrigger:
              trig[key] = "h_lep_pT_nT_%sALL_M_FAKE2L21_2LTrig_%s"%("%s_"%key if key else "",analy)
              subt_trig[key] = "h_lep_pT_nT_%sALL_M_trueREAL_FAKE2L21_2LTrig_%s"%("%s_"%key if key else "",analy)
            else:
              trig[key] = "h_lep_pT_nT_%sALL_M_FAKE2L21_2LTrig_%s"%("%s_"%key if key else "",analy)
              subt_trig[key] = "h_lep_pT_nT_%sALL_M_trueREAL_FAKE2L21_2LTrig_%s"%("%s_"%key if key else "",analy)
            ...
            if not usetriggerbytrigger:
              trig[key] = "h_lep_pT_nT_%sALL_E_FAKE2L21_2LTrig_%s"%("%s_"%key if key else "",analy)
              subt_trig[key] = "h_lep_pT_nT_%sALL_E_trueREAL_FAKE2L21_2LTrig_%s"%("%s_"%key if key else "",analy)
            else:
              trig[key] = "h_lep_pT_nT_%sALL_E_FAKE2L21_2LTrig_%s"%("%s_"%key if key else "",analy)
              subt_trig[key] = "h_lep_pT_nT_%sALL_E_trueREAL_FAKE2L21_2LTrig_%s"%("%s_"%key if key else "",analy)
...
for f in infiles:
    ...
    for nt in notrig:
        ...
        if objPtr and type(ht_notrig[proc]) is int: 
            ...
            #print("Adding %s from %s, now %.2f"%(nt,proc,ht_notrig[proc].Integral()))
        elif objPtr:
            ht_notrig[proc].Add(objPtr)
            #print("Adding %s from %s, now %.2f"%(nt,proc,ht_notrig[proc].Integral()))
        else:
            print("Could not find %s in %s"%(nt,indir+"/"+f))
        ...
        if objPtr and type(hl_notrig[proc]) is int: 
            ...
            print("Could not find %s in %s"%(nt.replace("nT","nL"),indir+"/"+f))
    ...
    for key in trig.keys():
        ...
        if objPtr and type(ht_trig[key][proc]) is int:
            ...
            print("Could not find %s in %s"%(trig[key],indir+"/"+f))
        ...
        if objPtr and type(hl_trig[key][proc]) is int:
            ...
            print("Could not find %s in %s"%(trig[key].replace("nT","nL"),indir+"/"+f))
        ...
        if typ == "heavy":
            if not key in ht_subt_trig.keys():
                ...
                ht_subt_trig[key][proc] = 0
            # Get first the tight histogram
            print("subt_trig tight[%s] = %s"%(key,subt_trig[key]))
            ...
            if objPtr and type(ht_subt_trig[key][proc]) is int:
                ...
                print("Could not find %s in %s"%(subt_trig[key],indir+"/"+f))
            ...
            if not key in hl_subt_trig.keys():
                ...
                hl_subt_trig[key][proc] = 0
            # Then get the loose histogram
            print("subt_trig loose[%s] = %s"%(key,subt_trig[key]))
            objPtr = mmfile.Get(subt_trig[key])
...
# Subtraction
for proc in ht_subt_notrig.keys():
    if "data" in proc: continue
...
# Subtraction
for proc in hl_subt_notrig.keys():
    if "data" in proc: continue
...
for key in hl_trig.keys():
    ...
    for proc in hl_trig[key].keys():
        if typ == "heavy" and proc == datakey:
            ht_trig_xsecup[key][proc], hl_trig_xsecup[key][proc] = doSubt(ht_trig[key][proc],hl_trig[key][proc],ht_trig[key][mckey],hl_trig[key][mckey],1.1)
            ht_trig_xsecdw[key][proc], hl_trig_xsecdw[key][proc] = doSubt(ht_trig[key][proc],hl_trig[key][proc],ht_trig[key][mckey],hl_trig[key][mckey],0.9)
            ht_trig[key][proc], hl_trig[key][proc] = doSubt(ht_trig[key][proc],hl_trig[key][proc],ht_trig[key][mckey],hl_trig[key][mckey])
...
for proc in hl_notrig.keys():
    ...
    if not "data" in proc:
      print("Adding %s"%proc)
      h_lT_notrig.Add(ht_notrig[proc])
    if typ == "heavy" and proc == datakey:
        ht_notrig_xsecup[proc], hl_notrig_xsecup[proc] = doSubt(ht_notrig[proc],hl_notrig[proc],ht_notrig[mckey],hl_notrig[mckey],1.1)
        ht_notrig_xsecdw[proc], hl_notrig_xsecdw[proc] = doSubt(ht_notrig[proc],hl_notrig[proc],ht_notrig[mckey],hl_notrig[mckey],0.9)
        ht_notrig[proc], hl_notrig[proc] = doSubt(ht_notrig[proc],hl_notrig[proc],ht_notrig[mckey],hl_notrig[mckey])
...
for key in heff_trig.keys():
    for proc in heff_trig[key].keys():
        ...
        if not heff_trig[key][proc].Integral(): continue
        if typ == "real":
        ...
        if proc in heff_trig_xsecup[key].keys() and not uncert:
            heff_trig_xsecup[key][proc].Write(prefix.replace(analy,key)+"_xsecup_"+proc if analy == "FCL" else prefix.replace(analy,lep)+"_xsecup_"+proc)
            heff_trig_xsecdw[key][proc].Write(prefix.replace(analy,key)+"_xsecdw_"+proc if analy == "FCL" else prefix.replace(analy,lep)+"_xsecdw_"+proc)
        heff_trig[key][proc].Write(prefix.replace(analy,key)+"_"+proc if analy == "FCL" else prefix.replace(analy,lep)+"_"+proc)
        print("Writing %s"%prefix.replace(analy,key)+"_"+proc if analy == "FCL" else prefix.replace(analy,lep)+"_"+proc)
...
for proc in heff_notrig.keys():
    ...
    if typ == "light" and not d_samp[proc]["leg"] in mcinleg:
      ...
      mcinleg.append(d_samp[proc]["leg"])
    if not typ == "light":
      leg.AddEntry(heff_notrig[proc],"m_notrig: "+d_samp[proc]["leg"],"fl")
...
if not typ == "real":
  ...
  for key in h_lT_trig.keys():
    ...
    d_max = -1
    if datakey in ht_trig[key].keys():
      d_max = ht_trig[key][datakey].GetMaximum()
      ht_trig[key][datakey].Draw("same ep")
