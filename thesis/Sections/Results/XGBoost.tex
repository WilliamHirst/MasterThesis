...
there are a total of 134 events with $\{250,400\}_{GeV}$\footnote{For the remainder of this thesis I will use the notation $\{A,B\}_{GeV}$, 
where A and B are the masses of $\tilde{\chi}_1$ and $\tilde{\chi}_2$ (which is the same as $\tilde{\chi}^{\pm}_1$), respectively.} compared to 6 events 
...
and most of the background is given a small output ($<0.1$). To further support the effect of statistics in the signal, we can take note of the 
...
that the bins are filled using the event-specific sample weights from the simulated data which explains why some bins
