...
where we observe a small distribution of signal values. This is due to the signal having some inherent differences
...
\begin{figure}
    ...
    \caption[A calculated visualization of the activation of a three layer maxout network, after training and displaying the
    results for two signal with each their own mass combination, highlighting the difference between two specific nodes.]{
...
\ref{subsec:LWTAARch}, channel-out, \ac{SCO} and maxout. The results presented in this section were made using the
original signal set (see section \ref{subsec:signal}), the training strategy described in section \ref{subsec:TrainingStrategy}
and the ensemble architectures described in section \ref{subsec:arch}.
\\
In figure \ref{fig:MaxOutGridSig} I present the expected sensitivity of the maxout model using the original signal set. 
