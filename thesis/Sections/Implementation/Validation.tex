...
\begin{figure}
    ...
    \makebox[0.7\linewidth][c]{%
        $
        \begin{array}{cccc}\label{table:Ref3L}
            \hline \text { Feature } & \text { $l_1$ } & \text { $l_2$ } & \text { $l_3$ } \\
            \hline\text{$p_t$} & - & \ref{fig:lep2_Pt} & \ref{fig:lep3_Pt}\\
        ...
        \end{array}
        \hfill
        \begin{array}{cc}\label{table:RefGen}
            \hline \text { Feature } & \text { $Refrence$ }  \\
            \hline\text{$\phi(miss)$} & \ref{fig:met_Phi} \\
        ...
        \caption{Refrences to figures for all event specific feature distribution.}
\end{figure}
...
This is exemplified in figure \ref{fig:lep1_Pt}, \ref{fig:lep1_Eta} and \ref{fig:met_Et}. In the $P_t$ figures we can 
clearly see that for (relatively) small $P_t$ ($<100GeV$) most events lay within a $0.9-1.1$ ratio. Whereas in for higher 
$P_t$ ($>200$), the errors move ratio to $0.8-1.2$. The difficulty of simulating \ac{SM} processes in high $P_t$ range is 
a known phenomenon, and not specific to this analysis. Thankfully, a smaller portion of the data lays in the high $P_t$-range
and therefore most of the data exhibits a solid reconstruction. By studying the figures in the appendix \ref{subsec:Dist}, we 
can deduce that this trend continues thought the full feature set. Simply put we observe that the \ac{MC} seems to adequately imitate 
