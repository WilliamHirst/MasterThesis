...
to tune to compensate. During testing, the dropout layers no longer take action. The prediction made by the resulting 
model can therefore be seen as the average of all the smaller networks, making a neural network with dropout-layers 
a bagging ensemble. \emph{CHECK SCALING OF WEIGHTS BEFORE PREDICTIONS.}
\\
Dropout-layers are not the only layers to dynamically change the architecture of a network. In this thesis I additionally 
explored the lesser known, \ac{LWTA}-layer. \ac{LWTA}, similarly to dropout creates an ensemble of networks by removing a set 
of nodes for each round of training. Contrary to dropout, \ac{LWTA} (or \emph{max-out}) does not choose the nodes to drop at random,
but instead creates local units by grouping the nodes, and then removes all nodes except the node with the largest
activation. The goal of max-out is to create en ensemble of networks where each network is specialized for a given 
trend in the data. 
\begin{figure}
    \centering
