...
going across the diagonal of the axis. This line is often drawn in \ac{ROC} curve. The better the 
...
\begin{figure}[H]
    ...
    \label{fig:ConfInt}
\end{figure}
Before attempting to search for deviations in the real data, we are interested in analysing the sensitivity of our method as well 
as calculate the expected number of signal event in our signal region. To study the sensitivity of an analysis one looks only at the 
...
0.95 (see figure \ref{fig:ConfInt}). There are many ways to inteprete this. A layman's intepratation (which is sufficient for this thesis), is 
...
For more information on significance and discovery in physics the reader is reffered to the following lecture \cite{magnar}.