...
class Particle(object):
    ...
    def add_decay(self, br, nda, ids):
        ...
        #for l in self.decays:
        #    print(type(l))
        self.decays.sort(key=lambda x: x.br)
...
def readSLHA(spcstr, ignorenobr=False, ignorenomass=True):
    ...
    # print blocks.keys()
    try:
        for pid in list(blocks["MASS"].entries.keys()):
            if pid in decays:
...
def writeSLHA(blocks, decays, ignorenobr=False, precision=8):
    ...
    for bname, b in sorted(blocks.items()):
        ...
        out += "\n"
    ## Decays
    for pid, particle in sorted(decays.items()):
...
def readISAWIG(isastr, ignorenobr=False):
    ...
    def getnextvalidlineitems():
        return list(map(_autotype, getnextvalidline().split()))
...
def writeISAWIG(blocks, decays, ignorenobr=False, precision=8):
    ...
    print(len(masses))
    print(len(decays))
    a = list(masses.keys()); a.sort(); print(a)
    b = list(decays.keys()); b.sort(); print(b)
