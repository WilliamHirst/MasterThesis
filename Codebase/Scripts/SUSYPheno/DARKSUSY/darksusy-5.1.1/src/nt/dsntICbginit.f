***********************************************************************
*** dsntICbginit initialises the pdfs which describe the IceCube
...
***                          number of events).  See dsntIC.h for
***                          the key.
***        
...
***********************************************************************
      ...
      subroutine dsntICbginit(filename, nbins_angular, nbins_nchan, 
     & first, second)
      ...
      if (nchan_min .gt. BGnchandist_nchan(1)) then
       ! write(*,*) 'Warning from dsntICbginit: nchan
     ...
     & !Assuming zeros for histograms entries outside given range.'
      ...
      if (nnchan_total .gt. max_nnchan) then
        ...
        write(*,*) 'Increase max_nnchan in dsntIC.h and recompile.'
        write(*,*)
        call exit(0)
      endif
     ...
     & BGangdist_sigma,IER)
      if (IER .lt. 0) then
        write(*,*) 'Error in dsntICbgnit: TSPSI failed with error'
        write(*,*) 'code',IER
        stop
      endif
     ...
     & BGangdist_sigma,IER)     
      if (IER .lt. 0) then
        write(*,*) 'Error in dsntICbgnit: TSINTL failed with error'
        write(*,*) 'code',IER
        stop
      endif
