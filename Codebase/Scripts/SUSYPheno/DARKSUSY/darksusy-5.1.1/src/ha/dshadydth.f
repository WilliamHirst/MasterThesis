...
c... get the yield at this energy
      if (phitype.eq.0) then
        yield=dshayieldf(e1cm,ethcm,phich,phifk,istat)
        if (phifk.ge.100) then  ! differential yields
          jac=1.0d0/dtldt0
          yield=yield*jac
        endif
      elseif (phitype.eq.1) then
        yield1=dshayieldf(e1cm,tcm(1),phich,phifk,istat)
        yield2=dshayieldf(e1cm,tcm(2),phich,phifk,istat)
        if (phifk.ge.100) then  ! differential yields
          jac=1.0d0/dtdt(1)
          yield1=yield1*jac
          jac=1.0d0/dtdt(2)
        ...
        yield=dshayieldf(e1cm,tcm(1),phich,phifk,istat)
        if (phifk.ge.100) then  ! differential yields
          jac=1.0d0/dtdt(1)
          yield=yield*jac
        endif
...
c        write(*,*) 'warning in dshadydth: integrated yields for',
...
c...  one particle.
      ...
      dshadydth=0.25d0*yield
c      write(*,*) 'cth = ',cth,'  dshadydth = ',0.25d0*yield
