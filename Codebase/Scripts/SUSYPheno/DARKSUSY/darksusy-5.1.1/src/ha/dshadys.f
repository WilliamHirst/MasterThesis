...
c...calculate the energy of particle 1 in the cm system
      e1cm=phim0/2.0d0 ! JE FIX: pick virtual mass
...
c            write(*,*) 'error in dshadys: ep < 0.99 mp'
c            write(*,*) '  channel = ',ch
c            write(*,*) '  mp = ',mp
c            write(*,*) '  ep = ',ep
c          else
...
c          endif
          ...
          if (ep.gt.ub(ch)) then
            write(*,*) 'error in dshadys: too high ep = ',ep
          endif
...
c          if (ep.lt.lb(ch)) goto 150
     ...
     &        dshayieldf(e1cm,ethcm,ch,phifk,istat)
            if (phifk.ge.100) then  ! differential yields
              jac=1.0d0/dtldt0
              yield1=yield1*jac
            endif
          ...
          elseif (phitype.eq.1) then
            yield1=dshayieldf(e1cm,tcm(1),ch,phifk,istat)
            yield2=dshayieldf(e1cm,tcm(2),ch,phifk,istat)
            if (phifk.ge.100) then  ! differential yields
              jac=1.0d0/dtdt(1)
              yield1=yield1*jac
              jac=1.0d0/dtdt(2)
            ...
            yield1=has0br(hdi(ch),phihno)
     &        *dshayieldf(e1cm,tcm(1),ch,phifk,
     &      istat)
            if (phifk.ge.100) then  ! differential yields
              jac=1.0d0/dtdt(1)
              yield1=yield1*jac
...
c        write(*,*) 'warning in dshadys: integrated yields for',
