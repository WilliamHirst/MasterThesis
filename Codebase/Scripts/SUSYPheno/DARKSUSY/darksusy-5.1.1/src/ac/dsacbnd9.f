...
c     110700 FeynHiggs NaN check included, Hamish Silverwood & Pat Scott
c     110906 HiggsBounds condemned b/c incompatible with upgrade to 3.4.0, pg
c=======================================================================
      implicit none
      include 'dsmssm.h'
      include 'dsio.h'
...
c...Declaration of HiggsBounds variables (el 090316)
      integer anH,aHBresult,achan,ancombined
      parameter (anH=3)
      character*5 awhichexpt
      parameter (awhichexpt="onlyL")
      double precision aMh(anH)
      double precision aCS_lep_hjZ_ratio(anH)
      double precision aCS_lep_hjhi_ratio(anH,anH)
      double precision aCS_tev_pp_hj_ratio(anH)
      double precision aCS_tev_pp_hjb_ratio(anH)
      double precision aCS_tev_pp_hjW_ratio(anH)
      double precision aCS_tev_pp_hjZ_ratio(anH)
      double precision aCS_tev_pp_vbf_ratio(anH)
      double precision aBR_hjbb(anH),aBR_hjtautau(anH)
      double precision aBR_hjWW(anH),aBR_hjgaga(anH)
      double precision aBR_hjhihi(anH,anH)
      double precision aobsratio
      double precision SMBR_Hbb,SMBR_Htautau
      double precision SMBR_HWW,SMBR_Hgamgam
c...
...
c...using HiggsBounds (currently only LEP limits on neutral higgses)
      elseif (higwid.eq.5) then

         write (*,*) 'DarkSUSY: obsolete call to HiggsBounds 2.7',
     &        ' in dsacbnd9'
         write (*,*) 'DarkSUSY: do not use higwid=5 or use more recent',
     &        ' accelerator bounds'
...
c         aMh(3)=mass(kh3)
c     
c         aCS_lep_hjZ_ratio(1)=xsec_lep_hjZ_ratio(1)
c         aCS_lep_hjZ_ratio(2)=xsec_lep_hjZ_ratio(2)
c         aCS_lep_hjZ_ratio(3)=xsec_lep_hjZ_ratio(3)
c
c         aCS_lep_hjhi_ratio(1,1)=xsec_lep_hjhi_ratio(1,1)
c         aCS_lep_hjhi_ratio(1,2)=xsec_lep_hjhi_ratio(1,2)
c         aCS_lep_hjhi_ratio(1,3)=xsec_lep_hjhi_ratio(1,3)
c         aCS_lep_hjhi_ratio(2,1)=xsec_lep_hjhi_ratio(2,1)
c         aCS_lep_hjhi_ratio(2,2)=xsec_lep_hjhi_ratio(2,2)
c         aCS_lep_hjhi_ratio(2,3)=xsec_lep_hjhi_ratio(2,3)
c         aCS_lep_hjhi_ratio(3,1)=xsec_lep_hjhi_ratio(3,1)
c         aCS_lep_hjhi_ratio(3,2)=xsec_lep_hjhi_ratio(3,2)
c         aCS_lep_hjhi_ratio(3,3)=xsec_lep_hjhi_ratio(3,3)
c
c         aBR_hjbb(1)=hdwidth(25,1)/width(kh1)
c         aBR_hjbb(2)=hdwidth(25,2)/width(kh2)
c         aBR_hjbb(3)=hdwidth(25,3)/width(kh3)
c
c         aBR_hjtautau(1)=hdwidth(19,1)/width(kh1)
c         aBR_hjtautau(2)=hdwidth(19,2)/width(kh2)
c         aBR_hjtautau(3)=hdwidth(19,3)/width(kh3)
c
c         aBR_hjhihi(1,1)=hdwidth(1,1)/width(kh1)
c         aBR_hjhihi(1,2)=hdwidth(3,1)/width(kh1)
c         aBR_hjhihi(1,3)=hdwidth(4,1)/width(kh1)
c         aBR_hjhihi(2,1)=hdwidth(1,2)/width(kh2)
c         aBR_hjhihi(2,2)=hdwidth(3,2)/width(kh2)
c         aBR_hjhihi(2,3)=hdwidth(4,2)/width(kh2)
c         aBR_hjhihi(3,1)=hdwidth(1,3)/width(kh3)
c         aBR_hjhihi(3,2)=hdwidth(3,3)/width(kh3)
c         aBR_hjhihi(3,3)=hdwidth(4,3)/width(kh3)
c
cc...We don't need the following for 'onlyL'. Putting them to zero.
c         do i=1,anH
c            aCS_tev_pp_hj_ratio(i)=0.0d0
c            aCS_tev_pp_hjb_ratio(i)=0.0d0   
c            aCS_tev_pp_hjW_ratio(i)=0.0d0
c            aCS_tev_pp_hjZ_ratio(i)=0.0d0
c            aCS_tev_pp_vbf_ratio(i)=0.0d0
c            aBR_hjWW(i)=0.0d0
c            aBR_hjgaga(i)=0.0d0
c         enddo
c
c
c         call run_higgsBounds_hadr(anH,aMh,
c     &    aCS_lep_hjZ_ratio,aCS_lep_hjhi_ratio,
c     &    aCS_tev_pp_hj_ratio,
c     &    aCS_tev_pp_hjb_ratio,
c     &    aCS_tev_pp_hjW_ratio,
c     &    aCS_tev_pp_hjZ_ratio,
c     &    aCS_tev_pp_vbf_ratio,
c     &    aBR_hjbb,aBR_hjtautau,
c     &    aBR_hjWW,aBR_hjgaga,
c     &    aBR_hjhihi,
c     &    aHBresult,achan,
c     &    aobsratio,ancombined)
c
c         if (aHBresult.ne.1) Excl=ibset(Excl,5)
