...
c...pdg2002 at http://pdg.lbl.gov/. The experimental bound from CLEO
c...end Belle is BR(b->s gamma) = (3.3 +- 0.4) e-4
c...We then also assume that the theoretical error is
c...+- 0.5e-4 which gives the total error 0.63e-4. The two sigma confidence
c...level is then (2.02 - 4.58 )e-4 which we round off to
c...(2.0 - 4.6)e-4
      call dsbsgammafull(bsg,1)  ! bsg w/ SUSY contribution
      if (bsg.lt.2.0d-4.or.bsg.gt.4.6d-4) then
         excl=ibset(excl,7)
      endif
...
c-----------------------------------------bound from gamma_z(invisible)
      ...
      gzinv = 3.0d0 * (g2weak/2/costhw)**2 * mz/(24.d0*pi)
      ! 3 sneutrinos
      do i=1,3
