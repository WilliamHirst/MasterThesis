      function dsmsbarmass(k,mk,mu1,mu2)
c     
c     run the mass mk of particle k from the 
c     msbar scale mu1 to the msbar scale mu2
...
c     
      ...
      include 'dsmssm.h'
      integer k
      real*8 dsmsbarmass,mk,mu1,mu2
      real*8 ans,x,cc,ccold,zeta,a,dsralph34loop
      real*8 pi
...
c
      if (mu1.ge.mtmt) then
         n1=6
      else if (mu1.ge.mbmb) then
         n1=5
      else if (mu1.ge.mcmc) then
         n1=4
      else
         n1=3
      endif
