...
!=======================================================================
! 
      SUBROUTINE dslnpinsum(n,thetab,thetas,sigma,
     ...
     &    .OR. (thetas .LT. 0d0) .OR. (sigma .LT. 0d0)) THEN
        WRITE(*,*) 'ERROR: dslnpinsum called with negative argument'
        STOP
      END IF
     ...
     &                         lnpmf,lnlesum,lngesum)
      ...
      ! Special case: n = 0
      IF (n .EQ. 0) THEN
        lnpmf   = dslnpin(n,thetab,thetas,sigma)
        lnlesum = lnpmf
        lngesum = 0d0
        RETURN
      END IF
      ...
      lnp = dslnpin(n+1,thetab,thetas,sigma)
      ...
      IF (is_increasing) THEN
        ...
        DO WHILE (k .GT. 0)
          k = k-1
          lnp = dslnpin(k,thetab,thetas,sigma)
          lnlesum = lnsum(lnlesum,lnp)
          IF (lnp - lnlesum .LE. LN_PRECISION) EXIT
        END DO
        lngesum = LOG(1d0 - (EXP(lnlesum) - EXP(lnpmf)))
