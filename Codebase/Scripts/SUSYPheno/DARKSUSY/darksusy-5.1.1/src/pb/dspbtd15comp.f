...
**********************************************************************
     ...
     & rlist(5000)
      external dspbcompint1
      real*8 tpint,zint,rint,rmaxint
      common/dspbcompcom/tpint,zint,rint,rmaxint
      real*8 epsabs
      integer ndigits
      integer comp0set
      ...
      rmaxint=pbrcy
      tpint=tp
      dummy=dspbcompint1(0.d0)
      low=0.d0
      up=pbzcy
 10   call dqagse(dspbcompint1,low,up,epsabs,epsrel,limit,
     &      result,abserr,neval,ier,alist,blist,rlist,elist,iord,last)
      ...
      dspbtd15comp=2.d0*result
 1000 format(a7,10(1x,e14.8))
      return
      end


      real*8 function dspbcompint1(z)
      implicit none
      include 'dspbcom.h'
      real*8 z
     ...
     & rlist(5000)
      ...
      integer ndigits
      integer compset
      common/compsetcom/epsabs,tpsetup,rmaxsetup,compset,ndigits
      real*8 tpint,zint,rint,rmaxint
      common/dspbcompcom/tpint,zint,rint,rmaxint
      limit=5000
     ...
     &   .gt.1.d-8) then
      ...
      epsabs=1.d-14     !numerical accuracy
      epsrel=10.d0**(-ndigits)
c      write(*,*) 'dspbcompint1 tabulation started for tp = ',tpint
c      write(*,*) '(tpsetup) = ',tpsetup
      ...
      do k=1,npoints-2
        ...
        low=0.d0
        up=rmaxint
 10     call dqagse(dspbcompint2,low,up,epsabs,epsrel,limit,
     &      result,abserr,neval,ier,alist,blist,rlist,elist,iord,last)
        if(dabs(result)/10.d0**ndigits.lt.epsabs) then
          epsabs=dabs(result)/10.d0**(ndigits+2) !too low accuracy
          goto 10
 ...
 30   continue
     ...
     &    .gt.reratio*max(dabs(ycomp(k)),dabs(ycomp(k+1)))) then
          ...
          low=0.d0
          up=rmaxint
 20       call dqagse(dspbcompint2,low,up,epsabs,epsrel,limit,
     &      result,abserr,neval,ier,alist,blist,rlist,elist,iord,last)
          if(dabs(result)/10.d0**ndigits.lt.epsabs) then
            epsabs=dabs(result)/10.d0**(ndigits+2) !too low accuracy
            goto 20
          ...
          if(npoints.le.1005) then
            ...
            write(*,*) 'in dspbcompint1 exceeded the maximum dim'
            write(*,*) 'allowed for vectors in the compspcom block'
            write(*,*) 'which is set equal to 1005' 
          endif
        endif  
      enddo
c      write(*,*) 'dspbcompint1 tabulation is over with npoints =',
c     &  npoints
      ...
      if(z.ge.xcomp(1).and.z.le.xcomp(npoints)) then
        ...
        write(*,*) 'dspbcompint1 called out of the allowed range :' 
        write(*,*) 'z = ',z
      ...
      real*8 function dspbcompint2(r)
      implicit none
      include 'dshmcom.h'
      real*8 tpint,zint,rint,rmaxint
      common/dspbcompcom/tpint,zint,rint,rmaxint
      ...
      dimension alist(5000),blist(5000),elist(5000),iord(5000),
     & rlist(5000)
      external dspbcompint3
      integer ndigits
      integer comp2set
      real*8 epsabs
      common/comp2setcom/epsabs,comp2set,ndigits
      limit=5000
      ...
      low=0.d0
      up=8.d0*datan(1.d0)
 10   call dqagseb(dspbcompint3,low,up,epsabs,epsrel,limit,
     &      result,abserr,neval,ier,alist,blist,rlist,elist,iord,last)
      if(dabs(result)/10.d0**ndigits.lt.epsabs) then
        epsabs=dabs(result)/10.d0**(ndigits+2) !too low accuracy
        goto 10
      ...
      dspbcompint2=result*(dshmaxirho(rint,zint)/rho0)**2*rint
 1000 format(a7,10(1x,e14.8))
      return
      end


      real*8 function dspbcompint3(theta)
      implicit none
      include 'dshmcom.h'
      real*8 theta,L,dspbtd15beuclsp
      real*8 tpint,zint,rint,rmaxint
      common/dspbcompcom/tpint,zint,rint,rmaxint
      L=dsqrt((r_0**2+rint**2-2.d0*r_0*rint*dcos(theta))+zint**2)
      dspbcompint3=dspbtd15beuclsp(L,tpint)
      return
      end
