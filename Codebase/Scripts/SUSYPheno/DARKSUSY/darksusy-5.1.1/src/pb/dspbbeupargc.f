...
**********************************************************************
      ...
      implicit none
      real*8 z,radcoord,vertcoord,dshmaxirho,dshmaxiprob,val
     &  ,r,kpc,arg
      include 'dspbcom.h'
      include 'dshmcom.h'
      ...
      elseif(hclumpy.eq.2) then                 ! clumpy halo
        ...
        write(*,*) 'dspbbeupargc called with wrong hclumpy=',hclumpy
        stop
      endif
      if(z.lt.0.d0.or.z.gt.pbhg/pbhh) then
        write(*,*) 'pbbeuparg called with wrong z=',z
        stop
      else
        arg=lambdag*pbhh*kpc*(pbhg/pbhh-z)
        if(arg.lt.20.d0) then
          dspbbeupargc=dcosh(lambdag*pbhh*kpc*(pbhg/pbhh-z))
     &              /dsinh(lambdag*pbhg*kpc)
        else  
          dspbbeupargc=
     &      dcosh(lambdag*pbhh*kpc*z)/dtanh(lambdag*pbhg*kpc)
     &      -dsinh(lambdag*pbhh*kpc*z)
        endif  
        dspbbeupargc=dspbbeupargc*val*pbhh*kpc
      endif
      return
      end
