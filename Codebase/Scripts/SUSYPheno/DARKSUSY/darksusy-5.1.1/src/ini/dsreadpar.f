...
c... gauge coupling constants at the z scale
      if (var.eq.'gfermi') GFermi=dsfval(val)
...
c... supersymmetric parameters
      if (var.eq.'modeltype') modeltype=dsival(val)
      if (var.eq.'m0var') m0var=dsfval(val)
      if (var.eq.'mhfvar') mhfvar=dsfval(val)
      if (var.eq.'a0var') a0var=dsfval(val)
      if (var.eq.'sgnmuvar') sgnmuvar=dsfval(val)
      if (var.eq.'tgbetavar') tgbetavar=dsfval(val)
...
c... set-up modules
      if (var.eq.'hmlabel') then
         call dshmset(dscval(val))
 ...
 99   continue
      ...
      s2thw=dsgf2s2thw(GFermi,alph3mz,mass(kz),mass(kt),1) ! on-shell
      s2wmz=dsgf2s2thw(GFermi,alph3mz,mass(kz),mass(kt),3) ! @ MZ, MS-bar
      if (m1gut) m1 = m2 * 5.0d0/3.0d0 * s2wmz/(1.d0-s2wmz)
      if (m3gut) m3 = m2 * (alph3mz*s2wmz)/alphem
      if (msq.ne.0.d0) then
         mass2q(3) = msq**2
         mass2q(3) = msq**2
         mass2u(3) = msq**2
