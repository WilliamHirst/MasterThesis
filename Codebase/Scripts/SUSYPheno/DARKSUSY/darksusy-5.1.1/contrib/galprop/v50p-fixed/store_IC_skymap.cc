...
int Galprop::store_IC_skymap(char* IC_type)
{
  int stat=0;
  cout<<" >>>> store_IC_skymap"<<endl;
  ...
  if(strcmp(IC_type,  "isotropic")==0) strcat(outfile,"ics_isotropic_skymap_");
  if(strcmp(IC_type,"anisotropic")==0) strcat(outfile,"ics_anisotropic_skymap_");
  strcat(outfile,galdef.galdef_ID);
  cout<<" storing IC "<<IC_type<< " skymap total in file "<<outfile<<endl;
