...
// GAMMA RAYS
   ...
   if(galdef.gamma_rays>=1)                    //AWS20050302 IMOS20060420
   {
     ...
     if(galdef.n_spatial_dimensions==2)
       {
         if(galdef.bremss) galaxy.bremss_emiss.        init(galaxy.n_rgrid,galaxy.n_zgrid,galaxy.n_E_gammagrid); // IMOS20060420
         if(galdef.bremss) galaxy.bremss_ionized_emiss.init(galaxy.n_rgrid,galaxy.n_zgrid,galaxy.n_E_gammagrid); // IMOS20060420
	   ...
	   for(int icomp=0; icomp<galaxy.n_ISRF_components; icomp++)
         ...
         if(galdef.pi0_decay) galaxy.pi0_decay_emiss.init(galaxy.n_rgrid,galaxy.n_zgrid,galaxy.n_E_gammagrid);   // IMOS20060420	 
	 if(galdef.DM_gammas) galaxy.DM_emiss.init(galaxy.n_rgrid,galaxy.n_zgrid,galaxy.n_E_gammagrid); // DM: IMOS20050912 IMOS20060420
       }
     
     if(galdef.n_spatial_dimensions==3)
       {
         if(galdef.bremss) galaxy.bremss_emiss.        init(galaxy.n_xgrid,galaxy.n_ygrid,galaxy.n_zgrid,galaxy.n_E_gammagrid); // IMOS20060420
         if(galdef.bremss) galaxy.bremss_ionized_emiss.init(galaxy.n_xgrid,galaxy.n_ygrid,galaxy.n_zgrid,galaxy.n_E_gammagrid); // IMOS20060420

         if(galdef.IC_isotropic+galdef.IC_anisotropic)                                                                       // IMOS20060420
	   for(int icomp=0; icomp<galaxy.n_ISRF_components; icomp++)
	     galaxy.IC_iso_emiss[icomp].init(galaxy.n_xgrid,galaxy.n_ygrid,galaxy.n_zgrid,galaxy.n_E_gammagrid);

         if(galdef.pi0_decay) galaxy.pi0_decay_emiss.init(galaxy.n_xgrid,galaxy.n_ygrid,galaxy.n_zgrid,galaxy.n_E_gammagrid);   // IMOS20060420	 
	 if(galdef.DM_gammas) galaxy.DM_emiss.init(galaxy.n_rgrid,galaxy.n_zgrid,galaxy.n_E_gammagrid); // DM: IMOS20050912 IMOS20060420
       }
...
// GAMMA-RAY SKYMAPS
      ...
      if(galdef.pi0_decay)                                                                // IMOS20060420
	{
      ...
      if(galdef.DM_gammas) galaxy.DM_skymap.init(galaxy.n_long,galaxy.n_lat,galaxy.n_E_gammagrid); // DM: IMOS20050912 IMOS20060420
   }
