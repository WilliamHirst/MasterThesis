...
c***********************************************************************
       ...
       REAL isrf_energy_density ! added 10 Jan 2000
...
c      EMISS = exp(-r/0.2)    ! for test2
      ...
      z=0.0
      emiss_= isrf_energy_density(r,z)
      emiss=emiss_

c      write(*,*)"    emiss: r emiss",r,emiss
      return
      end
...
c## integration over the Lorentz factor ##
      do n=1,NG
         ...
         S1 = SPEC*gam/gam**PLindex               *ddg  !!!!
         SFJ1 = FJONES(gam,E0,E)*gam/gam**PLindex *ddg  !!!!
         S = S +S1
         SFJ = SFJ + SFJ1
...
c    (the emissivity of unit area of the Galactic plane is equal to 1).
c    REFERENCES:
c I.V.Moskalenko & A.W.Strong 1998, ApJ submitted
c***********************************************************************
      implicit real*8 (a-h,o-z)
...
c## limits on cos(theta) from the geometry ##
...
c## absolute accuracy estimation ##
         ...
         call SIM1(c1,c2,(c2-c1)/30.,EPS,AEPS,FTHETA,DENS)
         DENS1=DENS
...
c## check if E4 lower then maximal energy ##
      ...
      B2 = B1*E4/(E1**2*(gam-E4))-1.d0
c#      A2 = A1*E4/(E1**2*(gam-E4))-1.d0
      A2 = E4/E1/gam/(gam-E4)/2.d0-1.d0   ! zeta0 calculation
...
c## limits on theta from kinematics ##
...
c## integration over cos(theta) ##
      ...
      SPEC = (SPEC2-SPEC1)/E1/(gam-E4)**2  /DENS
...
c***********************************************************************
     ...
     #      /ZETA/DENS,key,kbg
      ...
      cosfi = 1.d0
c## limits on cos(fi) from the geometry ##
...
c## weight function ##
      ...
      if(kbg. eq. 1) WEIGHT = 1.d0/costheta    ! transparent disk
...
c    REFERENCES:
c F.C.Jones 1968, Phys.Rew. 167, 1159
c***********************************************************************
      implicit real*8 (a-h,o-z)
     ...
     #   +(1.d0-q)/2.d0*(4.d0*E1*gam*q)**2/(1.d0+4.d0*E1*gam*q)
      FJONES = FJONES*2.d0/E1/gam**2  ! in units Pi*r0^2*c
