...
//**.****|****.****|****.****|****.****|****.****|****.****|****.****|****.****|
// * create_transport_arrays.cc *                  galprop package * 05/03/2002 
//**"****!****"****!****"****!****"****!****"****!****"****!****"****!****"****|
...
int create_transport_arrays(Particle &particle)
{
...
// ASSIGNING PRIMARY SOURCE FUNCTION
   ...
   double spec_shape;
   double rigid_br=galdef.nuc_rigid_br,                           // IMOS20000607
          g_1=galdef.nuc_g_1,
          g_2=galdef.nuc_g_2;
   ...
   if(galdef.n_spatial_dimensions==2)
   {
	 ...
	 {
            if(particle.rigidity[ip]< rigid_br) spec_shape=pow(particle.rigidity[ip]/rigid_br,-g_1);
            if(particle.rigidity[ip]>=rigid_br) spec_shape=pow(particle.rigidity[ip]/rigid_br,-g_2);
         }
...
// compute beta at break rigidity so that formula will give galdef.D0_xx at this point
   ...
   if(kinematic(particle.Z,particle.A,species,p_br,Ekin_br,Etot_br,beta_br,gamma_br,rigidity_br,0))
      exit(1);                                                                           // IMOS20000810.2
   ...
   if(galdef.n_spatial_dimensions==2)
   {
      for(int ir=0; ir<particle.n_rgrid; ir++)
      {
         for(int iz=0; iz<particle.n_zgrid; iz++)
         {
            for(int ip=0; ip<particle.n_pgrid; ip++)
            {
	       particle.Dxx.d2[ir][iz].s[ip] = particle.beta[ip]/beta_br*galdef.D0_xx;   // IMOS20000810.1
      ...
      for(int ix=0; ix<particle.n_xgrid; ix++)
      {
         for(int iy=0; iy<particle.n_ygrid; iy++)
         {
            for(int iz=0; iz<particle.n_zgrid; iz++)
            {
               for(int ip=0; ip<particle.n_pgrid; ip++)
               {
		  particle.Dxx.d3[ix][iy][iz].s[ip] = particle.beta[ip]/beta_br*galdef.D0_xx;   // IMOS20000810.3
                  if(particle.rigidity[ip]< galdef.D_rigid_br) 
...
// ELECTRON ATTACHMENT/STRIPPING CROSS SECTION                                  IMOS20010816
            ...
            if(galdef.K_capture)
	    {
	       for(K_electron=0;K_electron<=galdef.K_capture;K_electron++) nucdata(galdef_network_par
                  ,particle.Z,particle.A,K_electron,particle.Z,particle.A,&Z2,&A2,&t_half[K_electron]);
      ...
      if(galdef.n_spatial_dimensions==3)
      {
	    ...
	    nucleon_cs(galdef.total_cross_section,particle.Ekin[ip]*1.e-3,A1,Z2,A2,
               &PP_inel,&PA_inel,&aPP_non,&aPA_non,&aPP_ann,&aPA_ann); // AWS20010620
            He_to_H_CS(particle.Ekin[ip]*1.e-3,particle.Z,particle.A,999,999,&CSratio,&CStot_ratio);
