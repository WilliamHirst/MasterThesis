...
//**.****|****.****|****.****|****.****|****.****|****.****|****.****|****.****|
// * cr_luminosity.cc *                      galprop package * 4/14/2000 
//**"****!****"****!****"****!****"****!****"****!****"****!****"****!****"****|
...
for(int i=0;i<n_species;i++)if(gcr[i].A==4 && gcr[i].Z==2)iHelium =i;
 ...
 if(galaxy.n_spatial_dimensions==2){
   ...
   for(int iz=0;iz<gcr[iprotons].n_zgrid;iz++){
      ...
      CR_luminosity+=       
       particle.primary_source_function.d2[ir][iz].s[ip] / particle.beta[ip]* pow(particle.Ekin[ip],2)
...
if(galaxy.n_spatial_dimensions==3){
   ...
   for(int iz=0;iz<gcr[iprotons].n_zgrid;iz++){
     ...
     if(galdef.use_symmetry==1 && iz==0) sym_weight= 4.;// to avoid double-counting at z=0
      ...
      CR_luminosity+=       
       particle.primary_source_function.d3[ix][iy][iz].s[ip] / particle.beta[ip]* pow(particle.Ekin[ip],2)
	*particle.A  *particle.A 
        *galdef.dx*galdef.dy*galdef.dz
        *sym_weight;
...
CR_luminosity *= 4.0*Pi/c * 1.0e6*  eV_to_erg * pow(kpc2cm,3) * log(gcr[iprotons].Ekin[1]/ gcr[iprotons].Ekin[0]) ;

CR_luminosity *= gcr[iprotons].normalization_factor;
