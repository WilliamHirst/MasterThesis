...
!  if(predobs.eq.1) write(*,*) 'Interpolated value = ', interpol
 ...
 !**********************************************************
  ...
  do j=1,npart    
   ...
   if(     (abs(mass(jj)-mass(j)).le.S95_t1(c)%deltax) &
    ...
    case(8742,4493,9475,5482,5570,5876,1024,9889,3534,6089,10235,3047,3564,6166)   
    ...
    case(5757)
     call model_likeness(j,S95_t1(c)%id,t,model_like(j),fact(j))
    case(8957,5472,9219,9463,9596,5828,1970,3493,5972,3155,5613,9868,10068,6092,10217,10239,0874,6220)  
    ...
    case(1514,5601,5737)  
     fact(j)=(  t%tev%XS_hjZ_ratio(j) * t%tev%XS_HZ_SM(j)      &
      ...
      &    )                  & 
      &  *   t%BR_hjgaga(j)                    
    case(5858,6177,1887,10065,10485,4960)  
     call model_likeness(j,S95_t1(c)%id,t,model_like(j),fact(j)) 
    case(7081,9166,9483,5586,9642,1266,0432,9891,5285,3935,6087,6170,10212,6223,10583,10596)
     call model_likeness(j,S95_t1(c)%id,t,model_like(j),fact(j))
    case(10010,10607)
     call model_likeness(j,S95_t1(c)%id,t,model_like(j),fact(j))  
    ...
    case(9290)
     call model_likeness(j,S95_t1(c)%id,t,model_like(j),fact(j)) 
    case(5984,9714,6006,4481,6095,10432,6179,10599)
     call model_likeness(j,S95_t1(c)%id,t,model_like(j),fact(j))
    case(3556,1931)
    ...
    case(1811)
     call model_likeness(j,S95_t1(c)%id,t,model_like(j),fact(j))
    case(1812,2011138,2011151,8353,7712,11002,11008)
     call model_likeness(j,S95_t1(c)%id,t,model_like(j),fact(j))
    case(6008,9998)
     call model_likeness(j,S95_t1(c)%id,t,model_like(j),fact(j))
    case(6082,6182,6219,10573)
     call model_likeness(j,S95_t1(c)%id,t,model_like(j),fact(j))
    ...
    case(6091,1268)
     call model_likeness(j,S95_t1(c)%id,t,model_like(j),fact(j))
    case(2011048,11004,11015,11013,11028,11006,11017,110271,110272,14161,14162,5064,2011150,2011131)
     call model_likeness(j,S95_t1(c)%id,t,model_like(j),fact(j))
    case(2011162,1415)
     call model_likeness(j,S95_t1(c)%id,t,model_like(j),fact(j))
    case(11025,1997)
     call model_likeness(j,S95_t1(c)%id,t,model_like(j),fact(j))
    case(2011026,11005,11016,11026,3478,3357,2011148)
     call model_likeness(j,S95_t1(c)%id,t,model_like(j),fact(j))
    case(110212)
     call model_likeness(j,S95_t1(c)%id,t,model_like(j),fact(j))
  ...
  if(fact(jj).le.vvsmall)then!A  !Higgs jj doesn't contribute - wait until another call of this subroutine before
   ...
   elseif(S95_t1(c)%particle_x .ne. Hneut)then!B
    ...
    case(8742,5482,5570,4493,9475,5876,1024,9889,3534,6089,10235,3047,3564,6166)   
     do j=1,npart
    ...
    case(7081,9166,9483,5586,9642,1266,0432,9891,5285,3935,6087,6170,10212,6223,10583,10596)   
    case(9248,5845,4800,5858,6177,1887,10065,10485,10133,10439,4960) 
    case(9465,5871,9022,9023,0710,9887)
    case(6171)
    case(6183,3233) 
    case(6229)    
    case(5984,9714,4162,10102,6006,4481,4468,5757,6095,10432,6179,10599)
    case(5739,10574)
     do j=1,npart
 ...
 !******************************************************** 
  ...
  case(5482,5570,8742,4493,9475,5876,1024,9889,3534,6089,10235,3047,3564,6166)  
   descrip=' (p p-bar)->Z(h'//j//')->l l (b b-bar)   '  //label  
  case(9236,3930,8958,6039,3216,10433,6221,10600)  
   descrip=' (p p-bar)->h'//j//'->W W   '           //label    
  case(9219,9463,5472,8957,9596,5828,1970,3493,5972,3155,5613,9868,10068,6092,10217,10239,0874,6220)  
  ...
  case(8961,9465,9290,9713,9674,0598,9897,9998,9999,6008,6096,6183,3233,6229,10606,10806)  
  ...
  case(7081,9166,9483,5586,9642,1266,0432,9891,5285,3935,6087,6170,10212,6223,10583)    
  ...
  case(5984,9714,6006,9022,9023,0710,9887,4162,10102,4481,4468,6095,10432,6179,10599)  
  ...
  case(11006,11017,110271,110272,14161,14162,5064,2011150) 
  ...
  case(11005,11016,11026,3478,3357,2011148) 
...
! Added by OS 2012-03-12
 ...
 ! -1 fails the SM-like test and Mi.lt.MSingleLim 
  use usefulbits, only : dataset,div, vvsmall, newSMlikeness
  implicit none      
  !--------------------------------------input      
  type(dataset) :: t 
  integer :: id,j
  !-----------------------------------internal 
  integer :: ns,nb,n
  !--TESTING (TS 14/03/2011)
  integer :: ns_new,nb_new
  double precision :: XS_rat_temp, BR_rat_temp
  !----
  ...
  case(9465,5871,9022,9023,0710,9887,5984,9714,4162,10102,6006,4481,4468,6095,10432,6179,10599)
   ns = 4; nb = 1; call initialise_XS_rat_BR_rat  
...
!   BR_rat(3) = div(t%BR_hjtautau(j) , t%BR_Htautau_SM(j) ,0.0D0,1.0D0)
  ...
  case(11022,11032,1488,12008,2011157,2012019)
   ns = 2; nb = 5; call initialise_XS_rat_BR_rat   

   XS_rat(1) = t%lhc7%XS_hj_ratio(j)  
   XS_rat(2) = t%lhc7%XS_vbf_ratio(j)

!   OS - These are not valid for the range MH > 305 GeV, and will thus
!   always fail the SM likeness test
!   XS_rat(1) = t%lhc7%XS_hjW_ratio(j)
!   XS_rat(3) = t%lhc7%XS_hjZ_ratio(j)  
!   XS_rat(5) = t%lhc7%XS_tthj_ratio(j)
...
!----------------------------------------------------------
!--Testing an improved SM likeness check (TS 14/03/2012):
!----------------------------------------------------------
!--Search for production and decay channels which are set to zero and remove them from
!--the likeness-check. 
!--N.B.: This also affects the calculation of sigmaXbr.
!--      Make also sure that there is at least one channel left!
!----
  if(newSMlikeness) then
    ns_new=0
    nb_new=0
!    write(*,*) "Old XS ratio array before counting: ",XS_rat(:)
!    write(*,*) "Old BR ratio array before counting: ",BR_rat(:)
  endif   
!----

  do is=1,ns    
   if(abs(XS_rat(is)-unset).lt.1.0D-3)stop'error in subroutine model_likeness (4a)'
!----Count the relevant (non-zero) production channels and overwrite XS_rat
      if(newSMlikeness) then
        if(abs(XS_rat(is)).gt.vvsmall) then
          ns_new=ns_new+1
          XS_rat_temp=XS_rat(is)
          XS_rat(ns_new)=XS_rat_temp
        endif     
      endif      
!----
  enddo
!----
  if(newSMlikeness) then
    if(ns.ne.ns_new.and.ns_new.ne.0) then
!     write(*,*) "Old XS ratio array after counting: ",XS_rat(:)
     call reallocate_XS_rat
!     write(*,*) "New XS ratio array: ",XS_rat(:)
    endif   
  endif  
!----  
  do ib=1,nb    
   if(abs(BR_rat(ib)-unset).lt.1.0D-3)stop'error in subroutine model_likeness (4b)'
!----Count the relevant (non-zero) decay channels and overwrite BR_rat
      if(newSMlikeness) then
        if(abs(BR_rat(ib)).gt.vvsmall) then
          nb_new=nb_new+1
          BR_rat_temp=BR_rat(ib)
          BR_rat(nb_new)=BR_rat_temp
        endif      
      endif          
!----   
  enddo
!----
  if(newSMlikeness) then
    if(nb.ne.nb_new.and.nb_new.ne.0) then
!     write(*,*) "Old BR ratio array after counting: ",BR_rat(:)
     call reallocate_BR_rat
!     write(*,*) "New BR ratio array: ",BR_rat(:)
    endif   
  endif  
!----  
  s=sum(XS_rat)/ns 
  ...
  !----------------------------------------
!--New functions for testing a new SM likeness check(TS 14/03/2012)
  subroutine reallocate_XS_rat
   
   double precision, allocatable :: reallocate_array(:)
   allocate(reallocate_array(ns_new))
   
   reallocate_array(1:ns_new) = XS_rat(1:ns_new)
   deallocate(XS_rat)
   ns=ns_new   
   allocate(XS_rat(ns))
   XS_rat(:) = reallocate_array(:)
   deallocate(reallocate_array)
   
  end subroutine reallocate_XS_rat
  !----------------------------------------
  subroutine reallocate_BR_rat
   
   double precision, allocatable :: reallocate_array(:)
   allocate(reallocate_array(nb_new))
   
   reallocate_array(1:nb_new) = BR_rat(1:nb_new)
   deallocate(BR_rat)
   nb=nb_new   
   allocate(BR_rat(nb))
   BR_rat(:) = reallocate_array(:)
   deallocate(reallocate_array)
   
  end subroutine reallocate_BR_rat
  !----------------------------------------
