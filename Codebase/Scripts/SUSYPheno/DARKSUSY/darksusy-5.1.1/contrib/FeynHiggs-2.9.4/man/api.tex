...
\chapter*{FeynHiggs 2.9.0\\[2ex] Application Programming Interface}
...
the extended squark masses (needed in the NMFV case), with indices \\
\begin{tabular}{rcl}
  $a = 1\dots 6$ && extended sfermion index, \\
  $t = 3\dots 4$ && sfermion type: u, d.
\end{tabular}

\emph{Note:} in the absence of NMFV, $a = g + 3 (s - 1)$, where $g$ is 
the generation index (1\dots 3) and $s$ the sfermion index (1\dots 2).  
For example, the stop masses are given by the elements \CODE{MASf(3,3)}, 
\CODE{MASf(6,3)}.

\VAROUT{double complex UASf($a_1$,$a_2$,$t$)}
the extended squark mixing matrices (needed in the NMFV case), with indices \\
\begin{tabular}{rcl}
  $a_1 = 1\dots 6$ && extended sfermion index (enumerates mass eigenstates), \\
  $a_2 = 1\dots 6$ && extended sfermion index (enumerates gauge eigenstates, L/R), \\
  $t = 3\dots 4$   && sfermion type: u, d.
\end{tabular}

\emph{Note:} in the absence of NMFV, $a_{1,2} = g_{1,2} + 3 (s_{1,2} - 
1)$, where $g$ is the generation index (1\dots 3) and $s$ the sfermion 
index (1\dots 2).  For example, the stop mixing matrix is given by the 
elements \CODE{UASf(3,3,3)}, \CODE{UASf(3,6,3)}, \CODE{UASf(6,3,3)}, and 
\CODE{UASf(6,6,3)}.
