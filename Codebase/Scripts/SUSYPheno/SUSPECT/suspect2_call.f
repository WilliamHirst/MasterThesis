...
c  "SM-like" input:
      ...
      alphas =.1172d0
      mt =172.9d0
      mb = 4.19d0     ! mb(mb)_MSbar input
      mtau =1.777d0
c
c RG evolution parameters: 
c      ehigh = 1.2d3
c      elow =91.19d0
c      qewsb = 5.70884212d2 
...
c      A0 = -100.d0
      sgnmu0 = 1.d0
      tgbeta=10d0
cc      write(*,*) 'give  m0, mhalf, A0, tbeta, sign MU: '
cc      read(*,*) m0, mhalf, A0,tgbeta,sgnmu0
c      write(*,*) 'tbeta initial guess: '
c      read(*,*) tbin
c      t2in =tbin**2
c
c   
...
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c  just a loop test on m0:
cc       open(unit=20,file='Mede_test.d',status='unknown')
cc       dO i=1,30
cc       mhalf =100*15**(dble(i - 1)/(30 - 1))
c       mhalf= 1280d0+2*dble(i - 1)
cc       m0 = 2.5*mhalf
cc       A0 = -m0
       CALL suspect2(IKNOWL, INPUT, ICHOICE,ERRMESS)
cc       write(20,6) mhalf, g2ewsb,scale,egut,ml,msb1
cc       enddo
cc 6    format(6(g14.6,1x))
c
c
