...
c
c  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c  Authors: Abdelhak Djouadi, Jean-Loic Kneur and Gilbert Moultaka
c  (main coding and maintenance: J-L Kneur)
c              (Lab Charles Coulomb, CNRS & Universite Montpellier). 
c  VERSION: 2.5.1  
c  Last modifications : Sept 10, 2017
...
c  Home page:http://www.coulomb.univ-montp2.fr/perso/jean-loic.kneur/Suspect/
c
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c
...
c   Gaugino masses
  881   beta =datan(tbeta)
        if(ichoice(7).eq.2.and.irge.eq.irgmax) inorc =1 ! ino masses RC only at last iter
        CALL SU_GAUGINO(mu,m1,m2,m3,beta,a,gmc,gmn,xmn)
       if(inonpert.eq.-1.and.irge.eq.irgmax) then
       errmess(10)=-1.d0
       goto 801
...
c last thing: calculating now the R.C to chargino, neutralino masses:
      ...
      dmn4=xmn(4)
c    ****************************************************************
c    Now comes the writing in the output part. 
c     ****************************************************************
 801  continue
c
...
C ************  SUSPECT OUTPUT WRITING (in SUSPECT2.out)
     ...
     .      errmess(10).eq.-1.d0) then
      write(nout,'(a)')'CAUTION UNRELIABLE OUTPUT! check errmess below'
        ...
        if(ichoice(1).eq.10) then  
        write(nout,'(a)')'             SUSPECT2.5.1 OUTPUT: MSUGRA CASE'
        write(nout,'(a)')'             ------------------------------'
        write(nout,'(a)')
        else if(ichoice(1).eq.11) then 
        write(nout,'(a)')'             SUSPECT2.5.1 OUTPUT: GMSB CASE'
        write(nout,'(a)')'             ----------------------------'
        write(nout,'(a)')
        else if(ichoice(1).eq.12) then 
        write(nout,'(a)')'             SUSPECT2.5.1 OUTPUT: AMSB CASE'
        write(nout,'(a)')'             ----------------------------'
        write(nout,'(a)')
        else
        write(nout,'(a)')'             SUSPECT2.5.1 OUTPUT: pMSSM CASE'
        write(nout,'(a)')'             -----------------------------'
       endif
        if(ichoice(1).eq.0) then        
...
c  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      dimension u(2,2),v(2,2),z(4,4),dxmn(4),gmn(4),gmc(2),xmctree(2)
      COMMON/su_PARAM/GF,ALPH,MZ,MW
      COMMON/SU_higgsrunz/ml,mh,ma,mch,alfa 
      COMMON/SU_outginos/mc1,mc2,mn1,mn2,mn3,mn4,mgluino
      COMMON/SU_inoDR/U,V,Z,xmctree,dxmn
...
c
      ...
      dimension u(2,2),v(2,2),z(4,4),dxmn(4),mc(2),xmctree(2),
...
c  Pierce, Bagger, Matchev, Zhang (PBMZ) hep-ph/9606211. For these R.C. one
c  needs the one-loop Passarino-Veltman functions discussed above.
c
c  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++    
           SUBROUTINE SU_GAUGINO(mu,m1,m2,m3,b,a,mc,mn,xmn)
c  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++    
c  Calculates the chargino and neutralino masses and mixing angles (with 
c  analytical expressions) including full one-loop radiative corrections 
c  a la PBMZ. The input parameters at EWSB scale are: 
c  mu,m1,m2,m3: Higgs mass parameter and gaugino mass parameters, 
c  b = datan(tan(beta)), a = mixing angle in the Higgs sector 
c  Output parameters are: 
c  mc: the two chargino masses,
c  mn: the four neutralino masses (absolute values),
c  xmn: the four neutralino masses (including signs).  
...
c
      ...
      complex*16 cx(4)
      logical su_isNaN
      dimension mc(2),mn(4),mnDRbar(4),xmn(4),xmnsave(4),z(4,4),zx(4,4),
     . u(2,2),v(2,2),iord(4),irem(2),m(4,4),mtr(4,4),rcN(4,4),rcL(4,4),
     . rcR(4,4),rcS(4,4),rcC(2,2),rcCL(2,2),rcCR(2,2),rcCS(2,2),
     . rcCST(2,2),MMC(2,2),MMCtr(2,2)
c
      dimension x(2,2)
      dimension dxmn(4)  
      dimension ymn(4),yz(4,4),zsave(4,4),xmc(2),xmctree(2),xmcsave(2),
     . bu(2),bv(2),usave(2,2),vsave(2,2)
      COMMON/SU_param/gf,alph,mz,mw
     ...
     .            mudum,m1dum,m2dum,m3dum
      ...
      COMMON/SU_strc/irge,irgmax,ifix,isfrc,inorc
      COMMON/SU_inoDR/Usave,Vsave,Zsave,xmctree,mnDRbar
      COMMON/SU_matino/u,v,z,dxmn
...
c
      ...
      mzsave = mz               
      mwsave = mw
      cw = 1.d0/dsqrt(1d0+(g1ewsb/g2ewsb)**2)
      sw = g1ewsb/g2ewsb*cw
      sw2=sw**2
      if(su_isNaN(pizz).or.mz**2+pizz.le.0.d0) then
c !!! protections added
...
c  defining tree-level DRbar neutralino mass matrix:
      m(1,1) = m1 
      m(2,2) = m2 
      m(3,3) =0d0
      m(4,4) = 0d0
      ...
      m(4,3) = m(3,4)
c
c     tree masses +eigenvectors, RC included only at last iteration
c solving exactly the quartic neutralino eigenvalues equation:
      call su_inomass(m,xmn)
c
      do 10 i=1,4
c define DRbar 'tree' eigenvectors/diagonalization matrix elements
      r1= m(1,1)-xmn(i)
      r2= m(2,2)-xmn(i)
      r3= m(3,3)-xmn(i)
      zx(i,2)= -((r3*m(1,2)*m(1,4) -m(1,3)*m(1,4)*m(2,3) -r1*r3*m(2,4) + 
     . m(1,3)**2*m(2,4) - m(1,2)*m(1,3)*m(3,4) + r1*m(2,3)*m(3,4))/
     . (r2*r3*m(1,4) - m(1,4)*m(2,3)**2 - r3*m(1,2)*m(2,4) + 
     . m(1,3)*m(2,3)*m(2,4) - r2*m(1,3)*m(3,4) + m(1,2)*m(2,3)*m(3,4)))

      zx(i,3)=-((-(r2*m(1,3)*m(1,4)) +m(1,2)*m(1,4)*m(2,3)+m(1,2)*m(1,3)
     . *m(2,4) - r1*m(2,3)*m(2,4) + r1*r2*m(3,4) - m(1,2)**2*m(3,4))/
     . (-(r2*r3*m(1,4)) + m(1,4)*m(2,3)**2 + r3*m(1,2)*m(2,4) - 
     . m(1,3)*m(2,3)*m(2,4) + r2*m(1,3)*m(3,4) - m(1,2)*m(2,3)*m(3,4)))

      zx(i,4)= -((r1*r2*r3 -r3*m(1,2)**2 -r2*m(1,3)**2 +2*m(1,2)*m(1,3)*
     . m(2,3) - r1*m(2,3)**2)/
     . (r2*r3*m(1,4) - m(1,4)*m(2,3)**2 - r3*m(1,2)*m(2,4) + 
     . m(1,3)*m(2,3)*m(2,4) - r2*m(1,3)*m(3,4) + m(1,2)*m(2,3)*m(3,4)))
c
      zx(i,1)=1d0/dsqrt(1d0+zx(i,2)**2+zx(i,3)**2+zx(i,4)**2) 
      yz(i,1)=zx(i,1)
...
c save last RGE iteration DRbar 'tree' values used before calling RC:
      do i=1,4
      do j=1,4
      Mtr(i,j)=M(i,j)
      Zsave(i,j)= yz(i,j)
      ...
      mn(i)=dabs(xmn(i))
      enddo
c NB at this stage masses and matrix elmts Z(i,j) are saved as DRbar 'tree' values for later
c
        if(inorc.eq.1) then  ! neutralino mass RC calculated only at last iteration:
c   call full one-loop neutralino RC: 
       do ipole=1,2  ! iterated twice to reasonably appromixate RC scale(s)=pole mass(es)
         do inino = 1,4  ! 4 calls, at scales close to respective (ordered) pole masses
       call SU_RCNINO(mn(inino)**2,mnDRbar,xmctree,b, rcL,rcR,rcS)
c construct matrix rcN of neutralino RC:
c nb at first step tree-level M is used to be consistent with one-loop RCN matrix elmts
c defined here:
      do i=1,4
      do j=1,4
      rcN(i,j) = 0d0  ! reinitialization
        do k=1,4
      rcN(i,j) = rcN(i,j) 
     .-(rcR(i,k)*Mtr(k,j)+Mtr(i,k)*rcR(k,j)+Mtr(i,k)*rcL(k,j)+rcL(i,k)*
     . Mtr(k,j))/2
        enddo
      rcn(i,j)= rcn(i,j)-rcS(i,j)
      enddo
      enddo
c
c Now adding RC to all matrix elmts: nb for real MSSM the neutralino mass matrix 
c remains real symmetric even when including full one-loop matrix corrections:
      M(1,1) = m1 +rcn(1,1)
      M(2,2) = m2 +rcn(2,2)
      M(3,3) = rcn(3,3)
      M(4,4) = rcn(4,4)
      M(1,2) = rcn(1,2)
      M(2,1) = M(1,2)
      M(1,3) = -mz*cb*sw +rcn(1,3)
      M(3,1) = M(1,3)
      M(1,4) =  mz*sb*sw +rcn(1,4)
      M(4,1) = M(1,4)
      M(2,3) =  mz*cb*cw +rcn(2,3)
      M(3,2) = M(2,3)
      M(2,4) = -mz*sb*cw +rcn(2,4)
      M(4,2) = M(2,4)
      M(3,4) = -mu +rcn(3,4)
      M(4,3) = M(3,4)
c
c Solving exactly the quartic neutralino eigenvalues equation, now incl. RC:
      call su_inomass(m,xmn)
      xmnsave(inino) = xmn(inino) ! saving only ino mass i evaluated at its own scale
      enddo   ! end of loop on scale =mn(inino) ~pole masses used within RC calculations
c
      do i=1,4
      xmn(i)=xmnsave(i)
      mn(i)=dabs(xmn(i))
cc      dxmn(i)=xmn(i)
      enddo
c
       enddo  ! end of iteration to approximate scale(s)=pole mass(es)
c     ============ Saving final neutralino pole masses (incl. RC) for output:
      call su_ninord(xmn,zsave,z) ! reordering masses and Z matrix elmts consistently
      do i=1,4
      mn(i)=dabs(xmn(i))
      dxmn(i)=xmn(i)
      enddo
        endif  ! end of if(inorc.eq.1) calculation of RC
c
c     =================== Chargino masses and matrix elements =============
c
c   first define tree-level chargino mass matrix elmts:
        MMC(1,1)=m2
        MMC(1,2)=sq2*mw*dsin(b) 
        MMC(2,1)=sq2*mw*dcos(b) 
        MMC(2,2)=mu 
c
c  first stage: tree masses +eigenvectors, RC included only at last iteration
	delta=dabs(b-pi/4)
	ddd=mu*dcos(b)+m2*dsin(b)
	ccc=mu*dsin(b)+m2*dcos(b)
	if(delta.lt.1d-2) then
	phim=pi/4-.5d0*datan((m2-mu)/(2*mw))
	phip=phim
	else if	(dabs(ccc).lt.1d-5) then
	phim=0.d0
	phip=datan(sq2*mw*dsin(b)/(m2+1.d-5))
	else if	(dabs(ddd).lt.1d-5) then
	phip=0.d0
	phim=datan(sq2*mw*dcos(b)/(m2+1.d-5))
	else
	rad=dsqrt((m2**2-mu**2)**2+4*mw**4*dcos(2*b)**2
     .	+4*mw**2*(m2**2+mu**2+2*m2*mu*dsin(2*b)))
	phip=datan((rad-(m2**2-mu**2+2*mw**2*dcos(2*b)))
     .	/(2*sq2*mw*(mu*dcos(b)+m2*dsin(b))))
	phim=datan((rad-(m2**2-mu**2-2*mw**2*dcos(2*b)))
     .	/(2*sq2*mw*(mu*dsin(b)+m2*dcos(b))))
	endif
	cp=dcos(phip)
	sp=dsin(phip)
	cm=dcos(phim)
...
c  my convention
	...
	v(2,2)=cp
 555    continue
       xmc(1)=(u(1,1)*MMC(1,1)+u(1,2)*MMC(2,1))*v(1,1)
     .       +(u(1,1)*MMC(1,2)+u(1,2)*MMC(2,2))*v(1,2)
       xmc(2)=(u(2,1)*MMC(1,1)+u(2,2)*MMC(2,1))*v(2,1)
     .       +(u(2,1)*MMC(1,2)+u(2,2)*MMC(2,2))*v(2,2)
        if(xmc(1).lt.0.d0) then
c some corrections to deal with case where  MC1 and/or MC2 < 0:
        v(1,1)=-v(1,1)
        v(1,2)=-v(1,2)
        goto 555
        endif
...
c    Some saving of DRbar 'tree' values!
      ...
      enddo     
c
        if(inorc.eq.1) then   ! RC calculation done at last iteration
c first tree-level MMC to be consistent with one-loop RCC matrix elmts defined here:
c   call full one-loop chargino RC:
       do ipole=1,2  ! iterated twice to reasonably appromixate RC scale(s)=pole mass(es)
       if(ipole.eq.1) then ! first time initialize p_scale for RC with tree masses 
       do k=1,2
       xmcsave(k)=xmc(k)
       enddo
       endif

         do icino = 1,2  ! 2 calls, at scales close to respective (ordered) pole masses
      do i=1,2
      do j=1,2
      u(i,j)=usave(i,j)  ! make sure that DRbar matrix elmts are used within RC loops su_RCCINO
      v(i,j)=vsave(i,j)
      enddo
      enddo     
       call SU_RCCINO(xmc(icino)**2,mnDRbar,xmctree,b, rcCL,rcCR,rcCS)
c 
      do i=1,2
      do j=1,2
      rcCST(i,j)=rcCS(j,i)
      ...
      do i=1,2
      do j=1,2
      rcC(i,j)=0d0  ! reinitialization
        do k=1,2
      rcC(i,j) = rcC(i,j) 
     .-(rcCR(i,k)*MMCtr(k,j)+MMCtr(i,k)*rcCL(k,j)) ! nb actually should be rcCR^T,rcCL^T 
        enddo                                  ! but these are symmetrical (for real MSSM)
      rcC(i,j)= rcC(i,j)-rcCST(i,j)            ! note rcCST=rcCS^T, rcCS not symmetrical
      enddo
      enddo

c Now construct final matrix elmts including rcC of chargino RC:
        MMC(1,1)=m2 +rcC(1,1)
        MMC(1,2)=sq2*mw*dsin(b) +rcC(1,2)
        MMC(2,1)=sq2*mw*dcos(b) +rcC(2,1)
        MMC(2,2)=mu +rcC(2,2)
c then diagonalize exactly in each cases i.e. for different scales~pole masses:
	rad=dsqrt(((MMC(1,2)+MMC(2,1))**2 +(MMC(1,1)-MMC(2,2))**2)*
     .  ((MMC(1,2)-MMC(2,1))**2 +(MMC(1,1)+MMC(2,2))**2))
	phip=datan((rad+MMC(2,2)**2-MMC(1,1)**2+MMC(1,2)**2-MMC(2,1)**2)
     .	/(2*(MMC(1,1)*MMC(1,2) +MMC(2,2)*MMC(2,1) ) ) )
	phim=datan((rad+MMC(2,2)**2-MMC(1,1)**2-MMC(1,2)**2+MMC(2,1)**2)
     .	/(2*(MMC(1,1)*MMC(2,1) +MMC(2,2)*MMC(1,2) ) ) )
c
	cp=dcos(phip)
	sp=dsin(phip)
	cm=dcos(phim)
	sm=dsin(phim)
c  my convention ! the following U,V matrix elmts' contain RC but only used for pole masses:
        u(1,1)=cm
	u(1,2)=sm
	u(2,1)=-sm
	u(2,2)=cm
 ...
 556    continue
     ...
     .       +(u(1,1)*MMC(1,2)+u(1,2)*MMC(2,2))*v(1,2)
       xmc(2)=(u(2,1)*MMC(1,1)+u(2,2)*MMC(2,1))*v(2,1)
     .       +(u(2,1)*MMC(1,2)+u(2,2)*MMC(2,2))*v(2,2)
       xmcsave(icino) = xmc(icino) ! saving only chargino mass evaluated at its own pole mass scale
c  
        if(xmc(1).lt.0.d0) then
c some corrections to deal with case where BOTH MCh1,MCh2 <0:
        v(1,1)=-v(1,1)
        v(1,2)=-v(1,2)
        goto 556
        endif
        ...
        goto 556
          ...
          enddo  ! end of loop on two different ~pole mass scale used within RC
         enddo   ! end of iteration to approximate true chargino pole masses
c
c restoring DRbar values of U,V for outside after all RC have been calculated:
       do i=1,2
      do j=1,2
      u(i,j)=usave(i,j)
      v(i,j)=vsave(i,j)
      enddo
      enddo   
c reordering mass eigenvalues/vectors at the very end and saving pole masses for outside:  
       if(xmcsave(1).gt.xmcsave(2)) then
        mtemp=xmcsave(1)
        xmc(1)=xmcsave(2)
        xmc(2)=mtemp  
        do j=1,2
        ...
        mc(1)=dabs(xmcsave(1))
        mc(2)=dabs(xmcsave(2))        
         endif  ! end of chargino mass RC if(inorc.eq.1)
c
        mc(1)=dabs(xmc(1))
        mc(2)=dabs(xmc(2))        
      mz = mzsave               
...
c-------------------------
      subroutine su_inomass(m,xmn)
c     solve the 4th order eigenvalue eq. for input neutralino mass matrix elmts m(i,j):
c     also valid with RC included. Output: eigenvalues xmn(i), including signs.
      implicit real*8(a-h,m-z)
      complex*16 cx(4)
      dimension m(4,4), xmn(4)
c  coefficients of the eigenvalue eq. x^4+c3 x^3+c2 x^2+c1 x+c0=0:
      c3= -(m(1,1) + m(2,2) + m(3,3) + m(4,4))

      c2 = -m(1,2)**2 - m(1,3)**2 - m(1,4)**2 - m(2,3)**2 - m(2,4)**2 
     . -m(3,4)**2 + m(3,3)*m(4,4) + m(2,2)*(m(3,3) + m(4,4))  
     . +m(1,1)*(m(2,2) + m(3,3) + m(4,4))

      c1 =-2*m(1,2)*m(1,4)*m(2,4) + m(2,4)**2*m(3,3) + m(1,4)**2*(m(2,2) 
     . + m(3,3)) - 2*m(2,3)*m(2,4)*m(3,4) + m(2,2)*m(3,4)**2 - 
     . 2*m(1,3)*(m(1,2)*m(2,3) + m(1,4)*m(3,4)) + m(2,3)**2*m(4,4) - 
     . m(2,2)*m(3,3)*m(4,4) + m(1,3)**2*(m(2,2) + m(4,4)) + 
     . m(1,2)**2*(m(3,3) + m(4,4)) + 
     . m(1,1)*(m(2,3)**2 + m(2,4)**2 - m(2,2)*m(3,3) + m(3,4)**2 - 
     . (m(2,2) + m(3,3))*m(4,4))


      c0 = m(1,4)**2*(m(2,3)**2 - m(2,2)*m(3,3)) + 
     . 2*m(1,4)*(m(1,3)*(-(m(2,3)*m(2,4)) + m(2,2)*m(3,4)) + 
     . m(1,2)*(m(2,4)*m(3,3) - m(2,3)*m(3,4))) + 
     . m(1,3)**2*(m(2,4)**2 - m(2,2)*m(4,4)) + 
     . 2*m(1,2)*m(1,3)*(-(m(2,4)*m(3,4)) + m(2,3)*m(4,4)) + 
     . m(1,2)**2*(m(3,4)**2 - m(3,3)*m(4,4)) - 
     . m(1,1)*(m(2,4)**2*m(3,3) - 2*m(2,3)*m(2,4)*m(3,4) 
     . + m(2,3)**2*m(4,4) +m(2,2)*(m(3,4)**2 - m(3,3)*m(4,4)))
c  fourth order polynomial eq. solver:
      call solve4(c3,c2,c1,c0,cx)
      do i=1,4
      xmn(i) =dble(cx(i))
      enddo
      end
c-------------------------
      subroutine solve4(a,b,c,d,x)
c general 4th order algebraic equation solver
c x^4 +a x^3 +b x^2 +c x +d =0
c output x is 4-vector of solutions
c (traditional Tartaglia-Cardan-Ferrari method; coded in this form jlk)
      implicit real*8(a-h,k-z)
      complex*16 x(4),y(3),t,s,u
c   x->x -a/4 to give depressed form x^4 = p x^2 +q x +r:
      p = -b+3*a**2/8
      q = -(a**3-4*a*b+8*c)/8
      r = 3*a**4/256 -b*a**2/16 +a*c/4 -d
c  solve x^4 =(x^2+t)^2 -2t x^2 -t^2 = ..: solve for t / (x^2+t)^2= (s x+u)^2:
c (3rd order eq. in t):
      call solve3(1d0, p/2, r, p*r/2-q**2/8, y)
      t=y(1) ! any of the 3 solutions would do by symmetry
      s = cdsqrt(2*t+p)
      u = q/s/2
      x(1) =(s +cdsqrt(s**2-4*(t-u)))/2  -a/4
      x(2) =(s -cdsqrt(s**2-4*(t-u)))/2  -a/4
      x(3) =(-s +cdsqrt(s**2-4*(t+u)))/2 -a/4
      x(4) =(-s -cdsqrt(s**2-4*(t+u)))/2 -a/4
      end
c----------------
      subroutine solve3(a,b,c,d,cx)
c general 3rd order algebraic equation solver
c  a x^3 +b x^2 +c x +d =0
c output cx is complex 3-vector of solutions
c (traditional Tartaglia-Cardan method)
c
      implicit real*8(a-h,k-z)
      complex*16 cx(3),disc,cxc,cphase(3)
      cphase(1) =dcmplx(1d0,0d0)
      cphase(2) = dcmplx(-1d0/2,dsqrt(3d0)/2)  
      cphase(3) = dcmplx(-1d0/2,-dsqrt(3d0)/2) 
      al= b**2-3*a*c
      bet= 2*b**3-9*a*b*c+27*a**2*d
      disc= (bet**2-4*al**3)*dcmplx(1d0,0d0)
c nb: disc >0 => 3 distinct real roots; disc <0 => 1 real root +2 c.c.; disc =0 => 3 multiple roots 
      cxc = ((-bet+cdSqrt(disc))/2)**(1d0/3)
      do i=1,3
      cx(i) = (-b +cphase(i)*cxc +al/cphase(i)/cxc)/a/3 
      enddo
      end
c-------------------------------------------
      subroutine su_ninord(xmn,yz,z)
c   reordering neutralino masses and corresponding Z matrix elmts
      implicit real*8(a-h,m-z)
      dimension xmn(4),mn(4),iord(4),irem(4),ymn(4),yz(4,4),z(4,4)
c
      do i=1,4
      mn(i)=dabs(xmn(i))
      ymn(i)=xmn(i)
...
c  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     .            rcm1,rcm2,rcmu)
c  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c !!! obsolete routine: no longer used/called, just kept for possible crosschecks
c  Calculates the radiative corrections to the gaugino and MU mass 
c  parameters in approximations. 
...
c
c  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
       SUBROUTINE SU_RCCINO(q2,mn,mc,B, rcCL,rcCR,rcCS)
c  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c  Calculates the full one-loop radiative corrections to the chargino masses 
c  according to PBMZ expressions.  
c  Output are the RC matrix elements rcL,rcR,rcS(4,4) for neutralinos
c  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c 
      implicit real*8(a-h,m,o-z)
      real*8 nf
      complex*16 SU_B1,SU_B0
      dimension u(2,2),v(2,2),z(4,4),dxmn(4),mn(4),mc(2),rcCL(2,2),
     . rcCR(2,2),rcCS(2,2),rcCfsfL(2,2),rcCfsfR(2,2),xmctree(2),
     . rcCfsfS(2,2),
     . rcCCzL(2,2),rcCCzR(2,2),rcCCzS(2,2),rcCCgL(2,2),rcCCgR(2,2),
     . rcCCgS(2,2), rcCNwL(2,2),rcCNwR(2,2),rcCNwS(2,2),
     . rcCCHL(2,2),rcCCHR(2,2),rcCCHS(2,2), 
     . rccnHpL(2,2),rccnHpR(2,2),rccnHpS(2,2),
     . acuL(2),bcuL(2),bctL(2),acuR(2),actR(2),bcuR(2),acdL(2),acbL(2),
     . bcdL(2),acdR(2),bcdR(2),bcbR(2),aceL(2),bceL(2),aceR(2),bceR(2),
     . bctauR(2),acnue(2),
     . acchl(4,4),acchH(4,4),acchA(4,4),accG0(4,4),
     . bcchl(4,4),bcchH(4,4),bcchA(4,4),bccG0(4,4),
     . acnHp(2,4),acnGp(2,4),bcnHp(2,4),bcnGp(2,4)

      COMMON/SU_cte/nf,cpi,mz_pole,mwdum,tbetdum
      common/SU_runhiggsewsb/ma,ml,mh,mch,alfa
cc      COMMON/SU_hmass/ma,ml,mh,mch,mar
cc      COMMON/SU_outhiggs/dml,dmh,dmch,alfa 
      COMMON/SU_outginos/mc1,mc2,mn1,mn2,mn3,mn4,mgluino
      COMMON/SU_inoDR/U,V,Z,xmctree,dxmn
      COMMON/SU_yukaewsb/ytau,yb,yt,alsewsb,g2ewsb,g1ewsb
      COMMON/SU_tbewsb/vuewsb,vdewsb 
...
c
      ...
      zero = 1d-2
c  chargino-down-sup :
c distinguish 3rd from 1,2 generation for yukawa cplings, when necessary:
c (! bug in bcuL, acuR, corrected from v2.5 to v2.5.1)
       acuL(1) =g
       bcuL(1) =0d0
       bctL(1) =0d0
       acuR(1) =0d0
       ...
       acuL(2) =0d0
       bctL(2) =-yb
       bcuL(2) = 0d0   ! actually Yukawa non zero but negligible for 1,2 generation
       actR(2) =-yt
       acuR(2) = 0d0  ! actually Yukawa non zero but negligible
       bcuR(2) =0d0
c  chargino-up-sdown:
       acdL(1)=0.d0
       acbL(1)=0.d0
       ...
       bcbR(1)=0.d0
       acbL(2)=-yt
       acdL(2)= 0d0  ! actually Yukawa non zero but negligible
       bcdL(2)=0.d0
       acdR(2)=0.d0
       bcbR(2)=-yb
       bcdR(2)= 0.d0 ! actually Yukawa non zero but negligible
c  chargino-lepton-sneutrino:
      aceL(1) =0d0
      bceL(1) =g
      aceR(1) =0d0
      bceR(1) =0d0
      ...
      aceR(2) =0d0
      bctauR(2) =-ytau
      bceR(2) = 0d0  ! actually Yukawa non zero but negligible
c chargino-neutrino-slepton: 
      acnue(1) =g
      acnue(2) =0d0
c chargino-neutralino-W (same as neutralino-chargino-W for real MSSM U*,V*=U,V):
      an2c1w = -g
      bn2c1w = -g
      an4c2w = g/sq2
      bn3c2w = -g/sq2
c chargino-chargino-Z
      ac1c1z = g*cw
      bc1c1z = ac1c1z
      ac2c2z = g*(cw**2-sw**2)/cw/2
      bc2c2z = ac2c2z
c chargino-chargino-photon
      ac1c1g = e
      bc1c1g = ac1c1z
      ac2c2g = e
      bc2c2g = ac2c2z
c chargino-chargino-H0/A0/G0
      ac1c2s1 = g/sq2
      ac2c1s2 = g/sq2
      ac1c2p1 = g/sq2
      ac2c1p2 = -g/sq2
      ...
      bc2c1p2= -ac2c1p2
      do j=1,2
      acchl(1,j) = -sa*U(j,2)*ac1c2s1
      acchH(1,j) =  ca*U(j,2)*ac1c2s1
      acchA(1,j) = -sbet*U(j,2)*ac1c2p1
      accG0(1,j) =  cbet*U(j,2)*ac1c2p1
c
      bcchl(1,j) = -sa*V(j,2)*bc1c2s1
      bcchH(1,j) =  ca*V(j,2)*bc1c2s1
      bcchA(1,j) = -sbet*V(j,2)*bc1c2p1
      bccG0(1,j) =  cbet*V(j,2)*bc1c2p1
c
      acchl(2,j) = -sa*U(j,1)*ac2c1s2
      acchH(2,j) =  ca*U(j,1)*ac2c1s2
      acchA(2,j) = -sbet*U(j,1)*ac2c1p2
      accG0(2,j) =  cbet*U(j,1)*ac2c1p2
c
      bcchl(2,j) = -sa*V(j,1)*bc2c1s2
      bcchH(2,j) =  ca*V(j,1)*bc2c1s2
      bcchA(2,j) = -sbet*V(j,1)*bc2c1p2
      bccG0(2,j) =  cbet*V(j,1)*bc2c1p2
      enddo
c chargino-neutralino-H+/G+
      an1c2h1 = g1/sq2
      bn1c2h2 = an1c2h1
      an2c2h1 = g/sq2
      bn2c2h2 = an2c2h1
...
c
      do j=1,4
      acnHp(1,j) = cbet*Z(j,4)*bn4c1h2
      acnGp(1,j) = sbet*Z(j,4)*bn4c1h2
      acnHp(2,j) = cbet*(Z(j,1)*bn1c2h2+Z(j,2)*bn2c2h2)
      acnGp(2,j) = sbet*(Z(j,1)*bn1c2h2+Z(j,2)*bn2c2h2)
c
      bcnHp(1,j) = -sbet*Z(j,3)*an3c1h1
      bcnGp(1,j) =  cbet*Z(j,3)*an3c1h1
      bcnHp(2,j) = -sbet*(Z(j,1)*an1c2h1+Z(j,2)*an2c2h1)
      bcnGp(2,j) = cbet*(Z(j,1)*an1c2h1+Z(j,2)*an2c2h1)
      enddo
c
c
      do i=1,2
      do j=1,2
      rcCCHL(i,j)= 0d0   ! reinitializations
      rcCCHR(i,j)= 0d0
      rcCCHS(i,j)= 0d0
      rcCNHpL(i,j)= 0d0
      rcCNHpR(i,j)= 0d0
...
c  fermion/sfermion contributions
      rcCfsfL(i,j)=(3*((ct*acuL(i)+st*actR(i))*(ct*acuL(j)+st*actR(j))* ! bottom/stop
     . dble(SU_B1(q2,rmb,mst1))+
     .               (-st*acuL(i)+ct*actR(i))*(-st*acuL(j)+ct*actR(j))*   
     . dble(SU_B1(q2,rmb,mst2)))  +  
     .               6*acuL(i)*acuL(j)*    ! d,s/su_L,sc_L
     . dble(SU_B1(q2,zero,msu1)) +  
     .               6*acuR(i)*acuR(j)*    ! d,s/su_R,sc_R: actually zero from negligible yukawas
     . dble(SU_B1(q2,zero,msu2)) +         ! but kept for generality
     .                 aceL(i)*aceL(j)*  ! lepton/sneutrino
     . (dble(SU_B1(q2,rmtau,msntau)) +2*dble(SU_B1(q2,zero,msn1))) +  
     .              3*((cb*acbL(i)+sb*acdR(i))*(cb*acbL(j)+sb*acdR(j))*  ! top/sbottom
     . dble(SU_B1(q2,rmt,msb1))+
     .               (-sb*acbL(i)+cb*acdR(i))*(-sb*acbL(j)+cb*acdR(j))*
     . dble(SU_B1(q2,rmt,msb2))) +  
     .               6*acdL(i)*acdL(j)*   ! u,c/sd_L,ss_L
     . dble(SU_B1(q2,zero,msd1)) +  
     .               6*acdR(i)*acdR(j)*   ! u,c/sd_R,ss_R but zero from negligible yukawas
     . dble(SU_B1(q2,zero,msd2)) +
     .             (cta*aceL(i)+sta*aceR(i))*(cta*aceL(j)+sta*aceR(j))* !neutrino/stau
     . dble(SU_B1(q2,zero,msta1))+
     .           (-sta*aceL(i)+cta*aceR(i))*(-sta*aceL(j)+cta*aceR(j))*
     . dble(SU_B1(q2,zero,msta2)) +
     .               2*acnue(i)*acnue(j)*      ! neutrino/se,smu
     . dble(SU_B1(q2,zero,mse2))  )/2  
c
      rcCfsfR(i,j)=(3*((ct*bctL(i)+st*bcuR(i))*(ct*bctL(j)+st*bcuR(j))* ! bottom/stop
     . dble(SU_B1(q2,rmb,mst1))+
     .               (-st*bctL(i)+ct*bcuR(i))*(-st*bctL(j)+ct*bcuR(j))*   
     . dble(SU_B1(q2,rmb,mst2)))  +  
c   ! d,s/su,sc contribution = 0 since bcuL,bcuR ~ 0 for first 2 gen.
     .              3*((cb*bcdL(i)+sb*bcbR(i))*(cb*bcdL(j)+sb*bcbR(j))*  ! top/sbottom
     . dble(SU_B1(q2,rmt,msb1))+
     .               (-sb*bcdL(i)+cb*bcbR(i))*(-sb*bcdL(j)+cb*bcbR(j))*
     . dble(SU_B1(q2,rmt,msb2))) +  
     .               6*bcdL(i)*bcdL(j)*   ! u,c/sd_L,ss_L
     . dble(SU_B1(q2,zero,msd1)) +  
     .               6*bcdR(i)*bcdR(j)*   ! u,c/sd_R,ss_R but zero from negligible yukawas
     . dble(SU_B1(q2,zero,msd2)) +
     .             (cta*bceL(i)+sta*bctauR(i))*
     .             (cta*bceL(j)+sta*bctauR(j))* !neutrino/stau
     . dble(SU_B1(q2,zero,msta1))+
     .           (-sta*bceL(i)+cta*bctauR(i))*
     .           (-sta*bceL(j)+cta*bctauR(j))*
     . dble(SU_B1(q2,zero,msta2)) )/2
c        ! neutrino/se,smu contribution = 0 since bcnu ~ 0 for first 2 gen.
c S-contribution: note no contributions from first two gen. since either a or b cplings are ~0 
      rcCfsfS(i,j)= 3*((ct*bctL(i)+st*bcuR(i))*(ct*acuL(j)+st*actR(j))* 
     . rmb*dble(SU_B0(q2,rmb,mst1))+
     .               (-st*bctL(i)+ct*bcuR(i))*(-st*acuL(j)+ct*actR(j))*   
     . rmb*dble(SU_B0(q2,rmb,mst2)))    +
     .              3*((cb*bcdL(i)+sb*bcbR(i))*(cb*acbL(j)+sb*acdR(j))* 
     . rmt*dble(SU_B0(q2,rmt,msb1))+
     .               (-sb*bcdL(i)+cb*bcbR(i))*(-sb*acbL(j)+cb*acdR(j))*
     . rmt*dble(SU_B0(q2,rmt,msb2))) 
c   ! also no neutrino/stau nor tau/sneutrino contributions since either a or b or m_neut =0
c
c   chargino-H0/A0/G0 contributions
        do k=1,2
      rcCCHL(i,j) =rcCCHL(i,j) +
     .   (acchl(i,k)*acchl(j,k)*dble(SU_B1(q2,mc(k),ml)) +
     .    acchH(i,k)*acchH(j,k)*dble(SU_B1(q2,mc(k),mh)) +
     .    acchA(i,k)*acchA(j,k)*dble(SU_B1(q2,mc(k),mA)) +
     .    accG0(i,k)*accG0(j,k)*dble(SU_B1(q2,mc(k),mz)) )/2
c
      rcCCHR(i,j) =rcCCHR(i,j) + 
     .   (bCChl(i,k)*bCChl(j,k)*dble(SU_B1(q2,mc(k),ml)) +
     .    bCChH(i,k)*bCChH(j,k)*dble(SU_B1(q2,mc(k),mh)) +
     .    bCChA(i,k)*bCChA(j,k)*dble(SU_B1(q2,mc(k),mA)) +
     .    bCCG0(i,k)*bCCG0(j,k)*dble(SU_B1(q2,mc(k),mz)) )/2
c  S contributions
      rcCCHS(i,j) =rcCCHS(i,j) + 
     .    bCChl(i,k)*aCChl(j,k)*mc(k)*dble(SU_B0(q2,mc(k),ml)) +
     .    bCChH(i,k)*aCChH(j,k)*mc(k)*dble(SU_B0(q2,mc(k),mh)) +
     .    bCChA(i,k)*aCChA(j,k)*mc(k)*dble(SU_B0(q2,mc(k),mA)) +
     .    bCCG0(i,k)*aCCG0(j,k)*mc(k)*dble(SU_B0(q2,mc(k),mz)) 
        enddo 
c
c chargino-H+/G+ contributions
        do k=1,4
      rcCNHpL(i,j) =rcCNHpL(i,j) +
     .  (acnHp(i,k)*acnHp(j,k)*dble(SU_B1(q2,mn(k),mch)) +
     .  acnGp(i,k)*acnGp(j,k)*dble(SU_B1(q2,mn(k),mw)) )/2
c
      rcCNHpR(i,j) =rcCNHpR(i,j) +
     .  (bcnHp(i,k)*bcnHp(j,k)*dble(SU_B1(q2,mn(k),mch)) +
     .  bcnGp(i,k)*bcnGp(j,k)*dble(SU_B1(q2,mn(k),mw)) )/2
c  S contributions
      rcCNHpS(i,j) =rcCNHpS(i,j) +
     .  bcnHp(i,k)*acnHp(j,k)*mn(k)*dble(SU_B0(q2,mn(k),mch)) +
     .  bcnGp(i,k)*acnGp(j,k)*mn(k)*dble(SU_B0(q2,mn(k),mw)) 
        enddo
c
      enddo
      enddo
c  neutralino-W contributions
      do k=1,4
      rcCNwL(1,1) = rcCNwL(1,1) +(Z(k,2)*an2c1w)**2*
     . dble(SU_B1(q2,mn(k),mw)) 
      rcCNwL(2,2) = rcCNwL(2,2) +(Z(k,4)*an4c2w)**2*
     . dble(SU_B1(q2,mn(k),mw)) 
      rcCNwL(1,2) = rcCNwL(1,2) +(Z(k,2)*an2c1w)*(Z(k,4)*an4c2w)*
     . dble(SU_B1(q2,mn(k),mw)) 
      rcCNwL(2,1) = rcCNwL(1,2) 
c
      rcCNwR(1,1) = rcCNwR(1,1) +(Z(k,2)*bn2c1w)**2*
     . dble(SU_B1(q2,mn(k),mw)) 
      rcCNwR(2,2) = rcCNwR(2,2) +(Z(k,3)*bn3c2w)**2*
     . dble(SU_B1(q2,mn(k),mw)) 
      rcCNwR(1,2) = rcCNwR(1,2) +(Z(k,2)*bn2c1w)*(Z(k,3)*bn3c2w)*
     . dble(SU_B1(q2,mn(k),mw)) 
      rcCNwR(2,1) = rcCNwR(1,2) 
c
c S contributions
      rcCNwS(1,1) = rcCNwS(1,1) +(Z(k,2)*bn2c1w)*(Z(k,2)*an2c1w)*
     . mn(k)*dble(SU_B0(q2,mn(k),mw)) 
      rcCNwS(2,2) = rcCNwS(2,2) +(Z(k,3)*bn3c2w)*(Z(k,4)*an4c2w)*
     . mn(k)*dble(SU_B0(q2,mn(k),mw)) 
      rcCNwS(1,2) = rcCNwS(1,2) +(Z(k,2)*bn2c1w)*(Z(k,4)*an4c2w)*
     . mn(k)*dble(SU_B0(q2,mn(k),mw)) 
      rcCNwS(2,1) = rcCNwS(2,1) +(Z(k,3)*bn3c2w)*(Z(k,2)*an2c1w)*
     . mn(k)*dble(SU_B0(q2,mn(k),mw)) 
      enddo
c ! note the last S(i,j) is not symmetrical 
c
c chargino-Z contributions
      do k=1,2
      rcCCzL(1,1) = rcCCzL(1,1) +(V(k,1)*ac1c1z)**2*
     . dble(SU_B1(q2,mc(k),mz)) 
      rcCCzL(2,2) = rcCCzL(2,2) +(V(k,2)*ac2c2z)**2*
     . dble(SU_B1(q2,mc(k),mz)) 
      rcCCzL(1,2) = rcCCzL(1,2) +(V(k,1)*ac1c1z)*(V(k,2)*ac2c2z)*
     . dble(SU_B1(q2,mc(k),mz)) 
      rcCCzL(2,1) = rcCCzL(1,2) 
c
      rcCCzR(1,1) = rcCCzR(1,1) +(V(k,1)*bc1c1z)**2*
     . dble(SU_B1(q2,mc(k),mz)) 
      rcCCzR(2,2) = rcCCzR(2,2) +(V(k,2)*bc2c2z)**2*
     . dble(SU_B1(q2,mc(k),mz)) 
      rcCCzR(1,2) = rcCCzR(1,2) +(V(k,1)*bc1c1z)*(V(k,2)*bc2c2z)*
     . dble(SU_B1(q2,mc(k),mz)) 
      rcCCzR(2,1) = rcCCzR(1,2) 
c S contributions  (! not symmetrical)
      rcCCzS(1,1) = rcCCzS(1,1) +(V(k,1)*bc1c1z)*(V(k,1)*ac1c1z)*
     . mc(k)*dble(SU_B0(q2,mc(k),mz)) 
      rcCCzS(1,2) = rcCCzS(1,2) +(V(k,1)*bc1c1z)*(V(k,2)*ac2c2z)*
     . mc(k)*dble(SU_B0(q2,mc(k),mz)) 
      rcCCzS(2,1) = rcCCzS(2,1) +(V(k,2)*bc2c2z)*(V(k,1)*ac1c1z)*
     . mc(k)*dble(SU_B0(q2,mc(k),mz)) 
      rcCCzS(2,2) = rcCCzS(2,2) +(V(k,2)*bc2c2z)*(V(k,2)*ac2c2z)*
     . mc(k)*dble(SU_B0(q2,mc(k),mz))
      enddo 
c chargino-photon contributions
      do k=1,2
      rcCCgL(1,1) = rcCCgL(1,1) +(V(k,1)*ac1c1g)**2*
     . dble(SU_B1(q2,mc(k),zero)) 
      rcCCgL(2,2) = rcCCgL(2,2) +(V(k,2)*ac2c2g)**2*
     . dble(SU_B1(q2,mc(k),zero)) 
      rcCCgL(1,2) = rcCCgL(1,2) +(V(k,1)*ac1c1g)*(V(k,2)*ac2c2g)*
     . dble(SU_B1(q2,mc(k),zero)) 
      rcCCgL(2,1) = rcCCgL(1,2) 
c
      rcCCgR(1,1) = rcCCgR(1,1) +(V(k,1)*bc1c1g)**2*
     . dble(SU_B1(q2,mc(k),zero)) 
      rcCCgR(2,2) = rcCCgR(2,2) +(V(k,2)*bc2c2g)**2*
     . dble(SU_B1(q2,mc(k),zero)) 
      rcCCgR(1,2) = rcCCgR(1,2) +(V(k,1)*bc1c1g)*(V(k,2)*bc2c2g)*
     . dble(SU_B1(q2,mc(k),zero)) 
      rcCCgR(2,1) = rcCCgR(1,2) 
c S contributions  ( symmetrical because all cplings =e)
      rcCCgS(1,1) = rcCCgS(1,1) +(V(k,1)*bc1c1g)*(V(k,1)*ac1c1g)*
     . mc(k)*dble(SU_B0(q2,mc(k),zero)) 
      rcCCgS(1,2) = rcCCgS(1,2) +(V(k,1)*bc1c1g)*(V(k,2)*ac2c2g)*
     . mc(k)*dble(SU_B0(q2,mc(k),zero)) 
      rcCCgS(2,1) = rcCCgS(2,1) +(V(k,2)*bc2c2g)*(V(k,1)*ac1c1g)*
     . mc(k)*dble(SU_B0(q2,mc(k),zero)) 
      rcCCgS(2,2) = rcCCgS(2,2) +(V(k,2)*bc2c2g)*(V(k,2)*ac2c2g)*
     . mc(k)*dble(SU_B0(q2,mc(k),zero)) 
      enddo
c
      do i=1,2
      do j=1,2
      rcCL(i,j)=cpi*(rcCfsfL(i,j)+rcCCHL(i,j)+rcCNHpL(i,j)+rcCCZL(i,j)+
     . rcCCgL(i,j)+ rcCNwL(i,j))
      rcCR(i,j)=cpi*(rcCfsfR(i,j)+rcCCHR(i,j)+rcCNHpR(i,j)+rcCCZR(i,j)+
     . rcCCgR(i,j)+ rcCNwR(i,j))
      rcCS(i,j)=cpi*(rcCfsfS(i,j) -4*rcCNwS(i,j)  
     . -4*rcCCZS(i,j) -4*rcCCgS(i,j)       
     . +rcCNHpS(i,j)+rcCCHS(i,j)) 
      enddo                                                          
      enddo

      end
c  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
       SUBROUTINE SU_RCNINO(q2,mn,mc,B, rcL,rcR,rcS)
c  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c  Calculates the full one-loop radiative corrections to the neutralino masses 
c  according to PBMZ expressions.  
c  Output are the RC matrix elements rcL,rcR,rcS(4,4) for neutralinos
c  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c 
      ...
      real*8 nf
      complex*16 SU_B1,SU_B0
      dimension u(2,2),v(2,2),z(4,4),dxmn(4),mn(4),mc(2),xmctree(2),
     . rcL(4,4),
     . rcR(4,4),rcS(4,4),rcNfsfL(4,4),rcNfsfR(4,4),rcNfsfS(4,4),
     . rcNnZL(4,4),rcNnZR(4,4),rcNnZS(4,4),rcNcWL(4,4),rcNcWR(4,4),
     . rcNcWS(4,4),rcnnHL(4,4),rcnnHR(4,4),rcnnHS(4,4), 
     . rcncHpL(4,4),rcncHpR(4,4),rcncHpS(4,4),
     . anuL(4),bnuL(4),anuR(4),bnuR(4),andL(4),bndL(4),andR(4),bndR(4),
     . aneL(4),bneL(4),aneR(4),bneR(4),annu(4),bnnu(4),
c     . acuL(2),bcuL(2),acuR(2),bcuR(2),acdL(2),bcdL(2),acdR(2),bcdR(2),
c     . aceL(2),bceL(2),aceR(2),bceR(2),acnue(2),bcnue(2),
     . annhl(4,4),annhH(4,4),annhA(4,4),annG0(4,4),
     . bnnhl(4,4),bnnhH(4,4),bnnhA(4,4),bnnG0(4,4),
     . ancHp(4,2),ancGp(4,2),bncHp(4,2),bncGp(4,2)
...
c neutralino-up-sup:
       anuL(1) = 0d0
       bnuL(1) = g1/sq2*(1.d0/3)
       anuR(1) = g1/sq2*(-4.d0/3)
       bnuR(1) = 0d0
       anuL(2) = 0d0
       bnuL(2) = sq2*g*(.5d0)
       anuR(2) = 0d0
       ...
       bnuR(4) = yt
c
c neutralino-down-Sdown:
       andL(1) = 0.d0                 
       bndL(1) = g1/sq2*(1.d0/3)
       andR(1) = g1/sq2*(2.d0/3)
       bndR(1) = 0.d0
       andL(2) = 0.d0
       bndL(2) = sq2*g*(-.5d0)
       andR(2) = 0.d0
...
c neutralino-lepton-slepton:
      ...
      bneL(4) = 0d0
      bneR(4) = 0d0
c neutralino-neutrino-sneutrino:
      annu(1) = 0d0
      bnnu(1) = g1/sq2*(-1d0)
      annu(2) = 0d0
      bnnu(2) = sq2*g*(.5d0)
...
c 
      do j=1,4
      annhl(1,j) = -sa*Z(j,3)*an1n3s1+ca*Z(j,4)*an1n4s2
      annhH(1,j) =  ca*Z(j,3)*an1n3s1+sa*Z(j,4)*an1n4s2
      annhA(1,j) = -sbet*Z(j,3)*an1n3p1+cbet*Z(j,4)*an1n4p2
      annG0(1,j) =  cbet*Z(j,3)*an1n3p1+sbet*Z(j,4)*an1n4p2
c
      annhl(2,j) = -sa*Z(j,3)*an2n3s1+ca*Z(j,4)*an2n4s2
      annhH(2,j) =  ca*Z(j,3)*an2n3s1+sa*Z(j,4)*an2n4s2
      annhA(2,j) = -sbet*Z(j,3)*an2n3p1+cbet*Z(j,4)*an2n4p2
      annG0(2,j) =  cbet*Z(j,3)*an2n3p1+sbet*Z(j,4)*an2n4p2
c     
      annhl(3,j) = -sa*(Z(j,1)*an1n3s1+Z(j,2)*an2n3s1)
      annhH(3,j) =  ca*(Z(j,1)*an1n3s1+Z(j,2)*an2n3s1)
      annhA(3,j) = -sbet*(Z(j,1)*an1n3p1+Z(j,2)*an2n3p1)
      annG0(3,j) =  cbet*(Z(j,1)*an1n3p1+Z(j,2)*an2n3p1)
c     
      annhl(4,j) =  ca*(Z(j,1)*an1n4s2+Z(j,2)*an2n4s2)
      annhH(4,j) =  sa*(Z(j,1)*an1n4s2+Z(j,2)*an2n4s2)
      annhA(4,j) =  cbet*(Z(j,1)*an1n4p2+Z(j,2)*an2n4p2)
      annG0(4,j) =  sbet*(Z(j,1)*an1n4p2+Z(j,2)*an2n4p2)
c     
      bnnhl(1,j) = -sa*Z(j,3)*bn1n3s1+ca*Z(j,4)*bn1n4s2
      bnnhH(1,j) =  ca*Z(j,3)*bn1n3s1+sa*Z(j,4)*bn1n4s2
      bnnhA(1,j) = -sbet*Z(j,3)*bn1n3p1+cbet*Z(j,4)*bn1n4p2
      bnnG0(1,j) =  cbet*Z(j,3)*bn1n3p1+sbet*Z(j,4)*bn1n4p2
c
      bnnhl(2,j) = -sa*Z(j,3)*bn2n3s1+ca*Z(j,4)*bn2n4s2
      bnnhH(2,j) =  ca*Z(j,3)*bn2n3s1+sa*Z(j,4)*bn2n4s2
      bnnhA(2,j) = -sbet*Z(j,3)*bn2n3p1+cbet*Z(j,4)*bn2n4p2
      bnnG0(2,j) =  cbet*Z(j,3)*bn2n3p1+sbet*Z(j,4)*bn2n4p2
c     
      bnnhl(3,j) = -sa*(Z(j,1)*bn1n3s1+Z(j,2)*bn2n3s1)
      bnnhH(3,j) =  ca*(Z(j,1)*bn1n3s1+Z(j,2)*bn2n3s1)
      bnnhA(3,j) = -sbet*(Z(j,1)*bn1n3p1+Z(j,2)*bn2n3p1)
      bnnG0(3,j) =  cbet*(Z(j,1)*bn1n3p1+Z(j,2)*bn2n3p1)
c     
      bnnhl(4,j) =  ca*(Z(j,1)*bn1n4s2+Z(j,2)*bn2n4s2)
      bnnhH(4,j) =  sa*(Z(j,1)*bn1n4s2+Z(j,2)*bn2n4s2)
      bnnhA(4,j) =  cbet*(Z(j,1)*bn1n4p2+Z(j,2)*bn2n4p2)
      bnnG0(4,j) =  sbet*(Z(j,1)*bn1n4p2+Z(j,2)*bn2n4p2)
      enddo
c  neutralino-chargino-H+ :
      an1c2h1 = g1/sq2
      bn1c2h2 = an1c2h1
      an2c2h1 = g/sq2
      bn2c2h2 = an2c2h1
...
c
      do j=1,2
      ancHp(1,j) = -sbet*U(j,2)*an1c2h1
      ancGp(1,j) =  cbet*U(j,2)*an1c2h1
      ancHp(2,j) = -sbet*U(j,2)*an2c2h1
      ancGp(2,j) =  cbet*U(j,2)*an2c2h1
      ancHp(3,j) = -sbet*U(j,1)*an3c1h1
      ancGp(3,j) =  cbet*U(j,1)*an3c1h1
      ancHp(4,j) = 0d0
      ancGp(4,j) = 0d0
c
      bncHp(1,j) =  cbet*V(j,2)*bn1c2h2
      bncGp(1,j) =  sbet*V(j,2)*bn1c2h2
      bncHp(2,j) =  cbet*V(j,2)*bn2c2h2
      bncGp(2,j) =  sbet*V(j,2)*bn2c2h2
      bncHp(3,j) = 0d0
      bncGp(3,j) = 0d0
      bncHp(4,j) =  cbet*V(j,1)*bn4c1h2
      bncGp(4,j) =  sbet*V(j,1)*bn4c1h2
      enddo
c
      do i=1,4
      do j=1,4
      rcNnZL(i,j)= 0.d0  ! initialization for safety 
      rcNnZR(i,j)= 0.d0
      rcNnZS(i,j)= 0.d0
      rcNcwL(i,j)= 0.d0
      rcNcwR(i,j)= 0.d0
      ...
      rcncHpS(i,j)= 0d0
c  fermion/sfermion contributions
      rcNfsfL(i,j)= 3*((ct*anuL(i)+st*anuR(i))*(ct*anuL(j)+st*anuR(j))* ! top/stop
     . dble(SU_B1(q2,rmt,mst1))+
     .               (-st*anuL(i)+ct*anuR(i))*(-st*anuL(j)+ct*anuR(j))*   
     . dble(SU_B1(q2,rmt,mst2)))  +  
     .               6*anuR(i)*anuR(j)*    ! u,c/su,sc
     . dble(SU_B1(q2,zero,msu2)) +
c     .                annu(i)*annu(j)*     ! neut/sneut
c     . dble(SU_B1(q2,zero,msntau)) +
c     .               2*annu(i)*annu(j)*
c     . dble(SU_B1(q2,zero,msn1))    +
     .              3*((cb*andL(i)+sb*andR(i))*(cb*andL(j)+sb*andR(j))*  ! bottom/sbottom
     . dble(SU_B1(q2,rmb,msb1))+
     .               (-sb*andL(i)+cb*andR(i))*(-sb*andL(j)+cb*andR(j))*
     . dble(SU_B1(q2,rmb,msb2))) +  
     .               6*andR(i)*andR(j)*   ! d,s/sd,ss
     . dble(SU_B1(q2,zero,msd2)) +
     .             (cta*aneL(i)+sta*aneR(i))*(cta*aneL(j)+sta*aneR(j))* !tau/stau
     . dble(SU_B1(q2,rmtau,msta1))+
     .           (-sta*aneL(i)+cta*aneR(i))*(-sta*aneL(j)+cta*aneR(j))*
     . dble(SU_B1(q2,rmtau,msta2)) +
     .               2*aneR(i)*aneR(j)*      ! e,mu/se,smu
     . dble(SU_B1(q2,zero,mse2))    
c
      rcNfsfR(i,j)= 3*((ct*bnuL(i)+st*bnuR(i))*(ct*bnuL(j)+st*bnuR(j))* 
     . dble(SU_B1(q2,rmt,mst1))+
     .               (-st*bnuL(i)+ct*bnuR(i))*(-st*bnuL(j)+ct*bnuR(j))*   
     . dble(SU_B1(q2,rmt,mst2)))  +  
     .               6*bnuL(i)*bnuL(j)*   
     . dble(SU_B1(q2,zero,msu1)) +
     .                bnnu(i)*bnnu(j)*      ! neut/sneut
     . dble(SU_B1(q2,zero,msntau)) +
     .               2*bnnu(i)*bnnu(j)*
     . dble(SU_B1(q2,zero,msn1))    +
     .              3*((cb*bndL(i)+sb*bndR(i))*(cb*bndL(j)+sb*bndR(j))*  
     . dble(SU_B1(q2,rmb,msb1))+
     .               (-sb*bndL(i)+cb*bndR(i))*(-sb*bndL(j)+cb*bndR(j))*
     . dble(SU_B1(q2,rmb,msb2))) +  
     .               6*bndL(i)*bndL(j)*   
     . dble(SU_B1(q2,zero,msd1)) +
     .             (cta*bneL(i)+sta*bneR(i))*(cta*bneL(j)+sta*bneR(j))* 
     . dble(SU_B1(q2,rmtau,msta1))+
     .           (-sta*bneL(i)+cta*bneR(i))*(-sta*bneL(j)+cta*bneR(j))*
     . dble(SU_B1(q2,rmtau,msta2)) +
     .               2*bneL(i)*bneL(j)*     
     . dble(SU_B1(q2,zero,mse1))    
c S-contribution:
      rcNfsfS(i,j)= 3*((ct*bnuL(i)+st*bnuR(i))*(ct*anuL(j)+st*anuR(j))* 
     . rmt*dble(SU_B0(q2,rmt,mst1))+
     .               (-st*bnuL(i)+ct*bnuR(i))*(-st*anuL(j)+ct*anuR(j))*   
     . rmt*dble(SU_B0(q2,rmt,mst2)))    +
     .              3*((cb*bndL(i)+sb*bndR(i))*(cb*andL(j)+sb*andR(j))* 
     . rmb*dble(SU_B0(q2,rmb,msb1))+
     .               (-sb*bndL(i)+cb*bndR(i))*(-sb*andL(j)+cb*andR(j))*
     . rmb*dble(SU_B0(q2,rmb,msb2))) +
     .             (cta*bneL(i)+sta*bneR(i))*(cta*aneL(j)+sta*aneR(j))* 
     . rmtau*dble(SU_B0(q2,rmtau,msta1))+
     .           (-sta*bneL(i)+cta*bneR(i))*(-sta*aneL(j)+cta*aneR(j))*
     . rmtau*dble(SU_B0(q2,rmtau,msta2)) 
c ! nb first two gen. S ~ a.b contributions are zero since aL and bR cplings ~ 0
c (and similarly for neut/sneut contributions)
c   neutralino/H0 contributions
      do k=1,4
      rcnnHL(i,j) =rcnnHL(i,j) +
     .   (annhl(i,k)*annhl(j,k)*dble(SU_B1(q2,mn(k),ml)) +
     .    annhH(i,k)*annhH(j,k)*dble(SU_B1(q2,mn(k),mh)) +
     .    annhA(i,k)*annhA(j,k)*dble(SU_B1(q2,mn(k),mA)) +
     .    annG0(i,k)*annG0(j,k)*dble(SU_B1(q2,mn(k),mz)) )/2
c
      rcnnHR(i,j) =rcnnHR(i,j) + 
     .   (bnnhl(i,k)*bnnhl(j,k)*dble(SU_B1(q2,mn(k),ml)) +
     .    bnnhH(i,k)*bnnhH(j,k)*dble(SU_B1(q2,mn(k),mh)) +
     .    bnnhA(i,k)*bnnhA(j,k)*dble(SU_B1(q2,mn(k),mA)) +
     .    bnnG0(i,k)*bnnG0(j,k)*dble(SU_B1(q2,mn(k),mz)) )/2
c  S contributions
      rcnnHS(i,j) =rcnnHS(i,j) + 
     .    bnnhl(i,k)*annhl(j,k)*mn(k)*dble(SU_B0(q2,mn(k),ml)) +
     .    bnnhH(i,k)*annhH(j,k)*mn(k)*dble(SU_B0(q2,mn(k),mh)) +
     .    bnnhA(i,k)*annhA(j,k)*mn(k)*dble(SU_B0(q2,mn(k),mA)) +
     .    bnnG0(i,k)*annG0(j,k)*mn(k)*dble(SU_B0(q2,mn(k),mz)) 
      enddo 
c chargino/H+ contributions
      do k=1,2
      rcncHpL(i,j) =rcncHpL(i,j) +
     .  (ancHp(i,k)*ancHp(j,k)*dble(SU_B1(q2,mc(k),mch)) +
     .  ancGp(i,k)*ancGp(j,k)*dble(SU_B1(q2,mc(k),mw)) )
c
      rcncHpR(i,j) =rcncHpR(i,j) +
     .  (bncHp(i,k)*bncHp(j,k)*dble(SU_B1(q2,mc(k),mch)) +
     .  bncGp(i,k)*bncGp(j,k)*dble(SU_B1(q2,mc(k),mw)) )
c  S contributions
      rcncHpS(i,j) =rcncHpS(i,j) +
     .  bncHp(i,k)*ancHp(j,k)*mc(k)*dble(SU_B0(q2,mc(k),mch)) +
     .  bncGp(i,k)*ancGp(j,k)*mc(k)*dble(SU_B0(q2,mc(k),mw)) 
      enddo
c
      enddo
      enddo
c   neutralino-Z contributions
      do k=1,4
      rcNnZL(3,3) = rcNnZL(3,3) +(Z(k,3)*an3n3z)**2*
     . dble(SU_B1(q2,mn(k),mz)) 
      rcNnZL(3,4) = rcNnZL(3,4) +(Z(k,3)*an3n3z)*(Z(k,4)*an4n4z)*
     . dble(SU_B1(q2,mn(k),mz)) 
      rcNnZL(4,3)= rcNnZL(3,4)
      rcNnZL(4,4) = rcNnZL(4,4) +(Z(k,4)*an4n4z)**2*
     . dble(SU_B1(q2,mn(k),mz)) 
c
      rcNnZR(3,3) = rcNnZR(3,3) +(Z(k,3)*bn3n3z)**2*
     . dble(SU_B1(q2,mn(k),mz)) 
      rcNnZR(3,4) = rcNnZR(3,4) +(Z(k,3)*bn3n3z)*(Z(k,4)*bn4n4z)*
     . dble(SU_B1(q2,mn(k),mz)) 
      rcNnZR(4,3)= rcNnZR(3,4)
      rcNnZR(4,4) = rcNnZR(4,4) +(Z(k,4)*bn4n4z)**2*
     . dble(SU_B1(q2,mn(k),mz)) 
c S contributions
      rcNnZS(3,3) = rcNnZS(3,3) +(Z(k,3)*bn3n3z)*(Z(k,3)*an3n3z)*
     . mn(k)*dble(SU_B0(q2,mn(k),mz)) 
      rcNnZS(3,4) = rcNnZS(3,4) +(Z(k,3)*bn3n3z)*(Z(k,4)*an4n4z)*
     . mn(k)*dble(SU_B0(q2,mn(k),mz)) 
      rcNnZS(4,3)= rcNnZS(4,3)  +(Z(k,4)*bn4n4z)*(Z(k,3)*an3n3z)*
     . mn(k)*dble(SU_B0(q2,mn(k),mz)) 
      rcNnZS(4,4) = rcNnZS(4,4) +(Z(k,4)*bn4n4z)*(Z(k,4)*an4n4z)*
     . mn(k)*dble(SU_B0(q2,mn(k),mz)) 
      enddo
c  chargino-W contributions
      do k=1,2
      rcNcwL(2,2) = rcNcwL(2,2) +2*(V(k,1)*an2c1w)**2*
     . dble(SU_B1(q2,mc(k),mw)) 
      rcNcwL(4,4) = rcNcwL(4,4) +2*(V(k,2)*an4c2w)**2*
     . dble(SU_B1(q2,mc(k),mw)) 
      rcNcwL(2,4) = rcNcwL(2,4) +2*(V(k,1)*an2c1w)*(V(k,2)*an4c2w)*
     . dble(SU_B1(q2,mc(k),mw)) 
      rcNcwL(4,2) = rcNcwL(2,4)
c
      rcNcwR(2,2) = rcNcwR(2,2) +2*(U(k,1)*bn2c1w)**2*
     . dble(SU_B1(q2,mc(k),mw)) 
      rcNcwR(3,3) = rcNcwR(3,3) +2*(U(k,2)*bn3c2w)**2*
     . dble(SU_B1(q2,mc(k),mw)) 
      rcNcwR(2,3) = rcNcwR(2,3) +2*(U(k,1)*bn2c1w)*(U(k,2)*bn3c2w)*
     . dble(SU_B1(q2,mc(k),mw)) 
      rcNcwR(3,2) = rcNcwR(2,3)
c S contributions
      rcNcwS(2,2) = rcNcwS(2,2) +(U(k,1)*bn2c1w)*(V(k,1)*an2c1w)*
     . mc(k)*dble(SU_B0(q2,mc(k),mw)) 
      rcNcwS(3,2) = rcNcwS(3,2) +(U(k,2)*bn3c2w)*(V(k,1)*an2c1w)*
     . mc(k)*dble(SU_B0(q2,mc(k),mw)) 
      rcNcwS(2,4) = rcNcwS(2,4) +(U(k,1)*bn2c1w)*(V(k,2)*an4c2w)*
     . mc(k)*dble(SU_B0(q2,mc(k),mw)) 
      rcNcwS(3,4) = rcNcwS(3,4) +(U(k,2)*bn3c2w)*(V(k,2)*an4c2w)*
     . mc(k)*dble(SU_B0(q2,mc(k),mw)) 
c ! note the last S(i,j) are not symmetrical (only (2,2),(3,2),(2,4),(3,4) non-zero)
c but will be symmetrical in the final expression which is proportional to S +S^T 
      enddo
      do i=1,4
      do j=1,4
      rcL(i,j)=cpi*(rcNfsfL(i,j)+rcnnHL(i,j)+rcncHpL(i,j)+rcNnZL(i,j)+
     . rcNcwL(i,j))
      rcR(i,j)=cpi*(rcNfsfR(i,j)+rcnnHR(i,j)+rcncHpR(i,j)+rcNnZR(i,j)+
     . rcNcwR(i,j))
      rcS(i,j)=cpi*(2*rcNfsfS(i,j) -8*(rcNcwS(i,j)+rcNcwS(j,i))/2  ! trick to anticipate
     . -4*rcNnZS(i,j) +2*rcncHpS(i,j)+rcnnHS(i,j))                 ! resymmetrisation via S+S^T
      enddo                                                          
      enddo
...
c  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c  Calculates the sfermion masses and mixing angles including full one-loop 
c  radiative corrections a la PBMZ (Pierce, Bagger, Matchev, Zhang, hep-ph/9606211)
c  (NB MAJOR CHANGES done for ver >= 2.5)
...
c  The mixing angles (! changed to DRbar TREE-LEVEL ones for version >= 2.5) 
c  of the 3 generation sfermion are in the common block:
...
c  in a different (PBMZ) convention used in several other subroutines
...
c
       ...
       crLLsne=0.d0
c
c Soft and other terms used in sfermion matrix elements should be running masses 
c at EWSB scale, including SUSY radiative corrections
c
c first two generations:  no mixing included 
c up squarks: 
      do ifirst=0,2
c calculating rad. corr. with 2 different momenta scales (~2 different pole masses):
      if(isfrc.eq.1.and.irge.ge.2.and.ifirst.ge.1) then
      if(ifirst.eq.1) pscaleu= msu1bp
      if(ifirst.eq.2) pscaleu=msu2bp
      CALL SU_STOPCR(pscaleu,MU,M3,0d0,0d0,Au1,Ad1,                 
     . msu1bp,msu2bp,msd1bp,msd2bp,0d0,0d0,
     . crLLu,crLRdum,crRRu)
c !! nb first generation trilinear cplings Au1, Ad1 are irrelevant, being screened for first two
c generation by Yu,Yd=0 approximation. Note also that accordingly crLR=0 thus irrelevant
      endif
      mstl2=mql1**2+(0.5d0-2.d0/3*sw2)*mz**2*dcos(2*b) +crLLu
      mstr2=mur1**2+2.d0/3*sw2*mz**2*dcos(2*b)         +crRRu
      msu(1)=dsqrt(mstl2)
      msu(2)=dsqrt(mstr2)
      if(ifirst.eq.0) then
      msu1bp = msu(1)
      ...
      if(ifirst.eq.1) msu1true=msu(1)
      if(ifirst.eq.2) msu(1)=msu1true
      enddo
c
c down squarks
      do ifirst=0,2
c calculating rad. corr. with 2 different momenta scales (~2 different pole masses):
      if(isfrc.eq.1.and.irge.ge.2.and.ifirst.ge.1) then
      if(ifirst.eq.1) pscaled= msd1bp
      if(ifirst.eq.2) pscaled=msd2bp
      CALL SU_SBOTCR(pscaleu,MU,M3,0d0,0d0,Au1,Ad1,                 
     . msu1bp,msu2bp,msd1bp,msd2bp,0d0,0d0,
     . crLLd,crLRdum,crRRd)
      endif
      msbl2=mql1**2+(-0.5d0+1.d0/3*sw2)*mz**2*dcos(2*b) +crLLd
      msbr2=mdr1**2-1.d0/3*sw2*mz**2*dcos(2*b)          +crRRd
      msd(1)=dsqrt(msbl2)
      msd(2)=dsqrt(msbr2)
      if(ifirst.eq.0) then
      msd1bp = msd(1)
      ...
      if(ifirst.eq.1) msd1true=msd(1)
      if(ifirst.eq.2) msd(1)=msd1true
      enddo
c
c sleptons
      do ifirst=0,2
c calculating rad. corr. with 2 different momenta scales (~2 different pole masses):
      ...
      if(ifirst.eq.2) pscalee=mse2bp
      CALL SU_STAUCR(pscalee,MU, 0d0, Ae1,                 
     . mse1bp,mse2bp,msn1bp,0d0,
     . crLLe,crLRdum,crRRe)  
      if(ifirst.eq.1) CALL SU_SNUTAUCR(pscalesne,MU, 0d0, Ae,                 
     . mse1bp,mse2bp,msn1bp,0d0,
     . crLLsne)  
       endif
      msel2=mel1**2+(-0.5d0+sw2)*mz**2*dcos(2*b)  +crLLe
      mser2=mer1**2- sw2*mz**2*dcos(2*b)          +crRRe
      msnl2=mel1**2+0.5d0*mz**2*dcos(2*b)          +crLLsne
      mse(1)=dsqrt(msel2)
      mse(2)=dsqrt(mser2)
      ...
      if(ifirst.eq.1) mse1true=mse(1)
      if(ifirst.eq.2) mse(1)=mse1true
c
       enddo
c
...
c stop masses/mixing
      do ifirst=0,6
      istflip= 0
c calculating stop rad. corr. iterating twice with different momenta scales, 
c to reach close to the true pole masses:
      if(isfrc.eq.1.and.irge.ge.2.and.ifirst.ge.1) then
      if(ifirst.eq.1) pscalet= dmin1(mst1bp,mst2bp) ! in case not well-ordered
      if(ifirst.eq.2) pscalet= dmax1(mst1bp,mst2bp)
      if(ifirst.eq.3) pscalet= scale !  ESWB scale used for mixing angle
      if(ifirst.eq.4) pscalet= mst1true   ! 'pole' mass 1 from first iteration
      if(ifirst.eq.5) pscalet=mst2true    ! 'pole' mass 2 from first iteration
      if(ifirst.eq.6) pscalet= dsqrt(mst1true*mst2true)  !
c 
      CALL SU_STOPCR(pscalet,MU,M3, Ytewsb,Ybewsb,At,Ab,                 
     . mst1bp,mst2bp,msb1bp,msb2bp,thetbp,thebbp,
     . crLLt,crLRt,crRRt)
       endif
c
      mstl2=mql**2+(0.5d0-2.d0/3*sw2ew)*mz**2*dcos(2*b)  +crLLt
      mstr2=mur**2+2.d0/3*sw2ew*mz**2*dcos(2*b)          +crRRt
      mlrt=At-mu/tb                                          +crLRt/rmt
...
c  tachyonic sfermion 1 mass
      ...
      if(ifirst.eq.1.or.ifirst.eq.4) mst1true=mst(1)
      if(ifirst.eq.2.or.ifirst.eq.5) mst2true=mst(2)
      if(ifirst.eq.6) then
      mst(1)=mst1true   ! final pole masses and mixing angle
      mst(2)=mst2true
      thet=thettree     ! note new in v2.5: 'tree' DRbar angle (more consistent)
      endif
      ct=dcos(thet)
      st=dsin(thet)
...
c sbottom masses/mixing
      ...
      isbflip = 0
      if(isfrc.eq.1.and.irge.ge.2.and.ifirst.ge.1) then
c calculating sbottom rad. corr., iterating with 3 different momenta scales,
c to reach close to the true pole masses:
      if(ifirst.eq.1) pscaleb= dmin1(msb1bp,msb2bp) ! in case not well-ordered
      if(ifirst.eq.2) pscaleb=dmax1(msb1bp,msb2bp) 
      if(ifirst.eq.3) pscaleb= scale   ! EWSB scale used for mixing angle
      if(ifirst.eq.4) pscaleb= msb1true  ! 'pole' mass 1 from first iteration
      if(ifirst.eq.5) pscaleb=msb2true   ! 'pole' mass 2 from first iteration
      if(ifirst.eq.6) pscaleb= dsqrt(msb1true*msb2true) 
c 
      CALL SU_SBOTCR(pscaleb,MU,M3, Ytewsb,Ybewsb,At,Ab,                 
     . mst1bp,mst2bp,msb1bp,msb2bp,thetbp,thebbp,
     . crLLb,crLRb,crRRb)  
       endif     
      msbl2=mql**2+(-0.5d0+1.d0/3*sw2ew)*mz**2*dcos(2*b)  +crLLb
      msbr2=mdr**2-1.d0/3*sw2ew*mz**2*dcos(2*b)           +crRRb
      mlrb=Ab-mu*tb                                       +crLRb/rmb
...
c   tachyonic sfermion mass
      ...
      if(ifirst.eq.1.or.ifirst.eq.4) msb1true=msb(1)
      if(ifirst.eq.2.or.ifirst.eq.5) msb2true=msb(2)
      if(ifirst.eq.6) then
      msb(1)=msb1true   ! final pole masses
      msb(2)=msb2true
      theb=thebtree     ! note new in v2.5: 'tree' DRbar angle (more consistent)
      endif
      cb=dcos(theb)
      sb=dsin(theb)
...
c  stau masses/mixing
      do ifirst=0,6
      if(isfrc.eq.1.and.irge.ge.2.and.ifirst.ge.1) then
c calculating stau rad. corr with 3 different momenta scales (+ 2 iterations
c to be as close as possible to momenta = true pole masses):
      if(ifirst.eq.1) pscaletau=dmin1(msta1bp,msta2bp) ! in case not well-ordered
      if(ifirst.eq.2) pscaletau=dmax1(msta1bp,msta2bp)
      if(ifirst.eq.3) pscaletau= dmin1(msta1bp,msta2bp)
      if(ifirst.eq.4) pscaletau=msta1true ! 'pole' mass 1 from first iteration
      if(ifirst.eq.5) pscaletau=msta2true ! 'pole' mass 2 "    "     " 
      if(ifirst.eq.6) pscaletau= dsqrt(msta1true*msta2true) 
c 
      CALL SU_STAUCR(pscaletau,MU, Ytauewsb, Al,                 
     . msta1bp,msta2bp,msntaubp,thelbp,
     . crLLtau,crLRtau,crRRtau)  
      if(ifirst.eq.1.or.ifirst.eq.4) CALL SU_SNUTAUCR(msn(3),MU, 
     . Ytauewsb, Al,msta1bp,msta2bp,msntau,thel,crLLnutau)  
       endif
      msel2=mel**2+(-0.5d0+sw2ew)*mz**2*dcos(2*b) +crlltau
      mser2=mer**2- sw2ew*mz**2*dcos(2*b)        +crrrtau
      mlre=Al-mu*tb                              +crlrtau/rml
      msntau2=mel**2+0.5d0*mz**2*dcos(2*b)     +crLLnutau
...
c   tachyonic sfermion mass
      ...
      if(ifirst.eq.1.or.ifirst.eq.4) msta1true=msl(1)
      if(ifirst.eq.2.or.ifirst.eq.5) msta2true=msl(2)
      if(ifirst.eq.6) then
      msl(1)=msta1true  ! final pole masses
      msl(2)=msta2true
      thel=theltree     ! note new in v2.5: 'tree' DRbar angle (more consistent)
      endif
      cl=dcos(thel)
      sl=dsin(thel)
...
c  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c  Calculates running sfermion masses and mixing angles at scale=MZ 
c  in the BPMZ conventions.
c
      implicit real*8(a-h,k-z)
      logical su_isNaN
...
C
      ...
      ma2save=ma2
      if(ma2.lt.0.d0) ma2= .1d0   ! temporary protection 
      if(ma2save.lt.0.d0.and.irge.eq.irgmax) errma2z=-1.d0    !final ma2<0->error flag
      ma=dsqrt(dabs(ma2))
c       
      marunz = ma
      ...
      pi = 4*datan(1d0)
      alpharunz =0.5*atan(tan(2*b)*(marunz**2+mz2)
     .      /(marunz**2-mz2))
c  ! simpler way of accounting for correct alpha sign convention:
      if(cos(2*b)*(marunz**2-mz2).gt.0d0) alpharunz=alpharunz-pi/2
c
c first two generations:  no mixing included 
c up squarks: 
...
c  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c!! NB obsolete routine, no longer called in ver >= 2.5, but kept for possible crosschecks
c
c  Calculates the leading QCD (standard+SUSY) correction to squark masses
c  The input are: the strong coupling constant alphas, the gluino 
...
c
      ...
      COMMON/SU_renscale/scale
      COMMON/SU_tachyrc/tachsqrc
c       COMMON/SU_param/gf,alpha,mz,mw
      COMMON/SU_treesfer/msbtr1,msbtr2,msttr1,msttr2
      COMMON/SU_stepwi/wistep,h1,kpole
...
c
c ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        SUBROUTINE SU_STOPCR(pscale,mu,M3, Yu,Yd,Au,Ad,                 
     . msu1gen,msu2gen,msd1gen,msd2gen,thetu,thetd,
     . crLL,crLR,crRR)
c  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c  Calculates radiative corrections to the (generation-dependent!) Up squark masses, 
c  including full one-loop MSSM a la PBMZ. (new in v2.5)
c  The input at the EWSB scale are, respectively: pscale: self-energy ext momentum,
c  the mu parameter, the (running) gluino mass M3, Yu,Yd (generic) Yukawa cplings,
c  Au, Ad (generic) trilinear couplings, squark masses msu1gen...msd2gen, mixing angle thetu,thetd. 
c  Output: the radiative corrections to LL,LR,RR entries of up squark mass matrix. 
c---------------------------------------------------------
c
      implicit real*8(a-h,m,o-z)
      complex*16 SU_B0,SU_BG,SU_BF
      dimension gmc(2),xmctree(2),gmn(4),dxmn(4),u(2,2),v(2,2),z(4,4),
     ...
     .  fbtLL(2),gbtLL(2),fbtLR(2),gbtLR(2),fbtRR(2),gbtRR(2)   
      ...
      COMMON/SU_bpew/msu1,msu2,msd1,msd2,mse1,mse2,msn1,msntau,
     .      msta1,msta2,msb1,msb2,mst1,mst2,thet,theb,thel
      COMMON/SU_cte/xnf,cpi,mz_pole,mwdum,tbeta
      COMMON/SU_stepwi/wistep,h1,kpole
      COMMON/run_p/pizz        
c   fix ren. scale (used in B0 functions):
      if(kpole.eq.1) scale = dsqrt(mst1*mst2)     
c
      if(msu1gen.eq.0.d0.or.msd1gen.eq.0.d0) goto 100
c   (NB protection: means msu1gen or msd1gen undefined yet)        
       sq2=dsqrt(2.d0)
       pi = 4*datan(1.d0)
       g=g2ewsb
       g1=g1ewsb
       ...
       e=g1*cw
c
       vd2 = 2*(mz_pole**2+pizz)/(g1ewsb**2+g2ewsb**2)/(1.d0+tbeta**2)
       vu2 = vd2*tbeta**2
       vd= dsqrt(vd2)
       vu= dsqrt(vu2)
       mz= dsqrt(mz_pole**2+pizz)
...
c----------- Higgs couplings
c 
       s2tLtL = -g*mz/cw*(.5d0 -2*sw2/3)*sbet + sq2*yu*rmu
       s2tRtR = -g*mz/cw*(2*sw2/3)*sbet + sq2*yu*rmu
       s2tLtR = yu/sq2*Au
       s1tLtL = g*mz/cw*(.5d0 -2*sw2/3)*cbet
       s1tRtR = g*mz/cw*(2*sw2/3)*cbet
       s1tLtR = -yu/sq2*mu
c
       s2tLt1=cu*s2tLtL+su*s2tLtR
       s2tLt2=-su*s2tLtL+cu*s2tLtR
       s2tRt1=cu*s2tLtR+su*s2tRtR
       s2tRt2=-su*s2tLtR+cu*s2tRtR
       s1tLt1=cu*s1tLtL+su*s1tLtR
       s1tLt2=-su*s1tLtL+cu*s1tLtR
       s1tRt1=cu*s1tLtR+su*s1tRtR
       s1tRt2=-su*s1tLtR+cu*s1tRtR
c
...
c
       atLtR=-yu/sq2*(-mu*sbet-Au*cbet)
       gtLtR=+yu/sq2*(-mu*cbet+Au*sbet)
c
       gatLt1=su*atLtR
       gatLt2=cu*atLtR
       gatRt1=-cu*atLtR
...
c
       ...
       ggtRt2=su*gtLtR
c
       gctLbL = g*mw/sq2*dsin(2*b)-yu*rmu*cbet-yd*rmd*sbet
       gctRbR = -yu*rmd*cbet-yd*rmu*sbet
       gctLbR = yd*(-mu*cbet-Ad*sbet)
       gctRbL = yu*(-mu*sbet-Au*cbet)
c
       ggtLbL = -g*mw/sq2*dcos(2*b)-yu*rmu*sbet+yd*rmd*cbet
       ggtRbR = 0.d0
       ggtLbR = yd*(-mu*sbet+Ad*cbet)
       ggtRbL = -yu*(-mu*cbet+Au*sbet)
c
       gctLb1=cd*gctLbL+sd*gctLbR 
       gctLb2=-sd*gctLbL+cd*gctLbR
       gctRb1=cd*gctRbL+sd*gctRbR
       gctRb2=-sd*gctRbL+cd*gctRbR
c
       ggtLb1=cd*ggtLbL+sd*ggtLbR 
       ggtLb2=-sd*ggtLbL+cd*ggtLbR
       ggtRb1=cd*ggtRbL+sd*ggtRbR
       ggtRb2=-sd*ggtRbL+cd*ggtRbR
c----------- neutralino/chargino couplings:
c
       ap1tL = 0.d0
       bp1tL = g1/sq2*(1.d0/3.d0)
       ap1tR = g1/sq2*(-4.d0/3.d0)
       bp1tR = 0.d0
       ap2tL = 0.d0
       bp2tL = sq2*g*(.5d0)
       ap2tR = 0.d0
...
c       
c-------------------- LL contribution: 
       crLLqcd=16*pi*alphas/3*(2*dble(SU_BG(pscale**2,m3,rmu))
     . +cu**2*(dble(SU_BF(pscale**2,msu1gen,zero))+SU_A(msu1gen) )
     . +su**2*(dble(SU_BF(pscale**2,msu2gen,zero))+SU_A(msu2gen) ) )
c       
       crLLyuk=yu**2*(su**2*SU_A(msu1gen)+cu**2*SU_A(msu2gen) )
     .        +yd**2*(sd**2*SU_A(msd1gen)+cd**2*SU_A(msd2gen) )     
     . +0.5d0*(yu**2*sal**2-g**2*(.5d0-2*sw2/3)/2.d0/cw**2*
     .        (-dcos(2*alfa)) )*SU_A(mh) 
     . +0.5d0*(yu**2*cal**2-g**2*(.5d0-2*sw2/3)/2.d0/cw**2*
     .        (+dcos(2*alfa)) )*SU_A(ml) 
     . +0.5d0*(yu**2*sbet**2-g**2*(.5d0-2*sw2/3)/2.d0/cw**2*
     .        (-dcos(2*b))    )*SU_A(mz) 
     . +0.5d0*(yu**2*cbet**2-g**2*(.5d0-2*sw2/3)/2.d0/cw**2*
     .        (dcos(2*b))     )*SU_A(ma) 
     . +(yd**2*sbet**2+g**2*( (.5d0-2*sw2/3)/2.d0/cw**2-.5d0)*
     .        (-dcos(2*b)) )*SU_A(mch)
     . +(yd**2*cbet**2+g**2*( (.5d0-2*sw2/3)/2.d0/cw**2-.5d0)*
     .        (dcos(2*b)) )*SU_A(mw)
     . +ghtLt1**2*dble(SU_B0(pscale**2,mh,msu1gen))
     . +ghtLt2**2*dble(SU_B0(pscale**2,mh,msu2gen))
     . +gltLt1**2*dble(SU_B0(pscale**2,ml,msu1gen))
     . +gltLt2**2*dble(SU_B0(pscale**2,ml,msu2gen))
     . +ggtLt1**2*dble(SU_B0(pscale**2,mz,msu1gen))
     . +ggtLt2**2*dble(SU_B0(pscale**2,mz,msu2gen))
     . +gatLt1**2*dble(SU_B0(pscale**2,ma,msu1gen))
     . +gatLt2**2*dble(SU_B0(pscale**2,ma,msu2gen))          
     . +gctLb1**2*dble(SU_B0(pscale**2,mch,msd1gen))
     . +gctLb2**2*dble(SU_B0(pscale**2,mch,msd2gen))
     . +ggtLb1**2*dble(SU_B0(pscale**2,mw,msd1gen))
     . +ggtLb2**2*dble(SU_B0(pscale**2,mw,msd2gen))
c
       cyu= yu**2*(su**2*SU_A(msu1gen)+cu**2*SU_A(msu2gen) )
       cyd=     yd**2*(sd**2*SU_A(msd1gen)+cd**2*SU_A(msd2gen) )     
       cyn = 0.5d0*(yu**2*sal**2-g**2*(.5d0-2*sw2/3)/2.d0/cw**2*
     .        (-dcos(2*alfa)) )*SU_A(mh) 
     . +0.5d0*(yu**2*cal**2-g**2*(.5d0-2*sw2/3)/2.d0/cw**2*
     .        (+dcos(2*alfa)) )*SU_A(ml) 
     . +0.5d0*(yu**2*sbet**2-g**2*(.5d0-2*sw2/3)/2.d0/cw**2*
     ...
     . +0.5d0*(yu**2*cbet**2-g**2*(.5d0-2*sw2/3)/2.d0/cw**2*
     .        (dcos(2*b))     )*SU_A(ma) 
      cyc= (yd**2*sbet**2+g**2*( (.5d0-2*sw2/3)/2.d0/cw**2-.5d0)*
     .        (-dcos(2*b)) )*SU_A(mch)
      cyw= (yd**2*cbet**2+g**2*( (.5d0-2*sw2/3)/2.d0/cw**2-.5d0)*
     .        (dcos(2*b)) )*SU_A(mw)
      cys= ghtLt1**2*dble(SU_B0(pscale**2,mh,msu1gen))
     . +ghtLt2**2*dble(SU_B0(pscale**2,mh,msu2gen))
     . +gltLt1**2*dble(SU_B0(pscale**2,ml,msu1gen))
     . +gltLt2**2*dble(SU_B0(pscale**2,ml,msu2gen))
     . +ggtLt1**2*dble(SU_B0(pscale**2,mz,msu1gen))
...
c
     ...
     .  +(2*g1*cw/3)**2*(cu**2*dble(SU_BF(pscale**2,msu1gen,zero))
     .                  +su**2*dble(SU_BF(pscale**2,msu2gen,zero)) )
     .  +g**2/cw**2*(.5d0-2*sw2/3)**2*(
     .                   cu**2*dble(SU_BF(pscale**2,msu1gen,mz))
     .                  +su**2*dble(SU_BF(pscale**2,msu2gen,mz)) )    
     .  +g**2/2*( cd**2*dble(SU_BF(pscale**2,msd1gen,mw))          !bug corrected v2.44 jlk
     .         +sd**2*dble(SU_BF(pscale**2,msd2gen,mw)) )    
     .  +g**2/4*(cu**2*SU_A(msu1gen)+su**2*SU_A(msu2gen) 
     .       +2*(cd**2*SU_A(msd1gen)+sd**2*SU_A(msd2gen) ) )       
c
      crLLhyp=g**2*0.5d0*(
     ...
     .  +g1**2/4*(1.d0/3)**2*(cu**2*SU_A(msu1gen)+su**2*SU_A(msu2gen) ) 
     .  +g1**2/4*(1.d0/3.d0)*(
...
c     
      crLLnino=0.d0    
      do i=1,4
      crLLnino=crLLnino+ fttLL(i)*dble(SU_BG(pscale**2,gmn(i),rmu))
     .    -2.d0*rmu*dxmn(i)*gttLL(i)*dble(SU_B0(pscale**2,gmn(i),rmu))
      enddo
c
      crLLcino=0.d0
      do i=1,2
      crLLcino=crLLcino+ fbtLL(i)*dble(SU_BG(pscale**2,gmc(i),rmd))
     .    -2.d0*rmd*gmc(i)*gbtLL(i)*dble(SU_B0(pscale**2,gmc(i),rmd)) !bug corrected v2.44 jlk     
      enddo 
c
...
c-------------------- RR contribution: 
c
       crRRqcd=16*pi*alphas/3*(2*dble(SU_BG(pscale**2,m3,rmu))
     . +su**2*(dble(SU_BF(pscale**2,msu1gen,zero))+SU_A(msu1gen) )
     . +cu**2*(dble(SU_BF(pscale**2,msu2gen,zero))+SU_A(msu2gen) ) )
c       
       crRRyuk=yu**2*(cu**2*SU_A(msu1gen)+su**2*SU_A(msu2gen) )
     .        +yu**2*(cd**2*SU_A(msd1gen)+sd**2*SU_A(msd2gen) )     
     . +0.5d0*(yu**2*sal**2-g**2*(2*sw2/3)/2.d0/cw**2*
     .        (-dcos(2*alfa)) )*SU_A(mh) 
     . +0.5d0*(yu**2*cal**2-g**2*(2*sw2/3)/2.d0/cw**2*
     .        (+dcos(2*alfa)) )*SU_A(ml) 
     . +0.5d0*(yu**2*sbet**2-g**2*(2*sw2/3)/2.d0/cw**2*
     .        (-dcos(2*b))    )*SU_A(mz) 
     . +0.5d0*(yu**2*cbet**2-g**2*(2*sw2/3)/2.d0/cw**2*
     .        (dcos(2*b))     )*SU_A(ma) 
     . +(yu**2*cbet**2+g**2*( (2*sw2/3)/2.d0/cw**2)*
     .        (-dcos(2*b)) )*SU_A(mch)
     . +(yu**2*sbet**2+g**2*( (2*sw2/3)/2.d0/cw**2)*
     .        (dcos(2*b)) )*SU_A(mw)
     . +ghtRt1**2*dble(SU_B0(pscale**2,mh,msu1gen))
     . +ghtRt2**2*dble(SU_B0(pscale**2,mh,msu2gen))
     . +gltRt1**2*dble(SU_B0(pscale**2,ml,msu1gen))
     . +gltRt2**2*dble(SU_B0(pscale**2,ml,msu2gen))
     . +ggtRt1**2*dble(SU_B0(pscale**2,mz,msu1gen))
     . +ggtRt2**2*dble(SU_B0(pscale**2,mz,msu2gen))
     . +gatRt1**2*dble(SU_B0(pscale**2,ma,msu1gen))
     . +gatRt2**2*dble(SU_B0(pscale**2,ma,msu2gen))          
     . +gctRb1**2*dble(SU_B0(pscale**2,mch,msd1gen))
     . +gctRb2**2*dble(SU_B0(pscale**2,mch,msd2gen))
     . +ggtRb1**2*dble(SU_B0(pscale**2,mw,msd1gen))
     . +ggtRb2**2*dble(SU_B0(pscale**2,mw,msd2gen))
c
      crRRgau=4*g**2/cw**2*(2*sw2/3)**2*SU_A(mz)
     .  +(2*g1*cw/3)**2*(su**2*dble(SU_BF(pscale**2,msu1gen,zero))
     .                  +cu**2*dble(SU_BF(pscale**2,msu2gen,zero)) )
     .  +g**2/cw**2*(2*sw2/3)**2*(
     .                   su**2*dble(SU_BF(pscale**2,msu1gen,mz))
     .                  +cu**2*dble(SU_BF(pscale**2,msu2gen,mz)) )    
c
      crRRhyp=
     .   g1**2/4*(-4.d0/3)**2*(su**2*SU_A(msu1gen)+cu**2*SU_A(msu2gen) ) 
     .  +g1**2/4*(-4.d0/3.d0)*(
     .        3.d0*(1.d0/3.d0)*(ct**2*SU_A(mst1)+st**2*SU_A(mst2) )    
     .       +3.d0*(1.d0/3.d0)*(cb**2*SU_A(msb1)+sb**2*SU_A(msb2) )    
     .       +(-1.d0)*(cta**2*SU_A(msta1)+sta**2*SU_A(msta2) )    
     ...
     .       +2.d0*(-1.d0)*SU_A(mse1)
     .       +2.d0*(-1.d0)*SU_A(msn1) +(-1.d0)*SU_A(msntau) )
     .  +g1**2/4*(-4.d0/3.d0)*(                                    !bug corrected v2.44 jlk
     .        3.d0*(-4.d0/3.d0)*(st**2*SU_A(mst1)+ct**2*SU_A(mst2) )    
     .       +3.d0*(2.d0/3.d0)*(sb**2*SU_A(msb1)+cb**2*SU_A(msb2) )    
     .       +(2.d0)*(sta**2*SU_A(msta1)+cta**2*SU_A(msta2) )    
     .       +6.d0*(-4.d0/3.d0)*SU_A(msu2)+6.d0*(2.d0/3.d0)*SU_A(msd2)
...
c     
      crRRnino=0.d0    
      do i=1,4
      crRRnino=crRRnino+ fttRR(i)*dble(SU_BG(pscale**2,gmn(i),rmu))
     .    -2.d0*rmu*dxmn(i)*gttRR(i)*dble(SU_B0(pscale**2,gmn(i),rmu))
      enddo
c
      crRRcino=0.d0
      do i=1,2
      crRRcino=crRRcino+ fbtRR(i)*dble(SU_BG(pscale**2,gmc(i),rmd))
     .    -2.d0*rmd*gmc(i)*gbtRR(i)*dble(SU_B0(pscale**2,gmc(i),rmd)) !bug corrected v2.44 jlk   
      enddo 
c
...
c
     ...
     . 4*rmu*m3*dble(SU_B0(pscale**2,m3,rmu))
     . +cu*su*(dble(SU_BF(pscale**2,msu1gen,zero))-SU_A(msu1gen) 
     .        -dble(SU_BF(pscale**2,msu2gen,zero))+SU_A(msu2gen) ) )
c       
       crLRyuk=3*yu**2*su*cu*(SU_A(msu1gen)-SU_A(msu2gen) )
     . +ghtLt1*ghtRt1*dble(SU_B0(pscale**2,mh,msu1gen))
     . +ghtLt2*ghtRt2*dble(SU_B0(pscale**2,mh,msu2gen))
     . +gltLt1*gltRt1*dble(SU_B0(pscale**2,ml,msu1gen))
     . +gltLt2*gltRt2*dble(SU_B0(pscale**2,ml,msu2gen))
     . +ggtLt1*ggtRt1*dble(SU_B0(pscale**2,mz,msu1gen))
     . +ggtLt2*ggtRt2*dble(SU_B0(pscale**2,mz,msu2gen))
     . +gatLt1*gatRt1*dble(SU_B0(pscale**2,ma,msu1gen))
     . +gatLt2*gatRt2*dble(SU_B0(pscale**2,ma,msu2gen))          
     . +gctLb1*gctRb1*dble(SU_B0(pscale**2,mch,msd1gen))
     . +gctLb2*gctRb2*dble(SU_B0(pscale**2,mch,msd2gen))
     . +ggtLb1*ggtRb1*dble(SU_B0(pscale**2,mw,msd1gen))
     . +ggtLb2*ggtRb2*dble(SU_B0(pscale**2,mw,msd2gen))
c
      crLRgau=(2*g1*cw/3)**2*cu*su*(dble(SU_BF(pscale**2,msu1gen,zero))
     .                          -dble(SU_BF(pscale**2,msu2gen,zero)) )
     .  -g**2/cw**2*(.5d0-2*sw2/3)*(2*sw2/3)*su*cu*(
     .                           dble(SU_BF(pscale**2,msu1gen,mz))
     .                          -dble(SU_BF(pscale**2,msu2gen,mz)) )    
c
      crLRhyp=g1**2/4*(1.d0/3.d0)*(-4.d0/3.d0)*su*cu*(
     .                SU_A(msu1gen)-SU_A(msu2gen) ) 
c     
      crLRnino=0.d0    
      do i=1,4
      crLRnino=crLRnino+ fttLR(i)*dble(SU_BG(pscale**2,gmn(i),rmu))
     .    -2.d0*rmu*dxmn(i)*gttLR(i)*dble(SU_B0(pscale**2,gmn(i),rmu))
      enddo
c
      crLRcino=0.d0
      do i=1,2
      crLRcino=crLRcino+ fbtLR(i)*dble(SU_BG(pscale**2,gmc(i),rmd))
     .    -2.d0*rmd*gmc(i)*gbtLR(i)*dble(SU_B0(pscale**2,gmc(i),rmd))  !bug corrected v2.44 jlk    
      enddo 
c
...
c
c++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        SUBROUTINE SU_SBOTCR(pscale,mu,M3, Yu,Yd,Au,Ad,                 
     . msu1gen,msu2gen,msd1gen,msd2gen,thetu,thetd,
     . crLL,crLR,crRR)  
c  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c  Calculates full one-loop radiative corrections (generically) to generic down squarks 
c masses, similarly to stops in routine SU_STOPCR, a la PBMZ. (new in v2.5)
c  The input at the EWSB scale are, respectively: pscale: self-energy ext momentum,
c  the mu parameter, the (running) gluino mass M3, Yu,Yd (generic) Yukawa cplings,
c  Au, Ad (generic) trilinear couplings, squark masses msu1gen...msd2gen, mixing angle thetu,thetd. 
c  Output: the radiative corrections to LL,LR,RR entries of Down squark mass matrix. 
c---------------------------------------------------------
c
      implicit real*8(a-h,m,o-z)
      complex*16 SU_B0,SU_BG,SU_BF
      dimension gmc(2),xmctree(2),gmn(4),dxmn(4),u(2,2),v(2,2),z(4,4),
     .  anbR(4),anbL(4),bnbL(4),bnbR(4),acbL(2),acbR(2),bcbL(2),bcbR(2),
     .  fbbLL(4),gbbLL(4),fbbLR(4),gbbLR(4),fbbRR(4),gbbRR(4), 
     .  ftbLL(2),gtbLL(2),ftbLR(2),gtbLR(2),ftbRR(2),gtbRR(2)   
      common/SU_runhiggsewsb/ma,ml,mh,mch,alfa
      COMMON/SU_outginos/mc1,mc2,mn1,mn2,mn3,mn4,mgluino
      COMMON/SU_inoDR/U,V,Z,xmctree,dxmn
      COMMON/SU_yukaewsb/ytau,yb,yt,alsewsb,g2ewsb,g1ewsb
...
c----------- Higgs couplings
c 
       s2bLbL = -g*mz/cw*(-.5d0 +sw2/3)*cbet + sq2*yd*rmd
       s2bRbR = -g*mz/cw*(-sw2/3)*cbet + sq2*yd*rmd
       s2bLbR = yd/sq2*Ad
       s1bLbL = g*mz/cw*(-.5d0 +sw2/3)*sbet
       s1bRbR = g*mz/cw*(-sw2/3)*sbet
       s1bLbR = -yd/sq2*mu
c
       s2bLb1=cd*s2bLbL+sd*s2bLbR
       s2bLb2=-sd*s2bLbL+cd*s2bLbR
       s2bRb1=cd*s2bLbR+sd*s2bRbR
       s2bRb2=-sd*s2bLbR+cd*s2bRbR
       s1bLb1=cd*s1bLbL+sd*s1bLbR
       s1bLb2=-sd*s1bLbL+cd*s1bLbR
       s1bRb1=cd*s1bLbR+sd*s1bRbR
       s1bRb2=-sd*s1bLbR+cd*s1bRbR
c
       ghbLb1= cal*s1bLb1+sal*s2bLb1
       glbLb1=-sal*s1bLb1+cal*s2bLb1
       ghbLb2= cal*s1bLb2+sal*s2bLb2
       glbLb2=-sal*s1bLb2+cal*s2bLb2
       ghbRb1= cal*s1bRb1+sal*s2bRb1
       glbRb1=-sal*s1bRb1+cal*s2bRb1
       ghbRb2= cal*s1bRb2+sal*s2bRb2
       glbRb2=-sal*s1bRb2+cal*s2bRb2
                    
c
       abLbR=-yd/sq2*(-mu*cbet-Ad*sbet)
       gbLbR=+yd/sq2*(-mu*sbet+Ad*cbet)
c
       gabLb1=sd*abLbR
       gabLb2=cd*abLbR
       gabRb1=-cd*abLbR
...
c
       ...
       ggbRb2=sd*gbLbR
c
       gcbLtL = g*mw/sq2*dsin(2*b)-yd*rmd*sbet-yu*rmu*cbet
       gcbRtR = -yd*rmu*sbet-yu*rmd*cbet
       gcbLtR = yu*(-mu*sbet-Au*cbet)
       gcbRtL = yd*(-mu*cbet-Ad*sbet)
c
       ggbLtL = -g*mw/sq2*dcos(2*b)+yd*rmd*cbet-yu*rmu*sbet 
       ggbRtR = 0.d0
       ggbLtR = -yu*(-mu*cbet+Au*sbet)
       ggbRtL = yd*(-mu*sbet+Ad*cbet)
c
       gcbLt1=cu*gcbLtL+su*gcbLtR 
       gcbLt2=-su*gcbLtL+cu*gcbLtR
       gcbRt1=cu*gcbRtL+su*gcbRtR
       gcbRt2=-su*gcbRtL+cu*gcbRtR
c
       ggbLt1=cu*ggbLtL+su*ggbLtR 
       ggbLt2=-su*ggbLtL+cu*ggbLtR
       ggbRt1=cu*ggbRtL+su*ggbRtR
       ggbRt2=-su*ggbRtL+cu*ggbRtR
c----------- neutralino/chargino couplings:
c
       ap1bL = 0.d0                 
       bp1bL = g1/sq2*(1.d0/3)
...
c
       ...
       do i=1,4
       aNbR(i) = Z(i,1)*ap1bR +Z(i,2)*ap2bR +Z(i,3)*ap3bR +Z(i,4)*ap4bR	  
       bNbR(i) = Z(i,1)*bp1bR +Z(i,2)*bp2bR +Z(i,3)*bp3bR +Z(i,4)*bp4bR	  
       aNbL(i) = Z(i,1)*ap1bL +Z(i,2)*ap2bL +Z(i,3)*ap3bL +Z(i,4)*ap4bL	  
       bNbL(i) = Z(i,1)*bp1bL +Z(i,2)*bp2bL +Z(i,3)*bp3bL +Z(i,4)*bp4bL	  
       enddo
c
       do i=1,2
       aCbR(i) = V(i,1)*aw1bR +V(i,2)*aw2bR   
       bCbR(i) = U(i,1)*bw1bR +U(i,2)*bw2bR   
       aCbL(i) = V(i,1)*aw1bL +V(i,2)*aw2bL  
       bCbL(i) = U(i,1)*bw1bL +U(i,2)*bw2bL  
       enddo
c
       do i=1,4
       fbbLL(i) = aNbL(i)*aNbL(i) + bNbL(i)*bNbL(i) 
       gbbLL(i) = bNbL(i)*aNbL(i) + aNbL(i)*bNbL(i) 
       fbbLR(i) = aNbL(i)*aNbR(i) + bNbL(i)*bNbR(i) 
       gbbLR(i) = bNbL(i)*aNbR(i) + aNbL(i)*bNbR(i) 
       fbbRR(i) = aNbR(i)*aNbR(i) + bNbR(i)*bNbR(i) 
       gbbRR(i) = bNbR(i)*aNbR(i) + aNbR(i)*bNbR(i) 
       enddo      
c 
       do i=1,2
       ftbLL(i) = aCbL(i)*aCbL(i) + bCbL(i)*bCbL(i) 
       gtbLL(i) = bCbL(i)*aCbL(i) + aCbL(i)*bCbL(i) 
       ftbLR(i) = aCbL(i)*aCbR(i) + bCbL(i)*bCbR(i) 
       gtbLR(i) = bCbL(i)*aCbR(i) + aCbL(i)*bCbR(i) 
       ftbRR(i) = aCbR(i)*aCbR(i) + bCbR(i)*bCbR(i) 
       gtbRR(i) = bCbR(i)*aCbR(i) + aCbR(i)*bCbR(i) 
       enddo      
c
c-------------------- LL contribution: 
       crLLqcd=16*pi*alphas/3*(2*dble(SU_BG(pscale**2,m3,rmd))
     . +cd**2*(dble(SU_BF(pscale**2,msd1gen,zero))+SU_A(msd1gen) )
     . +sd**2*(dble(SU_BF(pscale**2,msd2gen,zero))+SU_A(msd2gen) ) )
c       
       crLLyuk=yd**2*(sd**2*SU_A(msd1gen)+cd**2*SU_A(msd2gen) )
     .        +yu**2*(su**2*SU_A(msu1gen)+cu**2*SU_A(msu2gen) )     
     . +0.5d0*(yd**2*sal**2-g**2*(-.5d0 +sw2/3)/2.d0/cw**2*
     .        (-dcos(2*alfa)) )*SU_A(mh) 
     . +0.5d0*(yd**2*cal**2-g**2*(-.5d0 +sw2/3)/2.d0/cw**2*
     .        (dcos(2*alfa)) )*SU_A(ml) 
     . +0.5d0*(yd**2*cbet**2-g**2*(-.5d0 +sw2/3)/2.d0/cw**2*
     .        (dcos(2*b))    )*SU_A(mz) 
     . +0.5d0*(yd**2*sbet**2-g**2*(-.5d0 +sw2/3)/2.d0/cw**2*
     .        (-dcos(2*b))     )*SU_A(ma) 
     . +(yu**2*cbet**2+g**2*( (-.5d0 +sw2/3)/2.d0/cw**2+.5d0)*
     .        (dcos(2*b)) )*SU_A(mch)
     . +(yu**2*sbet**2+g**2*( (-.5d0 +sw2/3)/2.d0/cw**2+.5d0)*
     .        (-dcos(2*b)) )*SU_A(mw)
     . +ghbLb1**2*dble(SU_B0(pscale**2,mh,msd1gen))
     . +ghbLb2**2*dble(SU_B0(pscale**2,mh,msd2gen))
     . +glbLb1**2*dble(SU_B0(pscale**2,ml,msd1gen))
     . +glbLb2**2*dble(SU_B0(pscale**2,ml,msd2gen))
     . +ggbLb1**2*dble(SU_B0(pscale**2,mz,msd1gen))
     . +ggbLb2**2*dble(SU_B0(pscale**2,mz,msd2gen))
     . +gabLb1**2*dble(SU_B0(pscale**2,ma,msd1gen))
     . +gabLb2**2*dble(SU_B0(pscale**2,ma,msd2gen))          
     . +gcbLt1**2*dble(SU_B0(pscale**2,mch,msu1gen))
     . +gcbLt2**2*dble(SU_B0(pscale**2,mch,msu2gen))
     . +ggbLt1**2*dble(SU_B0(pscale**2,mw,msu1gen))
     . +ggbLt2**2*dble(SU_B0(pscale**2,mw,msu2gen))
c
      crLLgau=4*g**2/cw**2*(-.5d0 +sw2/3)**2*SU_A(mz)+2*g**2*SU_A(mw)
     .  +(-g1*cw/3)**2*(cd**2*dble(SU_BF(pscale**2,msd1gen,zero))
     .                  +sd**2*dble(SU_BF(pscale**2,msd2gen,zero)) )
     .  +g**2/cw**2*(-.5d0 +sw2/3)**2*(
     .                   cd**2*dble(SU_BF(pscale**2,msd1gen,mz))
     .                  +sd**2*dble(SU_BF(pscale**2,msd2gen,mz)) )    
     .  +g**2/2*( cu**2*dble(SU_BF(pscale**2,msu1gen,mw))
     .         +su**2*dble(SU_BF(pscale**2,msu2gen,mw)) )    
     .  +g**2/4*(cd**2*SU_A(msd1gen)+sd**2*SU_A(msd2gen) 
     .       +2*(cu**2*SU_A(msu1gen)+su**2*SU_A(msu2gen) ) )       
c
      crLLhyp=g**2*(-0.5d0)*(
     .        3.d0*(+.5d0)*(ct**2*SU_A(mst1)+st**2*SU_A(mst2) )    
     .       +3.d0*(-.5d0)*(cb**2*SU_A(msb1)+sb**2*SU_A(msb2) )    
     ...
     .       +2.d0*(-.5d0)*SU_A(mse1)
     .       +2.d0*(.5d0)*SU_A(msn1) +(.5d0)*SU_A(msntau) )
     .  +g1**2/4*(1.d0/3)**2*(cd**2*SU_A(msd1gen)+sd**2*SU_A(msd2gen) ) 
     .  +g1**2/4*(1.d0/3.d0)*(
     .        3.d0*(1.d0/3.d0)*(ct**2*SU_A(mst1)+st**2*SU_A(mst2) )    
     .       +3.d0*(1.d0/3.d0)*(cb**2*SU_A(msb1)+sb**2*SU_A(msb2) )    
     .       +(-1.d0)*(cta**2*SU_A(msta1)+sta**2*SU_A(msta2) )    
...
c     
      crLLnino=0.d0    
      do i=1,4
      crLLnino=crLLnino+ fbbLL(i)*dble(SU_BG(pscale**2,gmn(i),rmd))
     .    -2.d0*rmd*dxmn(i)*gbbLL(i)*dble(SU_B0(pscale**2,gmn(i),rmd))
      enddo
c
      crLLcino=0.d0
      do i=1,2
      crLLcino=crLLcino+ ftbLL(i)*dble(SU_BG(pscale**2,gmc(i),rmu))
     .    -2.d0*rmu*gmc(i)*gtbLL(i)*dble(SU_B0(pscale**2,gmc(i),rmu))     
      enddo 
cc
      crLL=-cpi*(crLLqcd+crLLyuk+crLLgau+crLLhyp+crLLnino+crLLcino)
c       
c-------------------- RR contribution: 
c
       crRRqcd=16*pi*alphas/3*(2*dble(SU_BG(pscale**2,m3,rmd))
     . +sd**2*(dble(SU_BF(pscale**2,msd1gen,zero))+SU_A(msd1gen) )
     . +cd**2*(dble(SU_BF(pscale**2,msd2gen,zero))+SU_A(msd2gen) ) )
c       
       crRRyuk=yd**2*(cd**2*SU_A(msd1gen)+sd**2*SU_A(msd2gen) )
     .        +yd**2*(cu**2*SU_A(msu1gen)+su**2*SU_A(msu2gen) )     
     . +0.5d0*(yd**2*sal**2-g**2*(-sw2/3)/2.d0/cw**2*
     .        (-dcos(2*alfa)) )*SU_A(mh) 
     . +0.5d0*(yd**2*cal**2-g**2*(-sw2/3)/2.d0/cw**2*
     .        (dcos(2*alfa)) )*SU_A(ml) 
     . +0.5d0*(yd**2*cbet**2-g**2*(-sw2/3)/2.d0/cw**2*
     .        (dcos(2*b))    )*SU_A(mz) 
     . +0.5d0*(yd**2*sbet**2-g**2*(-sw2/3)/2.d0/cw**2*
     .        (-dcos(2*b))     )*SU_A(ma) 
     . +(yd**2*sbet**2+g**2*( (-sw2/3)/2.d0/cw**2)*
     .        (dcos(2*b)) )*SU_A(mch)
     . +(yd**2*cbet**2+g**2*( (-sw2/3)/2.d0/cw**2)*
     .        (-dcos(2*b)) )*SU_A(mw)
     . +ghbRb1**2*dble(SU_B0(pscale**2,mh,msd1gen))
     . +ghbRb2**2*dble(SU_B0(pscale**2,mh,msd2gen))
     . +glbRb1**2*dble(SU_B0(pscale**2,ml,msd1gen))
     . +glbRb2**2*dble(SU_B0(pscale**2,ml,msd2gen))
     . +ggbRb1**2*dble(SU_B0(pscale**2,mz,msd1gen))
     . +ggbRb2**2*dble(SU_B0(pscale**2,mz,msd2gen))
     . +gabRb1**2*dble(SU_B0(pscale**2,ma,msd1gen))
     . +gabRb2**2*dble(SU_B0(pscale**2,ma,msd2gen))          
     . +gcbRt1**2*dble(SU_B0(pscale**2,mch,msu1gen))
     . +gcbRt2**2*dble(SU_B0(pscale**2,mch,msu2gen))
     . +ggbRt1**2*dble(SU_B0(pscale**2,mw,msu1gen))
     . +ggbRt2**2*dble(SU_B0(pscale**2,mw,msu2gen))
c
      crRRgau=4*g**2/cw**2*(-sw2/3)**2*SU_A(mz)
     .  +(-g1*cw/3)**2*(sd**2*dble(SU_BF(pscale**2,msd1gen,zero))
     .                  +cd**2*dble(SU_BF(pscale**2,msd2gen,zero)) )
     .  +g**2/cw**2*(-sw2/3)**2*(
     .                   sd**2*dble(SU_BF(pscale**2,msd1gen,mz))
     .                  +cd**2*dble(SU_BF(pscale**2,msd2gen,mz)) )    
c
      crRRhyp=
     .   g1**2/4*(2.d0/3)**2*(sd**2*SU_A(msd1gen)+cd**2*SU_A(msd2gen) ) 
     .  +g1**2/4*(2.d0/3)*(
     .        3*(1.d0/3)*(ct**2*SU_A(mst1)+st**2*SU_A(mst2) )    
     .       +3*(1.d0/3)*(cb**2*SU_A(msb1)+sb**2*SU_A(msb2) )    
     .       +(-1.d0)*(cta**2*SU_A(msta1)+sta**2*SU_A(msta2) )    
     .       +6*(1.d0/3)*SU_A(msu1)+6*(1.d0/3)*SU_A(msd1)
     .       +2*(-1.d0)*SU_A(mse1)
     .       +2*(-1.d0)*SU_A(msn1) +(-1.d0)*SU_A(msntau) )
     .  +g1**2/4*(2.d0/3)*(    
     .        3*(-4.d0/3)*(st**2*SU_A(mst1)+ct**2*SU_A(mst2) )    
     .       +3*(2.d0/3)*(sb**2*SU_A(msb1)+cb**2*SU_A(msb2) )    
     .       +(2)*(sta**2*SU_A(msta1)+cta**2*SU_A(msta2) )    
     .       +6*(-4.d0/3)*SU_A(msu2)+6*(2.d0/3)*SU_A(msd2)
     .       +2*(2.d0)*SU_A(mse2)  )
c     
      crRRnino=0.d0    
      do i=1,4
      crRRnino=crRRnino+ fbbRR(i)*dble(SU_BG(pscale**2,gmn(i),rmd))
     .    -2*rmd*dxmn(i)*gbbRR(i)*dble(SU_B0(pscale**2,gmn(i),rmd))
      enddo
c
      crRRcino=0.d0
      do i=1,2
      crRRcino=crRRcino+ ftbRR(i)*dble(SU_BG(pscale**2,gmc(i),rmu))
     .    -2*rmu*gmc(i)*gtbRR(i)*dble(SU_B0(pscale**2,gmc(i),rmu))     
      enddo 
c
      crRR=-cpi*(crRRqcd+crRRyuk+crRRgau+crRRhyp+crRRnino+crRRcino)	    
...
c
       crLRqcd=16*pi*alphas/3*(
     . 4*rmd*m3*dble(SU_B0(pscale**2,m3,rmd))
     . +cd*sd*(dble(SU_BF(pscale**2,msd1gen,zero))-SU_A(msd1gen) 
     .        -dble(SU_BF(pscale**2,msd2gen,zero))+SU_A(msd2gen) ) )
c       
       crLRyuk=3*yd**2*sd*cd*(SU_A(msd1gen)-SU_A(msd2gen) )
     . +ghbLb1*ghbRb1*dble(SU_B0(pscale**2,mh,msd1gen))
     . +ghbLb2*ghbRb2*dble(SU_B0(pscale**2,mh,msd2gen))
     . +glbLb1*glbRb1*dble(SU_B0(pscale**2,ml,msd1gen))
     . +glbLb2*glbRb2*dble(SU_B0(pscale**2,ml,msd2gen))
     . +ggbLb1*ggbRb1*dble(SU_B0(pscale**2,mz,msd1gen))
     . +ggbLb2*ggbRb2*dble(SU_B0(pscale**2,mz,msd2gen))
     . +gabLb1*gabRb1*dble(SU_B0(pscale**2,ma,msd1gen))
     . +gabLb2*gabRb2*dble(SU_B0(pscale**2,ma,msd2gen))          
     . +gcbLt1*gcbRt1*dble(SU_B0(pscale**2,mch,msu1gen))
     . +gcbLt2*gcbRt2*dble(SU_B0(pscale**2,mch,msu2gen))
     . +ggbLt1*ggbRt1*dble(SU_B0(pscale**2,mw,msu1gen))
     . +ggbLt2*ggbRt2*dble(SU_B0(pscale**2,mw,msu2gen))
c
      crLRgau=(-g1*cw/3)**2*cd*sd*(dble(SU_BF(pscale**2,msd1gen,zero))
     .                          -dble(SU_BF(pscale**2,msd2gen,zero)) )
     .  -g**2/cw**2*(-.5d0+sw2/3)*(-sw2/3)*sd*cd*(
     .                           dble(SU_BF(pscale**2,msd1gen,mz))
     .                          -dble(SU_BF(pscale**2,msd2gen,mz)) )    
c
      crLRhyp=g1**2/4*(1.d0/3)*(2.d0/3)*sd*cd*(
     .                SU_A(msd1gen)-SU_A(msd2gen) ) 
c     
      crLRnino=0.d0    
      do i=1,4
      crLRnino=crLRnino+ fbbLR(i)*dble(SU_BG(pscale**2,gmn(i),rmd))
     .    -2.d0*rmd*dxmn(i)*gbbLR(i)*dble(SU_B0(pscale**2,gmn(i),rmd))
      enddo
c
      crLRcino=0.d0
      do i=1,2
      crLRcino=crLRcino+ ftbLR(i)*dble(SU_BG(pscale**2,gmc(i),rmu))
     .    -2*rmu*gmc(i)*gtbLR(i)*dble(SU_B0(pscale**2,gmc(i),rmu))     
      enddo 
c
      crLR=-cpi*(crLRqcd+crLRyuk+crLRgau+crLRhyp+crLRnino+crLRcino)	              
c
...
c++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        SUBROUTINE SU_STAUCR(pscale,mu, Ye, Ae,                 
     . mse1gen,mse2gen,msngen,thete,
     . crLL,crLR,crRR)  
c  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c  Calculates full one-loop radiative corrections (generically) to Slepton masses, 
c  similarly to squarks above in routines SU_STOPCR, SU_SBOTCR, a la PBMZ. (new in v2.5)
c  The input at the EWSB scale are, respectively: pscale: self-energy ext momentum,
c  the mu parameter, Ye (generic) Yukawa cplings, Ae (generic) trilinear couplings, 
c slepton/sneutrino masses mse1gen...msngen, mixing angle thete. 
c  Output: the radiative corrections to LL,LR,RR entries of the slepton mass matrix. 
c---------------------------------------------------------
c
      implicit real*8(a-h,m,o-z)
      complex*16 SU_B0,SU_BG,SU_BF
      dimension gmc(2),xmctree(2),gmn(4),dxmn(4),u(2,2),v(2,2),z(4,4),
     .  antauR(4),antauL(4),bntauL(4),bntauR(4),actauL(2),actauR(2),
     . bctauL(2),bctauR(2),
     .  ftautauLL(4),gtautauLL(4),ftautauLR(4),gtautauLR(4),
     . ftautauRR(4),gtautauRR(4), 
     .  fnutauLL(2),gnutauLL(2),fnutauLR(2),gnutauLR(2),fnutauRR(2),
     . gnutauRR(2)   
      common/SU_runhiggsewsb/ma,ml,mh,mch,alfa
      COMMON/SU_outginos/mc1,mc2,mn1,mn2,mn3,mn4,mgluino
      COMMON/SU_inoDR/U,V,Z,xmctree,dxmn
...
c   fix ren. scale (used in B0 functions):
      if(kpole.eq.1) scale = dsqrt(mst1*mst2)     
c
      if(msngen.eq.0.d0.or.mse1gen.eq.0.d0) goto 100
c   (NB protection: means msngen or mse1gen undefined yet)        
       sq2=dsqrt(2.d0)
       pi = 4*datan(1.d0)
       g=g2ewsb
       g1=g1ewsb
...
c----------- Higgs couplings
c 
      s2tauLtauL = -g*mz/cw*(-.5d0 +sw2)*cbet + sq2*ye*rme
      s2tauRtauR = -g*mz/cw*(-sw2)*cbet + sq2*ye*rme
      s2tauLtauR = ye/sq2*Ae
      s1tauLtauL = g*mz/cw*(-.5d0 +sw2)*sbet
      s1tauRtauR = g*mz/cw*(-sw2)*sbet
      s1tauLtauR = -ye/sq2*mu

      s2tauLtau1=ce*s2tauLtauL+se*s2tauLtauR
      s2tauLtau2=-se*s2tauLtauL+ce*s2tauLtauR
      s2tauRtau1=ce*s2tauLtauR+se*s2tauRtauR
      s2tauRtau2=-se*s2tauLtauR+ce*s2tauRtauR
      s1tauLtau1=ce*s1tauLtauL+se*s1tauLtauR
      s1tauLtau2=-se*s1tauLtauL+ce*s1tauLtauR
      s1tauRtau1=ce*s1tauLtauR+se*s1tauRtauR
      s1tauRtau2=-se*s1tauLtauR+ce*s1tauRtauR
  
      ghtauLtau1= cal*s1tauLtau1+sal*s2tauLtau1
      gltauLtau1=-sal*s1tauLtau1+cal*s2tauLtau1
      ghtauLtau2= cal*s1tauLtau2+sal*s2tauLtau2
      gltauLtau2=-sal*s1tauLtau2+cal*s2tauLtau2
      ghtauRtau1= cal*s1tauRtau1+sal*s2tauRtau1
      gltauRtau1=-sal*s1tauRtau1+cal*s2tauRtau1
      ghtauRtau2= cal*s1tauRtau2+sal*s2tauRtau2
      gltauRtau2=-sal*s1tauRtau2+cal*s2tauRtau2

      atauLtauR=-ye/sq2*(-mu*cbet-Ae*sbet)
      gtauLtauR=+ye/sq2*(-mu*sbet+Ae*cbet)

      gatauLtau1=se*atauLtauR
      gatauLtau2=ce*atauLtauR
      gatauRtau1=-ce*atauLtauR
      gatauRtau2=se*atauLtauR

      ggtauLtau1=se*gtauLtauR
      ggtauLtau2=ce*gtauLtauR
      ggtauRtau1=-ce*gtauLtauR
      ggtauRtau2=se*gtauLtauR
                    
      gctauLnutau = g*mw/sq2*sin(2*B)-ye*rme*sbet
      gctauRnutau = ye*(-mu*cbet-Ae*sbet)

      ggtauLnutau = -g*mw/sq2*cos(2*B)+ye*rme*cbet
      ggtauRnutau = ye*(-mu*sbet+Ae*cbet)
...
c
      ...
      bw2tauR=-ye
       ...
       do i=1,4
       aNtauR(i) = Z(i,1)*ap1tauR +Z(i,2)*ap2tauR +Z(i,3)*ap3tauR 
     . +Z(i,4)*ap4tauR	  
       bNtauR(i) = Z(i,1)*bp1tauR +Z(i,2)*bp2tauR +Z(i,3)*bp3tauR 
     . +Z(i,4)*bp4tauR	  
       aNtauL(i) = Z(i,1)*ap1tauL +Z(i,2)*ap2tauL +Z(i,3)*ap3tauL 
     . +Z(i,4)*ap4tauL	  
       bNtauL(i) = Z(i,1)*bp1tauL +Z(i,2)*bp2tauL +Z(i,3)*bp3tauL 
     . +Z(i,4)*bp4tauL	  
       enddo
c
       do i=1,2
       aCtauR(i) = V(i,1)*aw1tauR +V(i,2)*aw2tauR   
       bCtauR(i) = U(i,1)*bw1tauR +U(i,2)*bw2tauR   
       aCtauL(i) = V(i,1)*aw1tauL +V(i,2)*aw2tauL  
       bCtauL(i) = U(i,1)*bw1tauL +U(i,2)*bw2tauL  
       enddo
c
       do i=1,4
       ftautauLL(i) = aNtauL(i)*aNtauL(i) + bNtauL(i)*bNtauL(i) 
       gtautauLL(i) = bNtauL(i)*aNtauL(i) + aNtauL(i)*bNtauL(i) 
       ftautauLR(i) = aNtauL(i)*aNtauR(i) + bNtauL(i)*bNtauR(i) 
       gtautauLR(i) = bNtauL(i)*aNtauR(i) + aNtauL(i)*bNtauR(i) 
       ftautauRR(i) = aNtauR(i)*aNtauR(i) + bNtauR(i)*bNtauR(i) 
       gtautauRR(i) = bNtauR(i)*aNtauR(i) + aNtauR(i)*bNtauR(i) 
       enddo      
c 
       do i=1,2
       fnutauLL(i) = aCtauL(i)*aCtauL(i) + bCtauL(i)*bCtauL(i) 
       gnutauLL(i) = bCtauL(i)*aCtauL(i) + aCtauL(i)*bCtauL(i) 
       fnutauLR(i) = aCtauL(i)*aCtauR(i) + bCtauL(i)*bCtauR(i) 
       gnutauLR(i) = bCtauL(i)*aCtauR(i) + aCtauL(i)*bCtauR(i) 
       fnutauRR(i) = aCtauR(i)*aCtauR(i) + bCtauR(i)*bCtauR(i) 
       gnutauRR(i) = bCtauR(i)*aCtauR(i) + aCtauR(i)*bCtauR(i) 
       enddo      
c
c-------------------- LL contribution: 
c       
      crLLyuk=Ye*Ye*(se*se*SU_A(mse1gen)+ce*ce*SU_A(mse2gen) )
     . +0.5d0*(Ye*Ye*sal*sal-g*g*(-.5d0 +sw2)/2d0/cw2*
     .	    (-cos(2*alfa)) )*SU_A(mh) 
     . +0.5d0*(Ye*Ye*cal*cal-g*g*(-.5d0 +sw2)/2d0/cw2*
     .	    (cos(2*alfa)) )*SU_A(ml) 
     . +0.5d0*(Ye*Ye*cbet*cbet-g*g*(-.5d0 +sw2)/2d0/cw2*
     .	    (cos(2*B))    )*SU_A(mz) 
     . +0.5d0*(Ye*Ye*sbet*sbet-g*g*(-.5d0 +sw2)/2d0/cw2*
     .	     (-cos(2*B))     )*SU_A(ma) 
     . +( g*g*( (-.5d0 +sw2)/2d0/cw2 +.5d0)*
     . (cos(2*B)) )*SU_A(mch)
     . +( g*g*( (-.5d0 +sw2)/2d0/cw2 +.5d0)*
     . (-cos(2*B)) )*SU_A(mw)
     . +ghtauLtau1*ghtauLtau1*dble(SU_B0(pscale**2,mh,mse1gen))
     . +ghtauLtau2*ghtauLtau2*dble(SU_B0(pscale**2,mh,mse2gen))
     . +gltauLtau1*gltauLtau1*dble(SU_B0(pscale**2,ml,mse1gen))
     . +gltauLtau2*gltauLtau2*dble(SU_B0(pscale**2,ml,mse2gen))
     . +ggtauLtau1*ggtauLtau1*dble(SU_B0(pscale**2,mz,mse1gen))
     . +ggtauLtau2*ggtauLtau2*dble(SU_B0(pscale**2,mz,mse2gen))
     . +gatauLtau1*gatauLtau1*dble(SU_B0(pscale**2,ma,mse1gen))
     . +gatauLtau2*gatauLtau2*dble(SU_B0(pscale**2,ma,mse2gen))          
     . +gctauLnutau*gctauLnutau*dble(SU_B0(pscale**2,mch,msngen))
     . +ggtauLnutau*ggtauLnutau*dble(SU_B0(pscale**2,mw,msngen))  
c
      crLLgau=4*g*g/cw2*(-.5d0+sw2)*(-.5d0+sw2)*SU_A(mz)+2*g*g*SU_A(mw)
     .+(-g1*cw)*(-g1*cw)*(ce*ce*dble(SU_BF(pscale**2,mse1gen,zero))
     .		     +se*se*dble(SU_BF(pscale**2,mse2gen,zero)) )
     . +g*g/cw2*(-.5d0+sw2)*(-.5d0+sw2)*(
     .		           ce*ce*dble(SU_BF(pscale**2,mse1gen,mz))
     .			  +se*se*dble(SU_BF(pscale**2,mse2gen,mz)) )    
     .  +g**2/2*( dble(SU_BF(pscale**2,msngen,mw)) )    
     . +g*g/4*(ce*ce*SU_A(mse1gen)+se*se*SU_A(mse2gen)  
     .       +2*SU_A(msngen) )

      crLLhyp = g*g*(-0.5d0)*(
     .        3.d0*(+.5d0)*(ct**2*SU_A(mst1)+st**2*SU_A(mst2) )    
     .       +3.d0*(-.5d0)*(cb**2*SU_A(msb1)+sb**2*SU_A(msb2) )    
     .		      +(-.5d0)*(cta*cta*SU_A(msta1)+sta*sta*SU_A(msta2) )   
     .       +6.d0*(+.5d0)*SU_A(msu1)+6.d0*(-.5d0)*SU_A(msd1)
     .		      +(-.5d0)*2*SU_A(mSe1)
     .		      +(.5d0)*2*SU_A(mSn1) 
     .		      +(.5d0)*SU_A(msntau) )
     . +g1*g1/4*(-1d0)*(-1d0)*(ce*ce*SU_A(mse1gen)+se*se*SU_A(mse2gen) ) 
     . +g1*g1/4*(-1d0)*((-1d0)*(cta*cta*SU_A(msta1)+sta*sta*SU_A(msta2))    
     .       +3.d0*(1.d0/3.d0)*(ct**2*SU_A(mst1)+st**2*SU_A(mst2) )    
     .       +3.d0*(1.d0/3.d0)*(cb**2*SU_A(msb1)+sb**2*SU_A(msb2) )    
     .       +6.d0*(1.d0/3.d0)*SU_A(msu1)+6.d0*(1.d0/3.d0)*SU_A(msd1)
     .			+ (-1d0)*2*SU_A(mSe1)
     .			+ (-1d0)*2*SU_A(mSn1) 
     .			+ (-1d0)*SU_A(msntau) )
     . +g1*g1/4*(-1d0)*((2d0)*(sta*sta*SU_A(msta1)+cta*cta*SU_A(msta2))    
     .       +3.d0*(-4.d0/3.d0)*(st**2*SU_A(mst1)+ct**2*SU_A(mst2) )    
     .       +3.d0*(2.d0/3.d0)*(sb**2*SU_A(msb1)+cb**2*SU_A(msb2) )    
     .       +6.d0*(-4.d0/3.d0)*SU_A(msu2)+6.d0*(2.d0/3.d0)*SU_A(msd2)
     .			+(2d0)*2*SU_A(mSe2)  )
c     
       crLLnino=0.d0    
      do i=1,4
      crLLnino=crLLnino+ftautauLL(i)*dble(SU_BG(pscale**2,gmn(i),rme))
     . -2*rme*dxmn(i)*gtautauLL(i)*dble(SU_B0(pscale**2,gmn(i),rme))
      enddo
c
      crLLcino=0.d0
      do i=1,2
      crLLcino=crLLcino+ fnutauLL(i)*dble(SU_BG(pscale**2,gmc(i),zero))
      enddo 
cc
      crLL=-cpi*(crLLyuk+crLLgau+crLLhyp+crLLnino+crLLcino)
c       
c-------------------- RR contribution: 
c
c       
      crRRyuk=Ye*Ye*(ce*ce*SU_A(mse1gen)+se*se*SU_A(mse2gen) )
     . +0.5d0*(Ye*Ye*sal*sal-g*g*(-sw2)/2d0/cw2*
     .	   (-cos(2*alfa)) )*SU_A(mh) 
     . +0.5d0*(Ye*Ye*cal*cal-g*g*(-sw2)/2d0/cw2*
     .	   (+cos(2*alfa)) )*SU_A(ml) 
     . +0.5d0*(Ye*Ye*cbet*cbet-g*g*(-sw2)/2d0/cw2*
     .	   (cos(2*B))    )*SU_A(mz) 
     . +0.5d0*(Ye*Ye*sbet*sbet-g*g*(-sw2)/2d0/cw2*
     .	   (-cos(2*B))     )*SU_A(ma) 
     .  +(Ye*Ye*sbet*sbet+g*g*( (-sw2)/2d0/cw2)*
     . (cos(2*B)) )*SU_A(mch)
     . +(Ye*Ye*cbet*cbet+g*g*( (-sw2)/2d0/cw2)*
     . (-cos(2*B)) )*SU_A(mw)
     . +ghtauRtau1*ghtauRtau1*dble(SU_B0(pscale**2,mh,mse1gen))
     . +ghtauRtau2*ghtauRtau2*dble(SU_B0(pscale**2,mh,mse2gen))
     . +gltauRtau1*gltauRtau1*dble(SU_B0(pscale**2,ml,mse1gen))
     . +gltauRtau2*gltauRtau2*dble(SU_B0(pscale**2,ml,mse2gen))
     . +ggtauRtau1*ggtauRtau1*dble(SU_B0(pscale**2,mz,mse1gen))
     . +ggtauRtau2*ggtauRtau2*dble(SU_B0(pscale**2,mz,mse2gen))
     . +gatauRtau1*gatauRtau1*dble(SU_B0(pscale**2,ma,mse1gen))
     . +gatauRtau2*gatauRtau2*dble(SU_B0(pscale**2,ma,mse2gen))          
     . +gctauRnutau*gctauRnutau*dble(SU_B0(pscale**2,mch,msngen))
     . +ggtauRnutau*ggtauRnutau*dble(SU_B0(pscale**2,mw,msngen))
c
      crRRgau=4*g*g/cw2*(-sw2)*(-sw2)*SU_A(mz)
     . +(-g1*cw)*(-g1*cw)*(se*se*dble(SU_BF(pscale**2,mse1gen,zero))
     .		    +ce*ce*dble(SU_BF(pscale**2,mse2gen,zero)) )
     . +g*g/cw2*(-sw2)*(-sw2)*(
     .			 se*se*dble(SU_BF(pscale**2,mse1gen,mz))
     .			 +ce*ce*dble(SU_BF(pscale**2,mse2gen,mz)) )
c
      crRRhyp=
     . g1*g1/4*(2d0)*(2d0)*(se*se*SU_A(mse1gen)+ce*ce*SU_A(mse2gen) ) 
     . +g1*g1/4*(2d0)*((-1d0)*(cta*cta*SU_A(msta1)+sta*sta*SU_A(msta2) )    
     .       +3*(1.d0/3)*(ct**2*SU_A(mst1)+st**2*SU_A(mst2) )    
     .       +3*(1.d0/3)*(cb**2*SU_A(msb1)+sb**2*SU_A(msb2) )    
     .       +6*(1.d0/3)*SU_A(msu1)+6*(1.d0/3)*SU_A(msd1)
     .			  +(-1d0)*2*SU_A(mSe1)
     .			  +(-1d0)*2*SU_A(mSn1) 
     .			  +(-1d0)*SU_A(msntau) )
     . +g1*g1/4*(2d0)*((2d0)*(sta*sta*SU_A(msta1)+cta*cta*SU_A(msta2) )    
     .       +3*(-4.d0/3)*(st**2*SU_A(mst1)+ct**2*SU_A(mst2) )    
     .       +3*(2.d0/3)*(sb**2*SU_A(msb1)+cb**2*SU_A(msb2) )    
     .       +6*(-4.d0/3)*SU_A(msu2)+6*(2.d0/3)*SU_A(msd2)
     .			 +(2d0)*2*SU_A(mSe2)  )
c     
      crRRnino=0.d0    
      do i=1,4
      crRRnino=crRRnino+ftautauRR(i)*dble(SU_BG(pscale**2,gmn(i),rme))
     . -2*rme*dxmn(i)*gtautauRR(i)*dble(SU_B0(pscale**2,gmn(i),rme))
      enddo
c
      crRRcino=0.d0
      do i=1,2
      crRRcino=crRRcino+ fnutauRR(i)*dble(SU_BG(pscale**2,gmc(i),zero))
      enddo 
c
      crRR=-cpi*(crRRyuk+crRRgau+crRRhyp+crRRnino+crRRcino)	    

c-------------------- LR contribution: 
c       
      crLRyuk= Ye*Ye*se*ce*(SU_A(mse1gen)-SU_A(mse2gen) )
     . +ghtauLtau1*ghtauRtau1*dble(SU_B0(pscale**2,mh,mse1gen))
     . +ghtauLtau2*ghtauRtau2*dble(SU_B0(pscale**2,mh,mse2gen))
     . +gltauLtau1*gltauRtau1*dble(SU_B0(pscale**2,ml,mse1gen))
     . +gltauLtau2*gltauRtau2*dble(SU_B0(pscale**2,ml,mse2gen))
     . +ggtauLtau1*ggtauRtau1*dble(SU_B0(pscale**2,mz,mse1gen))
     . +ggtauLtau2*ggtauRtau2*dble(SU_B0(pscale**2,mz,mse2gen))
     . +gatauLtau1*gatauRtau1*dble(SU_B0(pscale**2,ma,mse1gen))
     . +gatauLtau2*gatauRtau2*dble(SU_B0(pscale**2,ma,mse2gen))          
     . +gctauLnutau*gctauRnutau*dble(SU_B0(pscale**2,mch,msngen))
     . +ggtauLnutau*ggtauRnutau*dble(SU_B0(pscale**2,mw,msngen))
c
      crLRgau=(-g1*cw)**2*ce*se*(dble(SU_BF(pscale**2,mse1gen,zero))
     .                     -dble(SU_BF(pscale**2,mse2gen,zero)) )
     . -g*g/cw2*(-.5d0+sw2)*(-sw2)*se*ce*(
     .			dble(SU_BF(pscale**2,mse1gen,mz))
     .	               -dble(SU_BF(pscale**2,mse2gen,mz)) )
c
      crLRhyp=g1*g1/4*(-1d0)*(2d0)*se*ce*(
     .	        	 SU_A(mse1gen)-SU_A(mse2gen) )
c     
      crLRnino=0.d0    
      do i=1,4
      crLRnino=crLRnino+ftautauLR(i)*dble(SU_BG(pscale**2,gmn(i),rme))
     . -2*rme*dxmn(i)*gtautauLR(i)*dble(SU_B0(pscale**2,gmn(i),rme))
      enddo
c
      crLRcino=0.d0
      do i=1,2
      crLRcino=crLRcino+ fnutauLR(i)*dble(SU_BG(pscale**2,gmc(i),zero))
      enddo 
c
      crLR=-cpi*(crLRyuk+crLRgau+crLRhyp+crLRnino+crLRcino)	              
c
 100  continue	              
      end
c++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        SUBROUTINE SU_SNUTAUCR(pscale,mu, Ye, Ae,                 
     . mse1gen,mse2gen,msngen,thete,
     . crLL)  
c  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c  Calculates full one-loop radiative corrections generically to sneutrino mass, 
c  similarly to sleptons, a la PBMZ. (new in v2.5)
c  The input at the EWSB scale are, respectively: pscale: self-energy ext momentum,
c  the mu parameter, Ye (generic) Yukawa cplings, Ae (generic) trilinear couplings, 
c slepton/sneutrino masses mse1gen...msngen, mixing angle thete. 
c  Output: the R.C. to the sneutrino_L mass^2. 
c---------------------------------------------------------
c
      implicit real*8(a-h,m,o-z)
      complex*16 SU_B0,SU_BG,SU_BF
      dimension gmc(2),xmctree(2),gmn(4),dxmn(4),u(2,2),v(2,2),z(4,4),
     . bnnutauL(2),acnutauL(2),bcnutauL(2),
     .  fnutauLL(2),ftaunutauLL(2),gtaunutauLL(2)
      common/SU_runhiggsewsb/ma,ml,mh,mch,alfa
      COMMON/SU_outginos/mc1,mc2,mn1,mn2,mn3,mn4,mgluino
      COMMON/SU_inoDR/U,V,Z,xmctree,dxmn
      COMMON/SU_yukaewsb/ytau,yb,yt,alsewsb,g2ewsb,g1ewsb
...
c----------- Higgs couplings
c 
      s2nutauLnutauL = -g*mz/cw*(.5d0)*sbet
      s1nutauLnutauL = g*mz/cw*(.5d0)*cbet
  
      ghnutauL= cal*s1nutauLnutauL+sal*s2nutauLnutauL
      glnutauL=-sal*s1nutauLnutauL+cal*s2nutauLnutauL
                        
      gcnutauLtauL = g*mw/sq2*sin(2*B) -Ye*rme*sbet
      gcnutauLtauR = Ye*(-mu*cbet-Ae*sbet)
 
      ggnutauLtauL = -g*mw/sq2*cos(2*B)+Ye*rme*cbet
      ggnutauLtauR = Ye*(-mu*sbet+Ae*cbet)
 
      gcnutauLtau1=ce*gcnutauLtauL+se*gcnutauLtauR 
      gcnutauLtau2=-se*gcnutauLtauL+ce*gcnutauLtauR
 
      ggnutauLtau1=ce*ggnutauLtauL+se*ggnutauLtauR 
      ggnutauLtau2=-se*ggnutauLtauL+ce*ggnutauLtauR
c
c----------- neutralino/chargino couplings:
c
      ap1nutauL = 0d0
      bp1nutauL = g1/sq2*(-1d0)
      ap2nutauL = 0d0
      bp2nutauL = sq2*g*(.5d0)
      ap3nutauL = 0d0
      bp3nutauL = 0d0
      ...
      bw2nutauL=-Ye
       ...
       do i=1,2
       bNnutauL(i) = Z(i,1)*bp1nutauL +Z(i,2)*bp2nutauL
       enddo
c
       do i=1,2
       aCnutauL(i) = V(i,1)*aw1nutauL +V(i,2)*aw2nutauL  
       bCnutauL(i) = U(i,1)*bw1nutauL +U(i,2)*bw2nutauL  
       enddo
c
       do i=1,2
       fnutauLL(i) =  bNnutauL(i)*bNnutauL(i) 
       enddo      
c 
       do i=1,2
       ftaunutauLL(i) = aCnutauL(i)*aCnutauL(i) +bCnutauL(i)*bCnutauL(i) 
       gtaunutauLL(i) = bCnutauL(i)*aCnutauL(i) +aCnutauL(i)*bCnutauL(i) 
       enddo      
c
c-------------------- LL contribution: 
c       
      crLLyuk=
     . Ye*Ye*(se*se*SU_A(mse1gen)+ce*ce*SU_A(mse2gen) )     
     . +0.5d0*(               -g*g*(.5d0)/2.d0/cw2*
     .	    (-cos(2*alfa)) )*SU_A(mh) 
     . +0.5d0*(               -g*g*(.5d0)/2.d0/cw2*
     .	    (+cos(2*alfa)) )*SU_A(ml) 
     . +0.5d0*(              -g*g*(.5d0)/2.d0/cw2*
     ...
     . +0.5d0*(             -g*g*(.5d0)/2.d0/cw2*
     .	     (cos(2*B))     )*SU_A(ma) 
     . +(Ye*Ye*sbet*sbet+g*g*( (.5d0)/2.d0/cw2-.5d0)*
     . (-cos(2*B)) )*SU_A(mch)
     . +(Ye*Ye*cbet*cbet+g*g*( (.5d0)/2.d0/cw2-.5d0)*
     . (cos(2*B)) )*SU_A(mw)
     . +ghnutauL*ghnutauL*dble(SU_B0(pscale**2,mh,msngen))
     . +glnutauL*glnutauL*dble(SU_B0(pscale**2,ml,msngen))
     . +gcnutauLtau1*gcnutauLtau1*dble(SU_B0(pscale**2,mch,mse1gen))
     . +gcnutauLtau2*gcnutauLtau2*dble(SU_B0(pscale**2,mch,mse2gen))
     . +ggnutauLtau1*ggnutauLtau1*dble(SU_B0(pscale**2,mw,mse1gen))
     . +ggnutauLtau2*ggnutauLtau2*dble(SU_B0(pscale**2,mw,mse2gen))
c
      crLLgau=4*g*g/cw2*(.5d0)*(.5d0)*SU_A(mz)+2*g*g*SU_A(mw)
     . +g*g/cw2*(.5d0)*(.5d0)* dble(SU_BF(pscale**2,msngen,mz))     
     . +g*g/2*( ce*ce*dble(SU_BF(pscale**2,mse1gen,mw))    
     .	    +se*se*dble(SU_BF(pscale**2,mse2gen,mw)) )    
     . +g*g/4*(SU_A(msngen) 
     .	     +2*(ce*ce*SU_A(mse1gen)+se*se*SU_A(mse2gen) ) )
     ...
     .		      +(.5d0)*2*SU_A(mSn1) 
     .		      +(.5d0)*2*SU_A(msntau) )
     . +g1*g1/4*(-1.d0)*(-1.d0)*(SU_A(msngen) ) 
     . +g1*g1/4*(-1.d0)*(  
     .        3.d0*(1.d0/3.d0)*(ct**2*SU_A(mst1)+st**2*SU_A(mst2) )    
     .       +3.d0*(1.d0/3.d0)*(cb**2*SU_A(msb1)+sb**2*SU_A(msb2) )    
     .		+(-1.d0)*(cta*cta*SU_A(msta1)+sta*sta*SU_A(msta2) )    
     .       +6.d0*(1.d0/3.d0)*SU_A(msu1)+6.d0*(1.d0/3.d0)*SU_A(msd1)
     .			+ (-1.d0)*2*SU_A(mSe1)
     .			+ (-1.d0)*2*SU_A(mSn1) 
     .			+ (-1.d0)*SU_A(msntau) )
...
c     
       crLLnino=0.d0    
      do i=1,2
      crLLnino=crLLnino+fnutauLL(i)*dble(SU_BG(pscale**2,gmn(i),zero))
      enddo
c
      crLLcino=0.d0
      do i=1,2
      crLLcino=
     . crLLcino+ftaunutauLL(i)*dble(SU_BG(pscale**2,gmc(i),rme))
     .-2*rme*gmc(i)*gtaunutauLL(i)*dble(SU_B0(pscale**2,gmc(i),rme)) 
      enddo 
cc
      crLL=-cpi*(crLLyuk+crLLgau+crLLhyp+crLLnino+crLLcino)
c       
...
c  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c  Calculates the full one-loop radiative correction to the gluino mass, delgino.
c  The input parameters at EWSB scale are: 
...
c
      if(kpole.eq.1) scale = dsqrt(mst1*mst2)
       ...
       mss2=msd2
c
       do iter=1,2    ! iterating twice to approximately obtain pole mass
       if(iter.eq.1) then
       M3save=M3
       else
       M3= dabs(m3)/(1.d0 -delgino/dabs(m3))
       endif
       sumB1= 
...
c
c   +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c   +++++++ End of the routines for gaugino, sfermion masses ++++++++++++
c
c  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
c  option where the calculation is made in an approximation based on
c  Heinemeyer, Hollik, Weiglein (hep-ph/0002213), which is fast
...
c defining s^2_w at EWSB scale:
       cw = 1d0/dsqrt(1d0+(g1ewsb/g2ewsb)**2)
       sw = g1ewsb/g2ewsb*cw
       sw2=sw**2
       cw2= cw**2
       cwm2 =1d0/cw2
...
c
       ...
       sta=dsin(thel)
c
       cbeta2=1d0/(1d0+tbeta**2)
       cbet= dsqrt(cbeta2)
       sbet=dsqrt(1d0-cbeta2)
       c2b =2*cbeta2-1d0
c        
       if(su_isNaN(pizzp).or.mz**2+pizzp.le.0d0) then  !!added protection
       pizzp=0d0
       if(irge.eq.irgmax)  inonpert=-1
       endif
c
...
c     compute running masses 
       vd2 = 2*(mz**2+pizzp)/(g1ewsb**2+g2ewsb**2)/(1d0+tbeta**2)
...
c      
     ...
     .  *dble(SU_B0(qsz,mmu,mmu))+mtau**2*dble(SU_B0(qsz,mtau,mtau)))
c     
      pizzb = -2*cw2**2*(2*qsz+mw**2-mz**2*sw2**2/cw2)
     . *dble(SU_B0(qsz,mw,mw))
     . -(8*cw2**2+(cw2-sw2)**2)*dble(SU_BT22(qsz,mw,mw))
c
      pizzh0=-dble(SU_BT22(qsz,mz,ml))-mz**2*dble(SU_B0(qsz,mz,ml))
      pizzhS = -dsin(beta-alfa)**2*(dble(SU_BT22(qsz,ma,mh))
     .  + dble(SU_BT22(qsz,mz,ml))-mz**2*dble(SU_B0(qsz,mz,ml)) )
     .        -dcos(beta-alfa)**2*(dble(SU_BT22(qsz,mz,mh))
     .  + dble(SU_BT22(qsz,ma,ml))-mz**2*dble(SU_B0(qsz,mz,mh)) )
     .  -(cw2-sw2)**2*dble(SU_BT22(qsz,mch,mch))
     .  -pizzh0
c
       pizzsu= -12*( (.5d0-2*sw2/3)*dcos(thet)**2
     .-(2*sw2/3)*dsin(thet)**2 )**2*dble(SU_BT22(qsz,mst1,mst1))
...
c
     ...
     .  *dble(SU_B0(qsz,mmu,mmu))+mtau**2*dble(SU_B0(qsz,mtau,mtau)))
c
      pizzb0 = -2*cw2**2*(2*qsz+mw**2-mz**2*sw2**2/cw2)
     . *dble(SU_B0(qsz,mw,mw))
     . -(8*cw2**2+(cw2-sw2)**2)*dble(SU_BT22(qsz,mw,mw))
c
      pizzh00=-dble(SU_BT22(qsz,mz,ml))-mz**2*dble(SU_B0(qsz,mz,ml))
...
c
     ...
     . +dble(SU_BH(qsw,mmu,eps))+dble(SU_BH(qsw,mtau,eps)))
c
      piwwb=-(1.d0+8*cw2)*dble(SU_BT22(qsw,mz,mw))-sw2*(
     . 8*dble(SU_BT22(qsw,mw,eps))+4*qsw*dble(SU_B0(qsw,mw,eps)))
     . -((4*qsw+mz**2+mw**2)*cw2-mz**2*sw2**2)
     . *dble(SU_B0(qsw,mz,mw))
c
      piwwh0=-   dble(SU_BT22(qsw,ml,mw))-mw**2*dble(SU_B0(qsw,ml,mw))         
      piwwhS = -dsin(beta-alfa)**2*(dble(SU_BT22(qsw,mh,mch))
...
c
     ...
     . +dble(SU_BH(qsw,mmu,eps))+dble(SU_BH(qsw,mtau,eps)))
c
      piwwb0=-(1.d0+8*cw2)*dble(SU_BT22(qsw,mz,mw))-sw2*(
     . 8*dble(SU_BT22(qsw,mw,eps))+4*qsw*dble(SU_B0(qsw,mw,eps)))
     . -((4*qsw+mz**2+mw**2)*cw2-mz**2*sw2**2)
     . *dble(SU_B0(qsw,mz,mw))
c
...
c Sum of the susy contributions for piww and final piww(0)  
      ...
      piww0=piwwsm0+piwwsusy0
c
c%%%%%% (Re)define vu,vd and running masses because pizz changed:
c thus (re)defining mtau,mb,mt running masses at ewsb scale:

      mz = mzsave             
      mw = mwsave              

       vd2 = 2*(mz**2+pizz)/(g1ewsb**2+g2ewsb**2)/(1d0+tbeta**2)
       vu2 = vd2*tbeta**2
       vd= dsqrt(vd2)
       vu= dsqrt(vu2)
       rmt=yt*vu
...
c
     ...
     .  3*(yt**2*cbet**2-g**2/cw2*0.5d0*(0.5d0-2*sw2/3)*dcos(2*b))*
     .   (ct**2*SU_A(mst1)+st**2*SU_A(mst2) ) +
     .  3*(yt**2*cbet**2-g**2/cw2*0.5d0*(2*sw2/3)*dcos(2*b))*
     .   (st**2*SU_A(mst1)+ct**2*SU_A(mst2) ) 
c     
        piaasup = 
     .  6*(-g**2/cw2*0.5d0*(0.5d0-2*sw2/3)*dcos(2*b))*SU_A(msu1)+
     .  6*(-g**2/cw2*0.5d0*(2*sw2/3)*dcos(2*b))*SU_A(msu2) 
c
        piaasbot=  3*2*gab1b2**2*dble(SU_B0(qsa,msb1,msb2))  +
     .  3*(yb**2*sbet**2 -g**2/cw2*0.5d0*(-0.5d0+sw2/3)*dcos(2*b))*
     .   (cb**2*SU_A(msb1)+sb**2*SU_A(msb2) ) +
     .  3*(yb**2*sbet**2 -g**2/cw2*0.5d0*(-sw2/3)*dcos(2*b))*
     .   (sb**2*SU_A(msb1)+cb**2*SU_A(msb2) ) 
c     
        piaasdo=  
     .  6*(-g**2/cw2*0.5d0*(-0.5d0+sw2/3)*dcos(2*b))*SU_A(msd1)+
     .  6*(-g**2/cw2*0.5d0*(-sw2/3)*dcos(2*b))*SU_A(msd2) 
c     
        piaastau= 2*gatau12**2*dble(SU_B0(qsa,msta1,msta2))+
     .  (ytau**2*sbet**2 -g**2/cw2*0.5d0*(-0.5d0+sw2)*dcos(2*b))*
     .   (cta**2*SU_A(msta1)+sta**2*SU_A(msta2) ) +
     .  (ytau**2*sbet**2 -g**2/cw2*0.5d0*(-sw2)*dcos(2*b))*
     .   (sta**2*SU_A(msta1)+cta**2*SU_A(msta2) ) 
c
        piaaslep=
     .  2*(-g**2/cw2*0.5d0*(-0.5d0+sw2)*dcos(2*b))*SU_A(mse1)+
     .  2*(-g**2/cw2*0.5d0*(-sw2)*dcos(2*b))*SU_A(mse2)
c
        piaasneu=
     .  2*(-g**2/cw2*0.5d0*(0.5d0)*dcos(2*b))*SU_A(msn1)+
     .  1*(-g**2/cw2*0.5d0*(0.5d0)*dcos(2*b))*SU_A(msntau)
c 
       piaas=piaastop+piaasbot+piaastau+piaasup+piaasdo+piaaslep
     .      +piaasneu
c      
       piaav = g**2/4* (2*dble(SU_BF(qsa,mch,mw))+
     . dsin(alfa-beta)**2*dble(SU_BF(qsa,MH,mz))/cw2+
     . dcos(alfa-beta)**2*dble(SU_BF(qsa,ml,mz))/cw2 )
     .       +g**2*mw**2/2*dble(SU_B0(qsa,mw,mch))
     .       +g**2*(2*SU_A(mw)+SU_A(mz)/cw2) 
c
      piaah=g**2/(4*cw2)/2*(
     . (3*dsin(2*beta)**2-1.d0)*SU_A(Mz)+3*dcos(2*beta)**2*SU_A(ma)+ 
     .   dcos(2*b)*dcos(2*alfa)*(SU_A(ml) -SU_A(MH)) +
     . 2*((cw2*(1d0+dsin(2*beta)**2)-sw2*dcos(2*beta)**2)*SU_A(mw)
...
c correction msn1 -> msntau:
     ...
     .  3*(yb**2*sbet**2-g**2/cw2*0.5d0*(0.5d0-2*sw2/3)*dcos(2*b)+
     . g**2/2*dcos(2*b))*(ct**2*SU_A(mst1)+st**2*SU_A(mst2) ) +
     .  3*(yt**2*cbet**2-g**2/cw2*0.5d0*(2*sw2/3)*dcos(2*b))*
     .   (st**2*SU_A(mst1)+ct**2*SU_A(mst2) ) +
     .  6*(-g**2/cw2*0.5d0*(0.5d0-2*sw2/3)*dcos(2*b)+g**2/2*
     .  dcos(2*b))*SU_A(msu1)+
     .  6*(-g**2/cw2*0.5d0*(2*sw2/3)*dcos(2*b))*SU_A(msu2) +
     .  (-g**2/cw2*0.5d0*(0.5d0)*dcos(2*b)+g**2/2*dcos(2*b))*
     .  2*SU_A(msn1) 
     . + (ytau**2*sbet**2-g**2/cw2*0.5d0*(0.5d0)*dcos(2*b)
     . + g**2/2*dcos(2*b))*SU_A(msntau) +
     .  3*(yt**2*cbet**2 +g**2/cw2*(-0.5d0)*(-0.5d0+sw2/3)*dcos(2*b)-
     . g**2/2*dcos(2*b))*(cb**2*SU_A(msb1)+sb**2*SU_A(msb2) ) +
     .  3*(yb**2*sbet**2 +g**2/cw2*(-0.5d0)*(-sw2/3)*dcos(2*b))*
     .   (sb**2*SU_A(msb1)+cb**2*SU_A(msb2) ) +
     .  6*(g**2/cw2*(-0.5d0)*(-0.5d0+sw2/3)*dcos(2*b) -g**2/2*
     .  dcos(2*b))*SU_A(msd1)+
     .  6*( g**2/cw2*(-0.5d0)*(-sw2/3)*dcos(2*b))*SU_A(msd2) +
     .  (ytau**2*cbet**2 +g**2/cw2*(-0.5d0)*(-0.5d0+sw2)*dcos(2*b)-
     . g**2/2*dcos(2*b))*(cta**2*SU_A(msta1)+sta**2*SU_A(msta2) ) +
     .  (ytau**2*sbet**2 +g**2/cw2*(-0.5d0)*(-sw2)*dcos(2*b))*
     .   (sta**2*SU_A(msta1)+cta**2*SU_A(msta2) ) +
     .  2*( g**2/cw2*(-0.5d0)*(-0.5d0+sw2)*dcos(2*b) -g**2/2*
     .  dcos(2*b))*SU_A(mse1) +
     .  2*( g**2/cw2*(-0.5d0)*(-sw2)*dcos(2*b))*SU_A(mse2)
c     
       piccv = g**2/4*(dsin(alfa-b)**2*dble(SU_BF(qsc,MH,mw)) +
     .   dcos(alfa-b)**2*dble(SU_BF(qsc,ml,mw))+dble(SU_BF(qsc,MA,mw))
     . +(cw2-sw2)**2/cw2 *dble(SU_BF(qsc,mch,mz)) )
     . +4*pi*alph* dble(SU_BF(qsc,mch,eps))+2*g**2*SU_A(mw)+
     . g**2*(cw2-sw2)**2/cw2 *SU_A(mz) +
     . g**2*mw**2/4* dble(SU_B0(qsc,mw,ma)) 
c
       picch4= g**2/(4*cw2)/2*(
     . (cw2*(1.d0+dsin(2*b)**2) -sw2*dcos(2*b)**2)*SU_A(mz) +
     . dcos(2*b)**2*SU_A(mA) +
     . (cw2*(1.d0+dsin(2*b)*dsin(2*alfa))
     . -sw2*dcos(2*b)*dcos(2*alfa))*SU_A(MH) +  
     . (cw2*(1.d0-dsin(2*b)*dsin(2*alfa))
     . +sw2*dcos(2*b)*dcos(2*alfa))*SU_A(ml) ) +
     . g**2/(4*cw2)* ( 
     . (2*dsin(2*b)**2-1.d0)*SU_A(mw) +2*dcos(2*b)**2*SU_A(mch) )  
c 
      picch3= g**2*mz**2/(4*cw2)/2 *(
     .(cal*(-dsin(2*b)*cbet+cw2*sbet)+
     . sal*(dsin(2*b)*sbet-cw2*cbet))**2*dble(SU_B0(qsc,MH,mw)) +
     .(-sal*(-dsin(2*b)*cbet+cw2*sbet)+
     . cal*(dsin(2*b)*sbet-cw2*cbet))**2*dble(SU_B0(qsc,ml,mw)) +
     .(cal*(-dcos(2*b)*cbet+2*cw2*cbet)+
     . sal*(dcos(2*b)*sbet+2*cw2*sbet))**2*dble(SU_B0(qsc,MH,mch)) +
     .(-sal*(-dcos(2*b)*cbet+2*cw2*cbet)+
     . cal*(dcos(2*b)*sbet+2*cw2*sbet))**2*dble(SU_B0(qsc,ml,mch)) )
...
c
       dt1v1h=-g**2*dcos(2*b)/(8*cw2)*(SU_A(ma)+2*SU_A(mch))
     . +g**2/2*SU_A(mch)
     . +g**2/(8*cw**2)*(3*sal**2-cal**2+s2al*tbeta)*SU_A(ml)
     . +g**2/(8*cw**2)*(3*cal**2-sal**2-s2al*tbeta)*SU_A(mh)

       dt1v1v=
     . +3*g**2/4.d0*(2*SU_A(mw)+SU_A(mz)/cw2) 
     . +g**2*dcos(2*b)/(8*cw2)*(2*SU_A(mw)+SU_A(mz))
...
c
       dt2v2b=g**2*dcos(2*b)/(8*cw2)*(SU_A(ma)+2*SU_A(mch))
     . +g**2/2*SU_A(mch)
     . +g**2/(8*cw2)*(3*cal**2-sal**2+s2al/tbeta)*SU_A(ml)
     . +g**2/(8*cw2)*(3*sal**2-cal**2-s2al/tbeta)*SU_A(mh)
...
c   vd gaugino contributions:
       dvdino=0d0
       do i=1,4
       dvdino = dvdino -g**2/mw/cbet*
     .  dxmn(i)*Z(i,3)*(Z(i,2)-Z(i,1)*sw/cw)*SU_A(gmn(i))
       enddo
       dvdino=dvdino
     . -dsqrt(2d0)*g**2/mw/cbet*(gmc(1)*V(1,1)*U(1,2)*SU_A(gmc(1)) 
     .                           +gmc(2)*V(2,1)*U(2,2)*SU_A(gmc(2))  )
c
...
c   vu gaugino contributions:
       dvuino=0d0
       do i=1,4
       dvuino = dvuino +g**2/mw/sbet*
     . dxmn(i)*Z(i,4)*(Z(i,2)-Z(i,1)*sw/cw)*SU_A(gmn(i))
       enddo
       dvuino= dvuino
     . -dsqrt(2d0)*g**2/mw/sbet*(gmc(1)*V(1,2)*U(1,1)*SU_A(gmc(1)) 
     .                           +gmc(2)*V(2,2)*U(2,1)*SU_A(gmc(2)) )
c     
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
c  +3 parameters (vu, vd, B) which are in fact dependent of others.
cc  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
c def. of EXTPAR names:
	...
	extcom(26) = ' MA_pole'
      ...
      write(nout,50) "                              ====================
     .="
      write(nout,50) "                             | SuSpect 2.5.1 OUTPUT 
     . |"
      write(nout,50) "                              ====================
     .="
      write(nout,105)
...
c -------------------------------------------- c
      ...
      write(nout,61) 2,'2.5.1         # version number'
c
